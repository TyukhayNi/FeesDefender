...
- Hoja **protegida**: editables solo E, I, K, M (y J por el render). El resto bloqueado. `autoFilter` activo en **`B3:M103`**, que es hasta donde llegan los 88 IDs. Hasta el 2026-09-11 era `B3:M88` y dejaba **12 preguntas fuera** —Team leader, Escritura y Reclamación—, así que el guion que salía del filtro no era el cuestionario (`MEJORAS #243`). El rango vive en **dos** sitios: el `autoFilter` de la hoja y el nombre definido oculto `_xlnm._FilterDatabase` del libro; cambiar uno y no el otro deja el filtro a medias.
