...
El tipo condiciona:
- **MOTIVOS DE IMPAGO** (`H12`): tiene sentido en `BAD_DEBT` y `NEGATIVA_*`. **Regla de relleno (cerrada): vacío por defecto; se rellena SOLO si hay postura del deudor documentada y anclada a fuente** — y esto aplica también a `VUELTA` (reconcilia el handoff con el caso real [inmueble]). Frase telegráfica EN MAYÚSCULAS y con terminología propietario/buscador (normaliza aunque el documento diga "comprador/vendedor"). En defensivos, vacío.
