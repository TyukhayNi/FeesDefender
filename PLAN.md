...
| 38 | [Con qué modelo corre la revisión adversarial: política provisional y su calibración](#siguiente-modelo-revisor-con-qué-modelo-corre-la-revisión-adversarial) | **política ✅ ESCRITA (PR [#395](https://github.com/TyukhayNi/FeesDefender/pull/395)), con la R1 de Codex adjudicada —`REQUIERE-REVISION`, 10 de 11 confirmados y remediados— y la fila de la gobernanza corregida por Nikolai a Sol·alto; ⏳ calibración 4 de 5** (ledger en su sección; la celda decía 0 hasta el 2026-09-26, con dos filas ya escritas) | encargo de Nikolai del 2026-09-23 sobre el informe de consumo de catorce días. **Disparador de cierre:** cinco encargos **ordinarios** corridos con `gpt-6-sol`·`high`, con su ficha en el ledger de abajo. Mientras no estén los cinco, la política es **provisional** y así se declara en `CLAUDE.md` | bajo |
...
| 47 | Las carpetas de E&V con el W-code DELANTE derivan su dirección, y la ciudad de SaRS1 se quita si es la del caso (`MEJORAS #301`) | **en construcción el 2026-09-26, junto a la fila #42** — [plan](docs/superpowers/plans/2026-09-26-token-drive-y-carpetas-w-delante-296-301.md). Promovida ese día por su propio disparador —«un censo de nombres de carpeta de E&V que diga qué plazas usan cada orden»—, hecho en solo lectura con la Drive API: 53 unidades, 231 carpetas con el W-code delante en muchas plazas (no solo Santander) y 331 con el W-code en medio sin guion, que siguen pidiendo el flag a propósito. Con el parser nuevo derivan 226 de las 231 | toca `core/intake_drive.py` y `scripts/abrir_caso.py`: una ronda sobre el diff, la misma que la fila #42 (`gpt-6-sol`·`high`, quinta fila de la calibración de la #38) | bajo |
...
**Ledger de calibración — 4 de 5.** Una fila por encargo **ordinario**, que es la fila de **una**
