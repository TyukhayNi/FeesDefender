...
| 20 | [Guardas de PII ciegas en worktree (`MEJORAS #161`)](#siguiente-pii-worktree-las-guardas-de-pii-solo-funcionan-en-la-raíz) | ✅ **CERRADA el 2026-09-05 (PR #289)** con las vías (1)+(2); la (3) fallar cerrado queda **diferida** con disparador escrito en `MEJORAS #161` | **disparador CONSUMADO el 2026-09-05**: una dirección de inmueble de la blocklist llegó a GitHub con el hook **en verde**; lo paró `leak-scan` en CI. Decidido por Nikolai el mismo día: avisar + resolver la lista desde el checkout principal. **R1 de Codex sobre el diff: NO-SHIP, 8 hallazgos, 8 confirmados, remediados** — el que más enseña: el aviso se imprimía y **pre-commit no lo mostraba** (solo enseña la salida de un hook que pasa si es `verbose`), así que la vía (1) era cosmética hasta la ronda. Adjudicación en `docs/superpowers/plans/2026-09-05-mejoras-161-blocklist-desde-la-raiz-comun.md` §4; acta literal en `docs/superpowers/specs/2026-09-05-mejoras-161-r1-adversarial-review.md` | bajo |
...
| 27 | [El tipo de un documento se decide por sus BYTES (`MEJORAS #214` + `#215`)](#siguiente-tipo-por-bytes-el-tipo-de-un-documento-se-decide-por-sus-bytes-no-por-el-nombre-mejoras-214--215) | **✅ COMPLETA — piezas A y B construidas y revisadas** — el sniff abre el contenedor (`PK` no es `.docx`: se lee el índice del zip y el `mimetype` de ODF, acotado), 16 tests y 2 mutantes. **Dos correcciones a la prosa, medidas contra la fuente:** un `.docx` va a `nativo` y **no** a `ofimatica` como decían el plan y `#215`; y `#215` **le quitó la premisa a la regla 2 del titular de duplicados** —con el DOCX sin extensión ya reconocido, las dos copias saben extraer— así que D9 se re-premisa con un RTF (no se relajó el aserto: se le devolvió un caso que la ejercite) y el mundo nuevo se contrata en D9c. **Ronda adversarial de la pieza B: sigue SIN correr — cobertura AUSENTE, no refutada**, y la R1 del 2026-09-13 no la cubre: su objeto fue el diff `9098731..436f23d`, o sea la pieza A. El verde de las dos semillas, que a la pieza B le faltaba por haberse construido en el contenedor Linux remoto, **ya está medido** en el cierre de la pieza A. **Pieza A (`MEJORAS #214`, punto 1) ✅ PR #358:** `hash_tree_local` devuelve `ArbolLocal(hashes, sin_verificar, renombrados)` — enumera con `os.walk(onerror=…)` porque `rglob` **suprime** los errores de recorrido, adopta el renombrado del montaje bajo su clave **efectiva**, y **declara** lo que no pudo leer en los tres destinos (resultado, evento forense y un `Pendiente` de V1 que **no tumba la etapa**). **R1 de Codex: NO-SHIP, 12 hallazgos, 12 confirmados, 0 refutados** — y no eran doce defectos sueltos sino **cuatro fronteras**: comparar nombres de fichero por igualdad de cadena en un FS que no distingue caja (H-01 ALTO: adoptaba un fichero ya listado y metía el hash de otro documento bajo dos claves); la declaración construida al final de funciones con cinco salidas (H-02 ALTO: el evento solo se escribía `if plan.con_sha`, así que **la corrida en que ningún fichero se pudo leer era la única sin rastro en el ledger**); el recorrido afirmando más de lo que mide (H-03 ALTO: el `stat()` de `_inventario_desde_hashes` repetía la misma carrera dos líneas más abajo); y oráculos de un solo elemento (H-10/H-11, **hallazgos contra el arnés y no contra el código**). **23 mutantes, 23 muertos**, con el arnés ya en el repo (`tests/_mutantes_mejoras_214.py`) porque en la R1 el revisor no pudo verificar la cifra anterior y la declaró SIN VERIFICAR. Verde con las semillas 777 y 31337; cobertura del diff 97%. Plan, decisiones y adjudicación: [`2026-09-13-fila27-pieza-a-custodia-tolerante.md`](docs/superpowers/plans/2026-09-13-fila27-pieza-a-custodia-tolerante.md); acta: [`…-r1-adversarial-review.md`](docs/superpowers/plans/2026-09-13-fila27-pieza-a-r1-adversarial-review.md). **Vivo y declarado:** los puntos 2 y 3 de `#214` siguen abiertos —el 2 ya está construido FUERA del pull, en `verificar_apertura` C1/C2, y el 3 es diferible con su premisa corregida—, y de los cuatro recorridos-y-abre de esa frontera quedan dos, en `MEJORAS #249` y `#250`. La carrera REAL del montaje de Drive for Desktop sigue **sin verificar**: solo la comprueba una apertura sobre `G:` | disparador: **ya disparado** en la apertura de W-048U77 (2026-09-10), cuya carpeta de E&V traía 11 de 58 ficheros sin extensión. Hoy eso **bloquea la apertura** (`[APER-65]`) y además ensucia el expediente con duplicados en cada ronda. Promovida por decisión de Nikolai el 2026-09-10 | medio |
| 29 | Rescatar la verificación por hash del pull de Drive E&V (`MEJORAS #225`, vía (a)) | **✅ RESCATADA el 2026-09-13 (PR #360) — y NO por donde decía esta fila.** Las 838 líneas quedan **congeladas y publicadas** en el tag `wip/via-a-225-2026-09-10` (`776d531`): el original, auditable, sin mergear. Al leerlas resultó que hacen **lo mismo** que `verificar_apertura` C1/C2 —censo remoto + `sha256` contra Drive—, construido un día después por otra vía (API de Drive en vez de rclone). Pero traían una **medición** que C1/C2 no tenía, y al comprobarla contra la fuente **C1/C2 salieron rotos**: con un nombre acentuado, C1 acusaba a un fichero que está —imprimiendo dos rutas idénticas— y **C2 dejaba de verificarlo** diciendo que el remoto no publica hash. `MEJORAS #251`. Se rescató por tanto **la lección, no el módulo**: `clave_de_cruce` en la pieza que ya estaba encadenada, más la detección de colisiones y un diagnóstico del relleno de `#225` que **prueba** en vez de sospechar —C2 ya abre el fichero, así que rehashea sin la cola de ceros—. Y `MEJORAS #252`: V1 corre la verificación al terminar, porque hasta entonces **no la disparaba nadie**. **R1 de Codex** sobre el diff; **11 mutantes, 11 muertos** (`tests/_mutantes_mejoras_251.py`), verde con las semillas 777 y 31337, cobertura del diff 96%. **Lo que NO se rescató, declarado:** la función pura `comparar` del módulo huérfano —no tiene consumidor— y la idea de verificar **dentro del pull**, que se descartó para no duplicar C1/C2 ni meter red en el camino del intake (§3.4 del handoff de la fila #27) | decisión de Nikolai del 2026-09-13 | alto |
| 30 | El botón de Streamlit deja de llamar al constructor deprecado de la sala de lectura (`[APER-70]`) | ✅ **CERRADA el 2026-09-13 (PR [#364](https://github.com/TyukhayNi/FeesDefender/pull/364) + [#365](https://github.com/TyukhayNi/FeesDefender/pull/365) (`c9447ac`), que porta el arnés al repo y corrige dos números del primero: eran **18 mutantes, 18 muertos** —no 17 de 17: al exigir que cada mutante muera por SU test apareció **M08 mal apuntado**— y la suite es **5.470**, no 5.467)** — el expander se queda y **deja de escribir**: dice si la sala está montada, cuántos documentos tiene, qué artefactos le faltan y da el texto para pedir el montaje (`core/sala_lectura_estado.py`). Guard `tests/test_guard_ui_sin_deprecados.py`: la UI no importa **ningún** módulo deprecado. El CLI no se toca. **Desbloquea P4** | decisión de Nikolai del 2026-09-13: **gobierna la skill**. **Una premisa del encargo quedó REFUTADA**: «lo que obtienen no es la sala plana» dejó de ser cierto **once minutos** después de escribirse (`a545e43` 22:47:39 → PR #328 `a2e6676` 22:58:29). Lo medido es peor: el botón **casi nunca podía terminar** —el clasificador manda a residuo todo lo que no case ~30 palabras clave y `organizar` se detiene pidiendo una sesión de Claude, que su destinatario no tiene—. **R1 Codex NO-SHIP, 9/9 confirmados**: el ALTO decía que yo había calibrado el lector con la regla del motor que acababa de retirar (la skill pone el catálogo DENTRO de la sala), y el H-03, que mi test «no toca un solo byte» daba **verde ante un `mkdir`**. Arnés 17/17. Plan y adjudicación: [`2026-09-13-fila30-boton-sala-lectura.md`](docs/superpowers/plans/2026-09-13-fila30-boton-sala-lectura.md) §5; acta en [`…-r1-adversarial-review.md`](docs/superpowers/plans/2026-09-13-fila30-r1-adversarial-review.md). **Tail operativo:** `.skill` v1.17 empaquetado en `dist/skills/`; **re-importarlo en Cowork es acción manual de Nikolai** | medio |
| 31 | Proceso: una sesión de apertura no toca el repo (P8 (a), (b) y (d)) | ✅ **CERRADA el 2026-09-14 (PR [#374](https://github.com/TyukhayNi/FeesDefender/pull/374), `f6ad4ab`)** — DEJÓ de ser «proceso, no código» el 2026-09-14**: nada obligaba al «al final» de (a), que es el defecto más medido de la casa (un hecho escrito que nadie aplica, tres veces en cinco días), así que Nikolai decidió que `session_close` **avise** de las aperturas sin fichar — aviso, nunca verja. (a) va al `RUNBOOK` (`[APER-73]`) con el formato literal del fichero de trabajo; (b) y (d) a `CLAUDE.md`, porque la sesión de reparación no lee el runbook de apertura. El fichero vive en `%LOCALAPPDATA%\FeesDefender\aperturas\`, hogar que ya usa `workspace_registry`, porque lo escribe una sesión y lo lee **otra**, días después y en otro worktree. **(c) —el lote en serie— sigue APARCADO.** [spec](docs/superpowers/specs/2026-09-14-p8-apertura-no-toca-el-repo-design.md) · [plan](docs/superpowers/plans/2026-09-14-p8-apertura-no-toca-el-repo.md) · [acta R1](docs/superpowers/specs/2026-09-14-p8-apertura-no-toca-el-repo-r1-adversarial-review.md) | decisión de Nikolai del 2026-09-13: **sí** a (a) la sesión de apertura ejecuta el runbook y **anota** los defectos para ficharlos al final leyendo `origin/main`; (b) como mucho una sesión de reparación en paralelo, dueña del código; (d) un bloque «aperturas del día» en la bitácora en vez de un cierre numerado por caso. **(c) —el lote en serie— se aparca**: es lo único que cuesta construir. **La evidencia de que hace falta apareció el mismo día:** las 838 líneas huérfanas de la fila #29 son exactamente una sesión de apertura que se puso a reparar y dejó el código sin commitear sobre una base que hoy está 21 commits atrás | medio |
| 32 | La sala de lectura se monta con lo que no sabe clasificar (P4: `[APER-61]` + la parada) | ✅ **CERRADA el 2026-09-14 (PR [#370](https://github.com/TyukhayNi/FeesDefender/pull/370), `530d033`)** — dos piezas sobre `core/sala_lectura.py`: (a) `[APER-61]`, la identidad y la titularidad dejan `06. PBC` por `01. ACTIVACIÓN` y la hoja de visita pasa a `03. OFERTAS` (**87 documentos cambian de categoría** sobre los 1.352 de los diez expedientes con catálogo: 59 de `06. PBC` a `01. ACTIVACIÓN`, 26 que salen del residuo —24 Anexos PBC que la tabla vieja no reconocía— y 2 que entran, los dos falsos positivos de substring); (b) `organizar` deja de abortar: el residuo recibe `08. PENDIENTE DE CLASIFICAR` con confianza 0.0 y la sala se monta entera. `detenido_por_residuo` se retira en favor de `n_pendientes`. **R1 de Codex sobre el diff: NO-SHIP, 7 hallazgos, 7 confirmados, todos remediados** — los dos ALTOS eran regresiones del propio diff y comparten frontera: apliqué el centinela nuevo a lo que se ESCRIBE y no a lo que se LEE, así que un `08` ya persistido con confianza 1.0 quedaba congelado e invisible. Adjudicación en [el spec](docs/superpowers/specs/2026-09-14-p4-sala-lectura-sin-parada-design.md) §4; acta literal en [`…-r1-adversarial-review.md`](docs/superpowers/specs/2026-09-14-p4-sala-lectura-sin-parada-r1-adversarial-review.md). Arnés `tests/_mutantes_p4.py`: 31 mutantes, 31 muertos, 1 superviviente declarado | **disparador medido, y es de tiempo del letrado:** la sala de W-030TZY se detuvo pidiendo clasificar **80** documentos a mano y la de W-02NHNC **21**, *antes de que existiera nada que leer* (`RUNBOOK [APER-67]`). **La propuesta LITERAL de P4 se midió y NO se hizo:** portar al motor la regla de la skill (`preclasificar.clasificar_por_patron`) manda el **90,5 %** a `07. RECLAMACIONES` y deja **0 de 123 fotos** en `00. FOTOS` —detecta imágenes por `screenshot|captura`, no por extensión—, y unir las dos reglas rescata **34 de 1.352**. El residuo no lo causa la regla: lo causa que el nombre no lleva la señal, y eso no se arregla con más patrones. Alcance decidido por Nikolai el 2026-09-14 (alinear dentro del módulo deprecado, sin extraer a uno compartido). **Lo que NO mejora, dicho:** la clasificación sigue igual; el 55 % de los nombres acaba en `08` en vez de en una parada — la ganancia es que la sala pasa de no existir a existir | medio |
| 33 | P6 — la ficha CRM y la actuación, sin YAML a mano y sin fundir a dos personas (`[APER-71]`, `[APER-63]`, `MEJORAS #209`) | ✅ **CERRADA el 2026-09-14 (PR [#371](https://github.com/TyukhayNi/FeesDefender/pull/371), `6e7e3a6`), revisada en TRES rondas** — tres piezas: (a) `resolver_parte` deja de resolver por email cuando hay un NIF que lo desmiente, y un documento pasa a tener TRES estados (ausente / utilizable / **no interpretable**); (b) `contrario:` admite lista y el CLI vincula **todos**; (c) `core/sudespacho_actuaciones.py` encapsula la receta de seis pasos con recibo reanudable. **TRES rondas, 36 hallazgos confirmados, 0 refutados** (8 + 19 + 9; los 19 de la R2 son los distintos de dos revisores en paralelo, que devolvieron 11 y 13 con cinco coincidentes). La **R3 fue excepcional, autorizada por Nikolai** sobre el techo de dos, y su objeto era la **remediación de la R2**: encontró que el remedio cerraba el caso y no la frontera en cinco sitios, y que el `37/37` del arnés eran **36 muertes y una coartada** —M29 llamaba a una regex retirada y moría por `NameError`—. Adjudicaciones en [el spec](docs/superpowers/specs/2026-09-14-p6-ficha-crm-y-actuaciones-design.md) §§6-8; actas literales con su digest: [`…-r1-…`](docs/superpowers/specs/2026-09-14-p6-ficha-crm-y-actuaciones-r1-adversarial-review.md), [`…-r2-…`](docs/superpowers/specs/2026-09-14-p6-ficha-crm-y-actuaciones-r2-adversarial-review.md), [`…-r2b-…`](docs/superpowers/specs/2026-09-14-p6-ficha-crm-y-actuaciones-r2b-adversarial-review.md) y [`…-r3-…`](docs/superpowers/specs/2026-09-14-p6-ficha-crm-y-actuaciones-r3-adversarial-review.md). Arnés: **39 mutantes, 39 muertos**, y ahora distingue morir por la propiedad de morir por estar roto. **Y lo que cerró la pieza no fue una cuarta ronda sino CORRERLA**: con 5.612 tests verdes, una ejecución contra el CRM real destapó tres defectos que ningún doble puede ver (`Prioridad: "Normal"` inexistente → HTTP 404, `fecha_alta` vacía y `precio_hora` a 0,00 con `SENIOR - …` en el asunto: **completa de aspecto y facturando cero**). Se añadieron, a petición de Nikolai y verificados en vivo, el **paso 7** que agenda el vencimiento en `calendario`, recordatorios e invitados, descripción, el eje de facturación y `cerrar_actuacion`. `MEJORAS #209` **cerrada**. Corrida en vivo: §9 del spec; contrato del CRM: `INTEGRACION_SUDESPACHO.md` §§15.8-15.12 | **disparador: dos defectos vivos sobre datos de cliente.** `[APER-71]` se midió el 2026-09-10 en el expediente 643: dos deudores que comparten el correo doméstico —un matrimonio, la norma entre propietarios— se fundían en una ficha, y `_completar_contrario_existente` rellenaba los huecos de la ajena. **Dos rondas** por el radio de daño (decide quién es quién). **Lo que la R1 más enseñó:** mi primer remedio *creaba* el estado que el propio código bloquea después, así que habría funcionado una corrida y bloqueado la siguiente para siempre. **Y dos premisas del handoff eran falsas:** el §15.6 ya estaba corregido, y `crm_ficha.py` no tenía dueña — verificado con `list_sessions` + `gh pr list` + `git ls-remote`, no leyendo `PLAN.md` | alto |
| 34 | P1/V2 — el lazo del CRM dentro de la secuencia de apertura | ✅ **CERRADA el 2026-09-14 (PR [#373](https://github.com/TyukhayNi/FeesDefender/pull/373), `c7ee86f`)** — **dos rondas, ambas NO-SHIP y ambas adjudicadas contra la fuente**: R1 sobre el PLAN (8 hallazgos, 8 confirmados — encontró tres llamadas que no compilaban y un agregador que ya existía, antes de escribir una línea) y R2 sobre el diff (7 hallazgos, 7 confirmados — el peor, que `verificar` se llamaba SIN fuentes y con `--crm api` habría dado `fallo` en toda corrida real). Más las **tres corridas contra el 636 verificadas POR LECTURA**: con `skip` cero escrituras, con `api` la actuación 21413, y relanzada **no duplicó** — que es lo único que ningún doble acredita. **La remediación de la R2 NO pasó ronda** (presupuesto agotado, y se dice). Deuda declarada: la ventana entre el efecto y su recibo, y la vida del recibo entre copias (`MEJORAS #260`, `#261`). Desbloqueada (estuvo parada unas horas por `MEJORAS #258`, que se cerró el mismo día: el `500` era la FORMA del parámetro, no la API). Lista para implementar — plan **rev. 2** escrito y con su **R1 adjudicada (NO-SHIP, 8 hallazgos, 8 confirmados, 0 refutados)**, que encontró tres llamadas que no compilaban y un agregador que ya existía. Se para porque **no hay lectura verificable del CRM** (`MEJORAS #258`): cuatro vías de lectura resultaron inertes, cada una con su control positivo fallando, así que la prueba de aceptación del plan —«releer y comprobar por resultado»— **no es ejecutable**. Cablear escrituras ahí sería aceptar la pieza con un `201`, que es lo que P6 midió que no basta. **Lo destapó una comprobación manual de Nikolai en la UI, no mis instrumentos.** Alcance ya recortado a `crm_alta` + `actuacion` + `verificar` (`crm_ficha` fuera: ejecuta los efectos de la §8.1, que es posterior a V3). Abre `MEJORAS #258`, `#259` y `#260`. [plan](docs/superpowers/plans/2026-09-14-apertura-v2-lazo-crm.md) · [acta R1](docs/superpowers/plans/2026-09-14-apertura-v2-lazo-crm-r1-adversarial-review.md) | **lista**; `#258` cerrada, la aceptación relee con `?properties=…` | alto |
| 35 | **[APERTURA-VIABILIDAD]** La corrida prepara el JSON de viabilidad y una sesión lo remata en un paso, y el correo entra en la secuencia | en PR #386, sin mergear — decisión de Nikolai el 2026-09-14, salida 3 de `MEJORAS #264` | Dos hallazgos medidos el 2026-09-15 mueven el alcance: el contrato del JSON que publica `MEJORAS #262` tiene **cuatro campos mal** (12.000 € se tiran en silencio y sale `OK`), y el **equipo comercial no es derivable** de `_ficha_crm.yaml` (10 fichas reales, 25 colaboradores, cero claves de rol). La corrida rellena **4 de los 12 campos** del contrato; el resto queda como residuo marcado. **No cierra `#264`:** la etapa `sala_lectura` sigue abierta. **Despliegue pendiente, fuera del repo:** `render_informe.py` corre en Cowork, no en el PC; sin re-empaquetar el `.skill` y re-importarlo a mano ahí, el aviso de claves desconocidas de esta salida no llega a producción. Spec: `docs/superpowers/specs/2026-09-15-corrida-prepara-sesion-remata-design.md` · Plan: `docs/superpowers/plans/2026-09-15-corrida-prepara-sesion-remata.md` · Acta R1 (Codex, NO-SHIP, 8/8 confirmados): `docs/superpowers/specs/2026-09-15-corrida-prepara-sesion-remata-r1-adversarial-review.md` | alto |
...
| 37 | El envío certificado por Codicert: burofax postal + correo + SMS a todos los requeridos, desde el CRM | **F1 ✅ CERRADA el 2026-09-21 (PR [#388](https://github.com/TyukhayNi/FeesDefender/pull/388), `6d60187`); F2 ✅ MERGEADA el 2026-09-21 (PR [#393](https://github.com/TyukhayNi/FeesDefender/pull/393), `37b6c3e`) —esta fila la daba «sin mergear» porque se escribió dos minutos antes del merge—; F3 ✅ MERGEADA el 2026-09-26 (PR [#398](https://github.com/TyukhayNi/FeesDefender/pull/398), `d2fd3f7`), construida el 2026-09-25 y revisada tres veces: R1 de Codex NO-SHIP, 7 hallazgos, los siete confirmados y remediados; R2 sobre esa remediación —autorizada por Nikolai— NO-SHIP, 7 hallazgos, los siete confirmados y remediados, y el aportable pasa a ser IMAGEN con capa de OCR; R3 sobre la remediación de la R2 —autorizada por Nikolai— NO-SHIP, 5 hallazgos, los cinco confirmados y remediados, y el PDF lo COMPONE el motor; la remediación de la R3 sin ronda (cobertura AUSENTE; una R4 solo con autorización de Nikolai).** Tres piezas en tres capas: `core/codicert.py` (transporte puro, no sabe qué es un expediente), `core/expedicion_certificada.py` (el criterio: planificar, ejecutar, refrescar, cosechar) y `scripts/codicert.py` (la única puerta humana, con plan que se lee antes de gastar). **Los tres canales son DOS endpoints**: `/envios/burofax` (exactamente un destinatario, 16,97 €) y `/envios/entrega-electronica-certificada` con `tipo_entrega` `correo`|`sms`. **Primera expedición real en sandbox** (1,06 € de los 20). **Dos rondas adversariales:** [R1](docs/superpowers/specs/2026-09-17-envio-certificado-codicert-r1-adversarial-review.md) sobre el diseño (REQUIERE-REVISION, 34 hallazgos) y [R2](docs/superpowers/plans/2026-09-17-codicert-f1-r2-adversarial-review.md) de **Codex** sobre el diff (**NO-SHIP, 16 hallazgos — 7 `alta`, 9 `media`, los dieciséis remediados**). Esa ronda **corrió** en vez de leer: sondas ejecutables por defecto y un mutante que sobrevivía a las 128 pruebas del ámbito. Seis de los siete altos eran **cobertura de mis propios tests**, no lógica que se me escapara — el más caro, `1apellido`/`2apellido` sin leer, habría impreso en el burofax **solo el nombre de pila del requerido, sin apellidos**. **La remediación de los dieciséis NO ha pasado ronda: la R3 quedó dispensada por decisión expresa de Nikolai el 2026-09-21, y su cobertura se declara AUSENTE —no refutada— en el §12.2 del spec.** **F2 (2026-09-21)** cierra la prueba del envío: `refrescar` (relee estados y agrupa **por requerido**, el nivel que manda para los plazos), `cosechar` (baja el certificado, **verifica que el remitente sea el emisor esperado** ex art. 17.2, lo archiva en `04_Output predemanda/Certificados` y lo cuelga del CRM), `core/certificado_lectura.py`, `core/sudespacho_documentos.py` (la subida del §17 con su verificación por resultado) y los subcomandos `estado` y `cosechar`. **Diez mediciones nuevas de producción, en solo lectura y 0,00 € gastados**, que llevaron el spec a rev. 12: el listado da un solo estado y el histórico da la fecha que cuenta —tres días de diferencia en un burofax real—, hay **seis códigos de estado vivos** que el §1.3 no clasificaba, y la razón social del emisor aparece **nueve veces en el certificado y una sola en el bloque bueno**, así que una verificación por búsqueda global habría aprobado el certificado de un tercero que nos mencione. **Una ronda adversarial** —la que la tabla de `CLAUDE.md` asigna a esta pieza—: [R1 de Codex sobre el diff](docs/superpowers/plans/2026-09-21-codicert-f2-r1-adversarial-review.md) (**NO-SHIP, 10 hallazgos — 3 `alta`, 7 `media`, los diez confirmados y remediados**). Dos de ellos los encontré yo antes de recibir el informe; el más caro de los otros ocho: **la recuperación de una subida fallida daba por bueno un binario que la corrida anterior había detectado incorrecto**. [plan de F2](docs/superpowers/plans/2026-09-21-codicert-f2.md). **F3 (2026-09-25)** produce **lo que va al juzgado**: `codicert aportable` deja junto a cada íntegro un `… - APORTABLE.pdf` —el certificado sin las páginas que reproducen las condiciones, sin firma ni sello de firma— y su `… - MANIFIESTO.json`, con las huellas tal como las lista el acta y las páginas retiradas en las tres numeraciones. Nace `core/certificado_aportable.py`, puro. **Diez mediciones sobre certificados y adjuntos reales** corrigieron dos premisas del spec (rev. 13): el motor **no compone** las condiciones, así que su texto se baja de Codicert y se verifica contra la huella del acta, y el requerimiento **sí lleva la deuda en euros**, así que el aviso de «sin cifras» habría saltado siempre y se rehízo sobre las frases de las propias condiciones. Tres paradas antes de entregar nada y **F3 nunca certifica que el íntegro sea aportable**. **Humo contra producción** en solo lectura sobre `W-04AKM2 - OVC`, 0,00 € gastados: la página de condiciones fuera **de lo visible** en los dos aportables — y **dentro del fichero**, que es lo que encontró la [R1 de Codex](docs/superpowers/plans/2026-09-25-codicert-f3-r1-adversarial-review.md) (H-01): los certificados reales comparten un `/Resources` con el formulario de cada página, y copiar las conservadas arrastraba el de las condiciones. Remediado —recursos podados, sin anotaciones, relectura del grafo entero— y re-verificado sin red sobre esos mismos certificados. **NO-SHIP, 7 hallazgos (2 `alta`, 5 `media`), los siete confirmados y remediados**; 26 de 26 mutantes muertos por su test. La [R2 de Codex](docs/superpowers/plans/2026-09-25-codicert-f3-r2-adversarial-review.md) sobre esa remediación (**NO-SHIP, 7 hallazgos — 1 `alta`, 5 `media`, 1 `baja` —, los siete confirmados y remediados**) enseñó **siete vías más** por las que el contenido retirado seguía dentro —patrones, fuentes, máscaras, metadatos—: la poda cerraba vías de una en una. **El aportable es ahora la IMAGEN de las páginas conservadas con una capa de texto de OCR** —lo que ninguna dibuja no está— y la relectura, una lista blanca. Sobre los reales, sin red: **ni un stream del íntegro sobrevive**, y correrlo destapó dos defectos del OCR real que el sintético no enseñaba (linealización por encima de 1 MB y `sys.stderr` secuestrado). La [R3 de Codex](docs/superpowers/plans/2026-09-25-codicert-f3-r3-adversarial-review.md), también autorizada por Nikolai (**NO-SHIP, 5 hallazgos — 2 `alta`, 3 `media` —, los cinco confirmados y remediados**), enseñó que la lista blanca admitía lo que no enumeraba —revisiones incrementales, objetos de otra generación, datos colgados de una fuente, páginas que guardan la imagen sin dibujarla—: la frontera era **admitir un fichero que no escribió el motor**. Desde ahí **el OCR (Tesseract directo) solo devuelve líneas y el aportable lo compone el motor**, y la relectura lo recompone y exige los mismos bytes, dibuja cada página y la casa contra todas las del íntegro. Sobre los reales, sin red: un segundo OCR da el **mismo fichero**. **La remediación de la R3 no ha pasado ronda.** **Los estados, medidos sobre los 117 envíos reales de `madrid.bd` (2026-09-26, solo lectura):** el SMS no tiene estados propios; el 22 sin ningún aviso entregado **cierra sin entrega** (decisión de Nikolai); un canal sin clasificar —el SMS Certificado— no se cosecha ni pone fechas al requerido; y lo no cosechable dice **su motivo** en `estado`, `cosechar` y el aportable, **estancado** incluido (más de 40 días quieto). Un solo veredicto cambia en los 119 envíos releídos. **Una ronda**, la que le toca: [R1 de Codex](docs/superpowers/plans/2026-09-26-codicert-estados-medidos-r1-adversarial-review.md) (`gpt-6-astra`·`medium`, **LISTA-CON-CAMBIOS, 2 hallazgos `baja`**, los dos confirmados y remediados en `b5c39ee`); **la remediación sin ronda (cobertura AUSENTE)**. [plan de los estados](docs/superpowers/plans/2026-09-26-codicert-estados-medidos.md) · `MEJORAS #305` (el aportable de un provisional). **Saneado, en el mismo PR:** el móvil, el email, el nombre y la empresa del tercero que el spec usaba de ejemplo desde F1 pasan a marcadores (spec rev. 18; el acta de la R1, redactada y declarada); historial sin purgar y ronda dispensada, por decisión de Nikolai; blocklist local ampliada y secret de CI republicado después del merge. **Siguiente:** la primera corrida real de `cosechar` + `aportable` sobre un expediente —las escrituras en el expediente no se han ejercido nunca—. Lo que no hace: la expedición **solo de burofax** (`MEJORAS #286`). [plan de F3](docs/superpowers/plans/2026-09-25-codicert-f3.md). | encargo de Nikolai del 2026-09-17. Predecesora: la fila **#36**, que produce el PDF refundido que esto expide. Gates duros medidos: el `id_personalizado` compuesto solo con el W-code hace que **la OVC posterior encuentre los envíos del requerimiento, se declare completa y no salga** (fallo silencioso y positivo), y los plazos de los arts. 7.3, 10.4 y 17.4 LO 1/2025 corren **por requerido**, no por expedición. [spec](docs/superpowers/specs/2026-09-17-envio-certificado-codicert-design.md) · [plan](docs/superpowers/plans/2026-09-17-codicert-f1.md) | alto |

| 38 | [Con qué modelo corre la revisión adversarial: la política y su calibración](#siguiente-modelo-revisor-con-qué-modelo-corre-la-revisión-adversarial) | **política ✅ ESCRITA (PR [#395](https://github.com/TyukhayNi/FeesDefender/pull/395)), con la R1 de Codex adjudicada —`REQUIERE-REVISION`, 10 de 11 confirmados y remediados— y la fila de la gobernanza corregida por Nikolai a Sol·alto; ✅ calibración 5 de 5; ✅ elección CONFIRMADA por Nikolai el 2026-09-26** —`gpt-6-sol`·`high` habitual y las filas de Astra como estaban—, sobre un resumen que omitía dos condiciones de la medición (no se reutilizaron objetos con defectos conocidos; cuatro de las cinco rondas recibieron el mandato sin caracteres no ASCII). Deja de decirse provisional en `CLAUDE.md` y en el contrato (rev. 12), PR [#411](https://github.com/TyukhayNi/FeesDefender/pull/411), `553ce5f`. **Sabiéndolas, la mantuvo el mismo día** y encargó otra medición (fila #50). Ajustar la política: **no**, lo decidió él ese mismo día; lo que diga la fila #50, con su criterio. Cerrar la calibración cumplió el disparador de `MEJORAS #280` y `#281`, promovidas a las filas #48 y #49 (ledger en su sección; la celda decía 0 hasta el 2026-09-26, con dos filas ya escritas) | encargo de Nikolai del 2026-09-23 sobre el informe de consumo de catorce días. **Disparador de cierre:** cinco encargos **ordinarios** corridos con `gpt-6-sol`·`high`, con su ficha en el ledger de abajo — **cumplido el 2026-09-26**; hasta entonces la política se declaró **provisional** en `CLAUDE.md` | bajo |

| 39 | Mejoras de las aperturas de la semana (2026-09-15 → 25): WhatsApp (`MEJORAS #236`, `#285`), `verificar_apertura` C2/C3/C9 (`#268`, `#269`, `#218`) y `_viabilidad.json` sobre `G:` (`#276`) | ✅ **CERRADA el 2026-09-25 (PR [#397](https://github.com/TyukhayNi/FeesDefender/pull/397), `4209582`)** — [plan y adjudicaciones](docs/superpowers/plans/2026-09-25-mejoras-aperturas-semana.md). **Dos piezas, dos rondas.** Bloque 1 con `gpt-6-sol`·`high`: `NO-SHIP`, **6 hallazgos, 6 confirmados, 0 refutados**, tres fronteras (qué identifica al chat de un export; limpiar un identificador no puede ligarlo a otros bytes; lo que el verificador declara tiene que ser lo que mira), remediados en `94522ce` y `61523e8`, y es la **primera fila de la calibración de la #38**. `#276` con `gpt-6-astra`·`high`: `SHIP`, 0 hallazgos, sus dos mutantes supervivientes muertos en `2b2653c`. El remedio de `#276` cambió al medir: `os.rename` en `G:` **no pisa** (`WinError 183`), así que en Windows se conservan las dos promesas y no hizo falta la ventana a medias de `O_EXCL`. **Backlog:** `#287` y `#288` rescatadas del PR #387 (cerrado: sus cinco números los ocupaban otras entradas y la nota de W-030A13 figuraba `fichado`), `#237` con su diagnóstico disputado, PARTICULAR-659 como `#289`-`#291`, y `[APER-74]` en el runbook (los audios no se transcriben solos). **Las dos remediaciones NO han pasado ronda, declarado** | encargo de Nikolai del 2026-09-25 sobre las aperturas de dos semanas. Decidido por él ese día: `#276` antes que `crm_ficha`; cerrar el #387, fichar PARTICULAR-659 y anotar la nota de W-02UIQU; trabajar en paralelo a Codicert F3 sin tocar lo suyo. **Siguiente, con su precio:** `crm_ficha` (`#283` + `#288`, pieza de dos rondas Astra: decide qué partes quedan vinculadas y da acceso), `#237` (medir `threads().modify` en un hilo real, con permiso), `#190`, el cableado de `#205` y `#284`/`#265`/`#267` como una tanda de skills (re-importación en Cowork) | medio |

| 40 | `crm_ficha`: el YAML se lee entero y se verifica el conjunto **y sus datos** (`MEJORAS #283` + `#288`) | ✅ **CERRADA el 2026-09-26 (PR [#400](https://github.com/TyukhayNi/FeesDefender/pull/400), `ca250a4`)** — [spec rev. 4](docs/superpowers/specs/2026-09-25-crm-ficha-claves-y-conjunto-design.md) · [acta R1](docs/superpowers/specs/2026-09-25-crm-ficha-claves-y-conjunto-r1-adversarial-review.md) · [plan](docs/superpowers/plans/2026-09-25-crm-ficha-claves-y-conjunto.md) · [acta R2](docs/superpowers/plans/2026-09-25-crm-ficha-claves-y-conjunto-r2-adversarial-review.md) · [acta R3](docs/superpowers/plans/2026-09-25-crm-ficha-claves-y-conjunto-r3-adversarial-review.md). **Tres rondas Astra·medium, la tercera autorizada por Nikolai sobre el techo de dos.** R1 (diseño) 4/4 y R2 (plan) 9/9, las dos `REQUIERE-REVISION`; la §9 del plan, aplicada **antes** del código (spec rev. 3 y plan rev. 2; la migración medida era de 23 tests, no de 17). Construida con TDD en nueve tareas: lector sin pérdida compartido con `apply`; claves, tipos, identidad (NIF, email o `id_crm`) y lo que no puede llegar, rechazados al validar y todos juntos; **fase previa de solo lectura** que no escribe nada ante un dato distinto, un `id_crm` inexistente o un conflicto; `id_crm` en los resolutores del core; verificación por **igualdad** de vínculos (`[SOBRA]`) y de datos (`[DATO]`). **R3 sobre el diff: `REQUIERE-REVISION`, 4 hallazgos (1 `alta`), 4 confirmados, y una observación**, remediados en el mismo PR (§10 del plan) **sin ronda que revise el remedio**: el NIF viaja canónico —mi doble del CRM canonizaba los dos lados y el CRM no quita separadores—, el NIF que se vacía se rechaza, el lector lo dice todo, y dos partes de un rol no pueden acabar en una ficha. Arnés sobre copia **34/34** por su aserto; P6 **39/39**; censo de los 14 `_ficha_crm.yaml` reales: 13 pasan, W-030A13 por su `apellido`. Suite **6.695 recogidos** tras fusionar `main` (+212 de esta pieza sobre sus 6.483), dos semillas, cobertura del diff **97 %**. Límites declarados: `#312`, `#313`, `#314` | encargo de Nikolai del 2026-09-26 (aplicar la §9, construir con TDD, R3); decisiones suyas del 2026-09-25: el `_ficha_crm.yaml` es la lista **completa** de partes y `crm_ficha` no desvincula ni pisa; `_COMPLETABLES_*` no se toca. **Pendiente antes de un caso real:** `--dry-run` y un expediente de prueba en el CRM, verificando por lectura (qué expediente lo decide Nikolai) | medio-alto |

| 41 | La verja de `session_close` corre los tests lentos del aportable cuando el diff toca F3 (`MEJORAS #292`) | **pendiente — promovida el 2026-09-25**: su disparador («el próximo cambio en `core/certificado_aportable.py` o en su adaptador de OCR») lo cumplió la remediación de la R3 de F3. Los dos lentos —el OCR real y el escaneo— se corrieron a mano con `--runslow` (2 verdes); ninguna verja los corre si el diff no toca `core/anon/`. | toca una verja de `scripts/session_close`: no queda exenta de ronda | bajo |
| 42 | Un solo lector del token de `gdrive_ev`, con timeout holgado, que distinga «tardó» de «no existe» y lo diga (`MEJORAS #296`) | ✅ **CERRADA el 2026-09-26, junto a la fila #47 (PR [#409](https://github.com/TyukhayNi/FeesDefender/pull/409), `b6249e2`)** — [plan y adjudicación](docs/superpowers/plans/2026-09-26-token-drive-y-carpetas-w-delante-296-301.md) · [acta R1](docs/superpowers/plans/2026-09-26-token-drive-y-carpetas-w-delante-296-301-r1-adversarial-review.md). **R1 de Codex (`gpt-6-sol`·`high`): `NO-SHIP`, 7 hallazgos (1 `alta`), 7 confirmados**, remediados en el mismo PR sin otra ronda: ningún motivo con una causa elegida al pintar (el 403 que no es de permisos, la cuota con su razón, «caducado o a punto», el remote por su línea `type`) y el diagnóstico que imprimía el token, arreglado aquí. Un solo lector con 30 s y un motivo tipado, `leer_carpeta_drive` con el suyo, el alta que lo imprime y el precheck de la skill con un 5 para «tardó». Medido al construir: `rclone config show <remote inexistente>` sale con 0, y el precheck lo tomaba por «client compartido». Promovida el 2026-09-26, por decisión de Nikolai: su disparador («el próximo alta que aborte con ese mensaje») se cumplió el mismo día en que se fichó, en la apertura de W-02Y2J6 —segunda alta abortada con «token/red» teniendo el token vigente; las dos veces la salida fue pasar `--team-id` explícito—. | toca `core/intake_drive.py` y el precheck de la skill `organizar-sala-lectura`: una ronda sobre el diff | bajo |
| 43 | El aviso «PLAN.md ↔ git» de `session_close`: una ruta no es una rama, `claude/` sí, y cada fila de la cola es su propio ítem (`MEJORAS #303`) | ✅ **CERRADA el 2026-09-26 (PR [#404](https://github.com/TyukhayNi/FeesDefender/pull/404), `e25a707`)** — [plan y adjudicación](docs/superpowers/plans/2026-09-26-mejoras-303-aviso-plan-git.md) · [acta R1](docs/superpowers/plans/2026-09-26-mejoras-303-aviso-plan-git-r1-adversarial-review.md). Promovida ese día por decisión de Nikolai, que pidió hacerla en la sesión de `crm_ficha`, en un PR aparte. Medido sobre el `PLAN.md` real: de 49 avisos, los 49 rutas `docs/…`, a uno —una nota que conservaba el nombre de una rama podada— y a cero al retirarlo. **R1 de Codex (`gpt-6-sol`·`high`): `LISTA-CON-CAMBIOS`, 4 hallazgos (1 `media`), 4 confirmados**, remediados en el mismo PR sin otra ronda: una fila es lo que queda tras la sangría y la cita, una fila sin número se identifica por su línea, y la ambigüedad rama/ruta que la sintaxis no cierra, declarada. Segunda fila de la calibración de la #38 | toca `scripts/session_close.py`: una ronda sobre el diff (`gpt-6-sol`·`high`, segunda fila de la calibración de la #38) | bajo |
| 44 | Git que falla en voz alta: ningún guard, verja ni CI lee una respuesta incompleta de git como «no hay nada» | ✅ **CERRADA el 2026-09-26 (PR [#405](https://github.com/TyukhayNi/FeesDefender/pull/405), `cc98244`)** — [plan y adjudicación](docs/superpowers/plans/2026-09-26-git-que-falla-en-voz-alta.md) · [acta R1](docs/superpowers/plans/2026-09-26-git-que-falla-en-voz-alta-r1-adversarial-review.md). Nació de una observación de la R1 de los estados medidos —los guards de gobernanza daban 48 verdes en una copia sin `.git`—; chip cerrado y hecha en la misma sesión por encargo de Nikolai. **R1 de Codex (`gpt-6-astra`·`medium`): `NO-SHIP`, 6 hallazgos (1 `alta`), 6 confirmados**, remediados en el mismo PR sin otra ronda: la remediación no la ha revisado nadie, y Nikolai la mergeó así. Fuera: `MEJORAS #315` | toca guards, la verja y CI: una ronda sobre el diff (`gpt-6-astra`·`medium`, fila «un guard») | medio |
| 45 | Los tres falsos rojos de `verificar_apertura` en las aperturas del 15-25/09: C1 con los nativos de Google, C2 con el relleno de lo que ya acaba en ceros, C3 por identidad (`MEJORAS #306`, `#307`, `#287`) | ✅ **CERRADA el 2026-09-26 (PR [#407](https://github.com/TyukhayNi/FeesDefender/pull/407), `1aa1871`)** — [plan y adjudicación](docs/superpowers/plans/2026-09-26-verificar-apertura-c1-c2-c3.md) · [acta R1](docs/superpowers/plans/2026-09-26-verificar-apertura-c1-c2-c3-r1-adversarial-review.md). Promovida ese día por decisión de Nikolai, que pidió seguir limpiando el backlog de las últimas aperturas y eligió este bloque. C1 cruza cada nativo por el nombre con que rclone lo exporta; C2 prueba cada frontera de la cola de ceros y el sha256 fija cuál es; C3 cruza la cobertura con el catálogo por ruta de origen y sha256, en las dos direcciones. Sobre los 23 expedientes con los dos: 5 en `ok`, 14 que nombran lo no catalogado (`MEJORAS #316`) y 4 con dos catálogos que no cuadran, que ya fallaban. **R1 de Codex (`gpt-6-sol`·`high`): `NO-SHIP`, 6 hallazgos (1 `alta`), 6 confirmados**, remediados en el mismo PR sin otra ronda: la unidad del catálogo es la entrada —un par ruta/sha256— y la que contradice a su fuente es `fallo`; lo que casa por una sola clave se declara; C1 no afirma el contenido de un nativo. Tercera fila de la calibración de la #38 | toca `core/verificar_apertura.py` y su adaptador: una ronda sobre el diff (`gpt-6-sol`·`high`, tercera fila de la calibración de la #38) | bajo |
| 46 | La sala de lectura da cuenta de todo lo de `00_Input`: contrato de población en los dos productores y C3 que honra lo declarado (`MEJORAS #316`) | ✅ **CERRADA el 2026-09-26 (PR [#408](https://github.com/TyukhayNi/FeesDefender/pull/408), `aa46fcc`)** — [plan y adjudicación](docs/superpowers/plans/2026-09-26-sala-lectura-poblacion-316.md) · [acta R1](docs/superpowers/plans/2026-09-26-sala-lectura-poblacion-316-r1-adversarial-review.md). Promovida ese día por decisión de Nikolai, que pidió hacerla en la misma sesión que el #407. Una frontera, dos productores: la skill no tenía verja de población —su verify miraba lo copiado, no lo que había— y el motor CLI decidía la población con una lista blanca de extensiones (`core/inventory.py`), que en W-02Y2J6 dejó fuera siete notas de voz, zips, vCards y un vídeo. Contrato de población y verja en `verificar_sala.py`; el inventario recoge todo lo que no es protocolo; tres ficheros de la migración de 05_CRM, registrados por nombre entero; C3 honra lo declarado `excluido` y lo enseña. **R1 de Codex (`gpt-6-sol`·`high`): `NO-SHIP`, 11 hallazgos (3 `alta`), 11 confirmados** —[acta](docs/superpowers/plans/2026-09-26-sala-lectura-poblacion-316-r1-adversarial-review.md)—, remediados en el mismo PR sin otra ronda: la unidad es el par ruta/sha256 de la fila (la frontera que la C3 cerró horas antes, reescrita en la verja nueva); el duplicado se mide, no se declara; la firma de `email_export` es un adjunto más (el productor la marca, no la descarta); el verify ya no da 0 sin cobertura; y cada regla de productor vale donde el productor escribe. Medir el remedio contra los datos cambió uno: la copia con el relleno de `#225` cuenta por su contenido. Arnés de mutación en el repo, 32 de 32. Cuarta fila de la calibración de la #38 | toca `core/`, `scripts/` y la skill `organizar-sala-lectura`: una ronda sobre el diff (`gpt-6-sol`·`high`, cuarta fila de la calibración de la #38) | bajo |
| 47 | Las carpetas de E&V con el W-code DELANTE derivan su dirección, y la ciudad de SaRS1 se quita si es la del caso (`MEJORAS #301`) | ✅ **CERRADA el 2026-09-26, junto a la fila #42 (PR [#409](https://github.com/TyukhayNi/FeesDefender/pull/409), `b6249e2`)** — [plan y adjudicación](docs/superpowers/plans/2026-09-26-token-drive-y-carpetas-w-delante-296-301.md). La R1 (H-01, `alta`) encontró que el último « - » solo separa el consultor por convención: se deriva solo cuando la dirección lleva número y el consultor no —219 de 229 carpetas medidas—, y lo demás pide el flag. Promovida ese día por su propio disparador —«un censo de nombres de carpeta de E&V que diga qué plazas usan cada orden»—, hecho en solo lectura con la Drive API: 53 unidades, 231 carpetas con el W-code delante en muchas plazas (no solo Santander) y 331 con el W-code en medio sin guion, que siguen pidiendo el flag a propósito. Con el parser final derivan 219 de las 231 | toca `core/intake_drive.py` y `scripts/abrir_caso.py`: una ronda sobre el diff, la misma que la fila #42 (`gpt-6-sol`·`high`, quinta fila de la calibración de la #38) | bajo |
| 48 | ¿Puede Codex repartir una ronda entre subagentes? `AGENTS.md` dice que no puede, y desde el 2026-09-23 los rollouts traen la capacidad (`MEJORAS #280`) | **pendiente de la decisión de Nikolai — promovida el 2026-09-26**: su disparador («que la calibración cierre (fila #38)») se cumplió ese día. Rectificar `AGENTS.md` **autoriza** a repartir una ronda, y eso multiplica el consumo: por eso se separó de la calibración, para no contaminar sus cinco filas | decisión de Nikolai; el cambio que salga toca el contrato de revisión, que no queda exento de ronda | bajo |
| 49 | El acta exige `modelo`, `esfuerzo` y `velocidad`: un guard de **presencia** para las actas nuevas (`MEJORAS #281`) | **pendiente — promovida el 2026-09-26**: su disparador («la segunda acta que llegue sin los tres campos, o el cierre de la calibración de la fila #38 —lo que pase antes—») lo cumplió el cierre de la calibración. El guard probaría que el dato está, nunca que es verdad: el rollout vive fuera del repo. Ya hay una sin ellos: el acta de la fila 1 del ledger (2026-09-25), posterior a la rev. 11, no los lleva en el frontmatter (lo vio la R1 de #411) | toca un guard: no queda exenta de ronda, y por la tabla de modelo va a `gpt-6-astra`·`medium` | bajo |
| 50 | Medir con `gpt-6-sol`·`high` dos o tres revisiones antiguas con defectos ya confirmados —la reutilización que el método de la #38 preveía y no se hizo— | **en curso — encargada por Nikolai el 2026-09-26**, al saber las dos condiciones omitidas de la calibración. Objetos que revisó Astra en su día, en el commit que se revisó, sobre copias y **sin enseñar el remedio**: dan el denominador de defectos conocidos y los **pares Sol/Astra sobre el mismo objeto** que el ledger no tenía. Lanzador con `$OutputEncoding` en UTF-8. **Criterio, fijado por Nikolai antes de correr (2026-09-26):** si Sol no encuentra un defecto `alta` ya confirmado —el mismo defecto, no el mismo fichero, adjudicado contra la fuente—, la categoría de ese objeto sube a Astra en la tabla; si los encuentra todos, la tabla se queda. **Objetos, elegidos antes de correr:** apertura-p2 (`0702f49..b051e96`: 13 confirmados, 7 ALTO) y apertura-p2red (`044798b..0c3f19c`: 14 confirmados, 8 ALTO), los dos de solo lectura —fila de Sol hoy— y revisados en su día por `gpt-6-astra`·`high` (releído del rollout), con su mismo mandato literal; límite: son del mismo módulo. Descartados: la acción 10 (publicar antes de decidir la acerca hoy a Astra·`high`), identidad sin teclado (dos rondas: Astra por construcción), la lectura de relaciones del CRM (la revisó `gpt-5.6-sol`, no Astra) y el generador de viabilidad (ningún ALTO) | es una medición, no una pieza: no lleva ronda propia, y cada corrida se archiva con su mandato | medio |

> **Fila 47 añadida el 2026-09-26, al final y sin reordenar**: es la promoción de `MEJORAS #301`,
> cuyo disparador alternativo —el censo— se hizo al construir la fila #42 en la misma sesión,
> porque las dos tocan `core/intake_drive.py`.

> **Fila 41 añadida el 2026-09-25, al final y sin reordenar**: es la promoción de `MEJORAS #292`,
> cuyo disparador —«el próximo cambio en `core/certificado_aportable.py` o en su adaptador de
> OCR»— lo cumplió la remediación de la R3 de F3 (fila 37). Número 41 porque la 39 y la 40
> entraron el mismo día desde otra rama.
...
> apertura de las dos últimas semanas, y la sesión fue un encargo expreso de Nikolai. **La 40, el
> mismo día**, es la primera de las «fuera, con su precio» de la 39, diseñada a petición suya en
> la misma sesión.
...
> significa nada enseña a ignorar los rojos, y la verja entera depende de que signifiquen algo.

> **Fila 22 añadida el 2026-09-06, al final y sin reordenar, por el mismo criterio que las anteriores.** No es una mejora de producto: es el arnés que verifica todo lo demás, y por eso se hizo con `main` limpia —0 PRs abiertos, 0 ramas vivas— que es el único momento en que cambiar el suelo no se lo cambia a nadie. **La regla que la gobierna, y que vale más que el paralelismo:** todo test que hubiera que serializar se declara con su porqué, y el conteo del `--junit-xml` tiene que cuadrar exactamente antes y después. No hizo falta serializar ninguno.
...
> criterio que todas las anteriores. Son la consecuencia directa de las **tres decisiones** que
> Nikolai resolvió ese día (arriba): no son promociones de backlog por completitud, cada una
> tiene decisión expresa. La #29 es además el caso raro de una fila que nace **con el código ya
> escrito y sin commitear** — y esa anomalía es justo lo que motiva la #31.


> **Las tres decisiones abiertas del handoff del 2026-09-10 §6, RESUELTAS por Nikolai el
> 2026-09-13.** Llevaban abiertas desde entonces y bloqueaban P3, P4 y el proceso:
>
> 1. **`MEJORAS #225` (el relleno con ceros del pull) — se DECLARA la limitación y se rescata
>    la vía (a); el histórico se repone solo por disparador.** No se repone en masa, y la razón
>    es la que el propio backlog ya tenía escrita y no estaba en la lista de vías: **un caso
>    repuesto a mano no admite otro `--fuente drive_ev`** —rclone decide por modtime y el
>    re-pull lo vuelve a rellenar—, así que reponer 242 + 171 ficheros **antes** de cerrar la
>    causa (la vía (b)) es trabajo que se deshace solo. El daño es de acreditación, no de
>    contenido: el OCR sale `ok` y el texto es el del original. **Y el dato que cambió la
>    decisión:** la vía (a) **no estaba sin construir** — 838 líneas sin commitear desde el
>    2026-09-10 en el worktree de otra sesión, con `core/intake_drive_hash.py` y sus tests.
>    Rescatarlas es una fila nueva: **#29**. (El nombre de su rama se retiró el 2026-09-26: ya no
>    existe, y la rama es un hecho de git, no de este fichero —`MEJORAS #303`—.)
> 2. **`[APER-70]` (dos constructores de la sala de lectura) — gobierna la SKILL**, que ya era
>    el estado de facto. Y lo que tiene daño no es la contradicción del runbook §7 sino
>    `streamlit_app.py:1397`: el botón «📚 Sala de lectura» llama al motor **declarado
>    muerto**, así que Paola y Ana pueden disparar el camino deprecado sin saberlo y lo que
>    obtienen no es la sala plana. Fila **#30**.
> 3. **P8 (una sesión, N aperturas) — SÍ a (a), (b) y (d); (c) aparcado.** La sesión de
>    apertura ejecuta el runbook y **no toca el repo**: anota defectos y los ficha al final,
>    leyendo `origin/main`. Como mucho una sesión de reparación en paralelo, dueña del código.
>    En la bitácora, un bloque «aperturas del día» en vez de un cierre numerado por caso. El
>    lote en serie (c) se aparca: es lo único que cuesta construir. **Evidencia de que hacía
>    falta, encontrada el mismo día:** las 838 líneas huérfanas del punto 1 son exactamente
>    una sesión de apertura que se puso a reparar. Fila **#31**.
...
> **✅ CERRADA el 2026-09-13 (PR #356 la pieza B, PR #358 la pieza A).** El detalle vive en
> la fila #27 de la tabla y en
> `docs/superpowers/plans/2026-09-13-fila27-pieza-a-custodia-tolerante.md`. Lo que sigue es
> el diagnóstico tal como se escribió al promover, **con una corrección**: el punto 2 de la
> pieza A no se construyó dentro del pull porque ya estaba hecho fuera (`verificar_apertura`
> C1/C2, fila #28 P2), y el punto 3 quedó diferido con su premisa corregida.

**Estado al promover: se sabía hacer a mano, no estaba construido.** El recorrido manual completo está en el
...
| 6 | Incorporación de correo y adjuntos (checklist por fuente, `--extraer-adjuntos` por defecto, filtro de ruido) | ✅ **(a) y (b) CERRADAS — PR #299 (`9a8af1e`)**; sub-pieza (a) el 2026-09-06 — el filtro de ruido; cierra además la casilla 1 de la fila #8, que era la misma pieza vista desde otro sitio. **R1 de Codex `NO-SHIP`, 6 hallazgos, 6 confirmados, 0 refutados, remediados.** **(b) `--extraer-adjuntos` a default `True` + `es_firma_incrustada`, con su propia R1 (`NO-SHIP`, 4/4)**. Actas: `2026-09-06-accion-6{a,b}-…-r1-adversarial-review.md`; adjudicaciones en los §6 y §7 del plan | **Queda solo el «checklist por fuente»**, sin diseño y solapado con la acción 12. La razón que bloqueaba el default —«mueve la superficie de dedup»— **caducó el 2026-09-06** con `MEJORAS #147` vía A (PR #296). Gate 2 re-medido sobre 444 adjuntos reales: 0 correos — pero **del corpus, NO por construcción** (R1/H-02 lo refutó; ver `MEJORAS §55.1`) |
...
| 12 | Ficha operativa de cierre de apertura + medir el recorrido en tres aperturas | **apertura 2 de 3 MEDIDA** (W-030TZY y W-02NHNC, las dos el 2026-09-10); el INSTRUMENTO sigue sin construir | Reparto real en `RUNBOOK [APER-67]`: **46,5 min de máquina, el 82 % en OCR**. Y el cuello que no es de máquina: la sala de lectura se detiene pidiendo clasificar **80 documentos a mano**. Tres hallazgos con número: `MEJORAS #219` (el `_tiempos.jsonl` no guarda páginas, que es lo que decide el paralelismo que su propio docstring dice querer decidir), `#221` (`organizar` dice «organizada» con 2 de 4 artefactos) y `#222` (paralelismo por documento **refutado**, 0,91; `rotate_pages` cuesta el 28 % y es un seguro, con control positivo sobre página rasterizada y girada). El recorte del instrumento —`ronda_id`/`iniciada`/`terminada`/`duracion_s` al evento de `registrar_cierre_v1`— **sigue sin hacerse**. **La 2ª apertura medida (W-02NHNC, 103º cierre): 1:37:22 de reloj, 14:18 de máquina** (OCR 13:36 sobre 24 documentos), repartidos en mecánica 38:41 · sala de lectura 12:16 · ficha CRM 6:48 · viabilidad 39:36. **El cuello fue distinto**: aquí la sala de lectura solo pidió clasificar 21 documentos, y lo que dominó fue el OCR más un reproceso propio de ~15 min por `MEJORAS #225` |
...
> **✅ Cerrado el 2026-09-05 (PR #289) con (1)+(2), decidido por Nikolai el mismo día.**
> La lista se resuelve como **unión** del árbol commiteado y del checkout principal del mismo
> repo — el primero de `git worktree list`, aceptado solo verificado por resultado
> (`resolver_blocklist`); si aun así sale vacía, `main` imprime en STDERR cada ruta buscada con
> su estado observado y que la comprobación **NO se ha ejecutado**, y el hook es `verbose` para
> que pre-commit lo muestre. El mutante de arriba tiene test con un repo git real y su worktree,
> y **ahora bloquea**; la fila «se salta» de la tabla también cae, porque
> `test_no_pii_en_tests.py` encuentra la lista desde el worktree. **R1 de Codex: NO-SHIP, 8/8
> confirmados, remediados** (`docs/superpowers/plans/2026-09-05-mejoras-161-blocklist-desde-la-raiz-comun.md` §4).
> **(3) fallar cerrado queda diferida** con su disparador escrito en `MEJORAS #161`.
> El residuo de `refs/pull/279/head` sigue como estaba: no depende de este repo.
...
(7 xfailed, 0 xpassed): los siete defectos seguían vivos **entonces**. Desde el
2026-08-25 son **SEIS**: `MEJORAS #93-B` cerró A-2c (remedido el 2026-09-03: 6 `xfailed`,
0 `xpassed`).
...
> **✅ CERRADO el 2026-09-05 — PR #294 (`63c8aa2`).** Implementado como acción 10 de la fila 21; el plan y la adjudicación en `docs/superpowers/plans/2026-09-05-accion-10-ofimatica-en-la-sala-de-maquina.md`. El texto que sigue es el diagnóstico original.
...
- [x] **Fase 0 — banco de pruebas del frontal.** ✅ **PR #170** (`4dba135`, PR-A: barrera + `Entorno`
      ...
      **Los defectos siguen vivos**: están reproducidos, no arreglados, y su arreglo no es de
      esta fase. Eran **siete** al cerrar la Fase 0 y **son SEIS desde el 2026-08-25**, cuando
      `MEJORAS #93-B` cerró A-2c (remedido el 2026-09-03: 6 `xfailed`, 0 `xpassed`).
...
- [x] **Fase 1 — núcleo de workspace.** ✅ **PRs #230** (`291436d`, R7 + modelo + registro),
      ...
      **Lo que NO cierra, dicho para que nadie lo dé por hecho:** los **6 defectos del frontal**
      siguen vivos en `xfail(strict=True)` —**remedido el 2026-09-03: 6 `xfailed`, 0 `xpassed`**;
      eran 7 hasta que `MEJORAS #93-B` cerró A-2c el 2026-08-25— y son la lista
...
- ✅ **[APERTURA-MENOS-DECISIONES]** Tres piezas para que la apertura pregunte menos
  (`MEJORAS #224`, `#227`, `#228`), **6 de 6**, más las tres entradas que su última ronda destapó
  sobre la plantilla de viabilidad. PRs [#336](https://github.com/TyukhayNi/FeesDefender/pull/336) (`eb8ac09`), [#339](https://github.com/TyukhayNi/FeesDefender/pull/339) (`7fa38aa`),
  [#342](https://github.com/TyukhayNi/FeesDefender/pull/342) (`0702f49`), [#343](https://github.com/TyukhayNi/FeesDefender/pull/343) (`044798b`), [#344](https://github.com/TyukhayNi/FeesDefender/pull/344) (`2cc9a24`),
  [#348](https://github.com/TyukhayNi/FeesDefender/pull/348) (`c097ce4`), [#353](https://github.com/TyukhayNi/FeesDefender/pull/353) (`45950c6`), [#350](https://github.com/TyukhayNi/FeesDefender/pull/350) (`47d157f`),
  [#351](https://github.com/TyukhayNi/FeesDefender/pull/351) (`f147b94`) y [#354](https://github.com/TyukhayNi/FeesDefender/pull/354) (`d2236c4`).

  **`MEJORAS #227` cerró al SEGUNDO intento y por otro diseño**: el primero (PR #338, cerrado)
  **localizaba** la línea dentro del cuerpo Markdown de `_caso.md` y murió con seis formas de romper
  la misma propiedad; el que entró **compara** —si el cuerpo es exactamente el que genera la
  plantilla, se reescribe; si difiere en cualquier cosa, no se toca ni un byte—. La vía muerta está
  en `docs/DEAD_ENDS.md` para que nadie la reintente.

  **Cuatro rondas adversariales, 25 hallazgos, 25 confirmados, 0 refutados.** Dos cosas que entran
  declaradas: **el techo del censo de escrituras sube de 91 a 92** (decisión expresa de Nikolai,
  porque su trinquete dice que solo baja), y **la remediación de la R2 de `#227` no pasó por ninguna
  ronda** —presupuesto agotado, tercera no autorizada—, con 18 mutantes dirigidos como única red.

  Diseño y adjudicaciones:
  `docs/superpowers/specs/2026-09-11-cuantia-en-caso-md-comparar-no-localizar-design.md` y
  `docs/superpowers/plans/2026-09-11-plantilla-viabilidad-filtro-y-rojo.md`. Abre `MEJORAS #246`,
  `#247` y `#248`. **Pendiente y humano:** abrir la plantilla en Excel (desplegable + alerta) y
  reimportar la skill `viabilidad-prerelleno` en Cowork.

- ✅ **[SALA-LECTURA-PLANA]** La sala de lectura queda **plana** y una colisión de nombre deja de pisar un documento (`MEJORAS #67.b`, `#67.c`, `#36`). Destino plano con la subcarpeta del documento compuesto como única excepción, `__<sha8>` para **todos** los miembros de un grupo colisionado (nadie conserva el nombre pelado), reserva **global** de rutas, copiar-antes-de-borrar, poda del cascarón por fuente confinada y sin seguir enlaces, y `n_en_sala` en el resumen — el `COPY` contaba copias intentadas, no ficheros escritos. **Mergeado: PR [#328](https://github.com/TyukhayNi/FeesDefender/pull/328)** (`a2e6676`). Skill `organizar-sala-lectura` a **v1.17** con el literal de desambiguación corregido; **pendiente re-importar el `.skill` en Cowork**. **Dos rondas adversariales, 17 hallazgos, 17 confirmados**, y la segunda sobre la implementación hermana del mismo mecanismo encontró **3 defectos vivos en el código ya remediado por la primera**. Plan y adjudicación: `docs/superpowers/plans/2026-09-10-sala-lectura-plana.md` (§5 y §6); actas literales con su digest en `…-r1-adversarial-review.md` y `…-r2-hermana-adversarial-review.md`. **Cobertura declarada:** ninguna de las dos remediaciones pasó revisión — el techo de dos rondas estaba consumido y Nikolai declinó la tercera por radio de daño; lo acreditan 28 tests con sus controles positivos, un mutante muerto y **dos expedientes reales migrados y verificados por contenido** (W-02YZO4 26/26; W-048UOL 48 ficheros, 51/51 byte a byte, 0 huérfanos). Preexistentes NO arreglados, con su disparador: `MEJORAS #229`, `#230`, `#231`, `#232`, `#233`.

- ✅ **[PODERES-CRM-SANEADO]** El fichero `poderes` del CRM, de 85 registros sucios a un contrato escrito entero. **Saneado por API con verificación por lectura en cada bloque**: 431 campos contra la hoja de cambios (0 discrepancias), 86 documentos renombrados conservando `nombreoriginal`, **0 alteraciones no pedidas** contra un snapshot previo de los 85, 22 vínculos añadidos y 0 perdidos. El hallazgo que lo justificó: **la vigencia no vive en ningún campo del CRM, la lleva el certificado de apoderamiento**, y al leerlos salieron **16 de 85 caducados** que el CRM presentaba igual que los vivos (diez apud-acta de la cascada de 2021, seis notariales rusos con plazo expreso, más uno que vencía a cinco días). El cruce contra el registro judicial de la sede encontró **cinco apoderamientos que faltaban**, cuatro vivos de E&V: bajados sus certificados, dados de alta con el PDF en el gestor (`#72` completado, `#87`-`#90` nuevos; el fichero pasa a **89**). **Contrato documentado de punta a punta** en `docs/INTEGRACION_SUDESPACHO.md` **§16** (el elemento, ocho apartados) y **§17** (la subida al gestor en tres pasos, descubierta **leyendo los bundles JS del front** en vez de capturando un HAR — más barato, no exige ejecutar la operación en la UI, y da los nombres de clave literales). Dos endpoints que devuelven **201 sin hacer nada** quedan en `DEAD_ENDS.md`. **Dos rondas adversariales de Codex, las dos `NO-SHIP`: 13 hallazgos, 13 confirmados, 0 refutados**, y **nueve eran la misma clase** —el sitio viejo que quedaba mintiendo—, incluida una en un plan ajeno con `estado: vigente`. Actas: [R1](docs/superpowers/specs/2026-09-08-poderes-crm-contrato-r1-adversarial-review.md) y [R2](docs/superpowers/specs/2026-09-09-poderes-crm-subida-r2-adversarial-review.md). Backlog: `MEJORAS #185` (el atlas oculta ≥28 elementos), `#186` y `#187`. **PR [#307](https://github.com/TyukhayNi/FeesDefender/pull/307)** (2026-09-09). **Deuda viva, y es de Nikolai: está en el bloque `[SIGUIENTE-ALTA-PODER]`** — `#79` y `#86` sin apoderamiento detrás en el registro, 7 poderes cuyo compareciente no es Nikolai y quedan **sin verificar**, el duplicado `#54`/`#56`, el par `#17`/`#18` con el mismo PDF, `#1` y `#13` con fechas a 794 y 218 días del documento, y `#35`, que no es un poder y para el que el enum `Formato` no tiene valor

- ✅ **[SIGUIENTE-DOC-LIBREOFFICE]** Los `.doc` dejan de ser ilegibles en la sala de máquina: ruta `ofimatica` (`.doc`/`.dot`/`.odt`/`.ott`/`.ppt`/`.pps`/`.pptx`/`.odp`) → LibreOffice headless con perfil efímero, verificado por resultado → PDF buscable en `01_OCR/` → camino PDF; sin conversor, `sin_soporte` con la causa en la nota y aviso en `plan`/`apply`. Era la fila 10 de la cola y la acción 10 de la fila 21 (el mismo ítem, dos filas): cerrado como **PR #294** (`63c8aa2`), R1 de Codex 5/5 confirmados y adjudicados en `docs/superpowers/plans/2026-09-05-accion-10-ofimatica-en-la-sala-de-maquina.md` §4. `MEJORAS #61` punto `.doc` `[RESUELTO]` (2026-09-05)

- ✅ **[GITIGNORE-REGLAS-INERTES]** `.claude/settings.local.json` estaba trackeado pese a figurar en el `.gitignore` — y `.gitignore` **no aplica a lo que ya está en el índice**, así que esa línea era una **regla inerte**: escrita, visible y sin morder. Síntoma: no se podía crear una rama desde la UI («Haz commit o stash de los cambios»), porque Claude Code le añade permisos al conceder autorizaciones. Sale del índice (queda en disco), plantilla `.example`, y un **guard que cierra la clase**: ningún fichero trackeado puede estar ignorado por un `.gitignore` versionado. **Correr el guard fue la mitad del trabajo**: dio cinco positivos, uno **falso del propio guard** (decidía desde `check-ignore -v`, que reporta también las negaciones, así que `.env.example` salía acusado por su propio rescate) y **cuatro verdaderos** — los `.claude/skills/**/logs/README.md`, ignorados de verdad por `logs/` y sin ninguna negación que los rescatase, contra lo que el encargo afirmaba. Tres afirmaciones del encargo se cayeron al medirlas, entre ellas que un `!ruta` a secas sirva de remedio: **no rescata un fichero cuyo padre está excluido por un patrón de directorio**, hacen falta tres líneas. **R1 de Codex (`LISTA-CON-CAMBIOS`, 4 hallazgos, 4 confirmados)**; el caro solo se ve ejecutando: con el `git init` del laboratorio roto **los cinco tests quedaban verdes** mientras las sondas se añadían con `-f` al índice del repositorio **padre** — el guard escrito para detectar contaminación del índice contaminaba uno de verdad, callado. **10 mutantes, 9 matados y uno superviviente declarado**: la anulación de `core.excludesFile` no sostiene la corrección, y dejé de atribuirle la garantía en vez de enviar una línea inerte dentro del propio guard. Acta: [`2026-09-04-gitignore-reglas-inertes-r1-adversarial-review.md`](docs/superpowers/specs/2026-09-04-gitignore-reglas-inertes-r1-adversarial-review.md). **PR [#273](https://github.com/TyukhayNi/FeesDefender/pull/273)** (`501db93`) (2026-09-04)

- ✅ **[CRM-LECTURA-RELACIONES]** La API **sí** sabe leer las relaciones de un expediente — `INTEGRACION_SUDESPACHO.md` afirmaba que «no hay ruta REST de LECTURA de relaciones» y que el vínculo «solo se verifica por UI»: era una **generalización de una ruta a toda la API**. El 405 es de `relation_element`; la lectura vive en **`related_register`**, declarada en el `/api/docs.json` público. Cableada en `core.sudespacho_relations.get_relaciones`, con la trampa del formato resuelta —la respuesta viene **acumulada**, y aplanarla cuenta 6 vínculos donde hay 3—. `scripts/crm_ficha.py` deja de rematar con «verifícalo en la UI» y contrasta **todo lo que afirma haber escrito**, Notas incluidas: si la lectura desmiente a los `201`, sale con código 1. **R1 de Codex: `NO-SHIP`, 8 hallazgos, 8 confirmados**, y **cuatro eran el mismo defecto que el cambio venía a arreglar, un nivel más abajo** —entre ellos una guarda de red cuyo grito se tragaba el `except Exception` del código que vigilaba—. Contrato en **§15.5**; acta [`2026-09-04-crm-lectura-relaciones-r1-adversarial-review.md`](docs/superpowers/specs/2026-09-04-crm-lectura-relaciones-r1-adversarial-review.md). **PR #271** (2026-09-04)

- ✅ **[CRM-DEDUP]** Deduplicación de partes y de expediente, que **falla cerrado** — encargo de Nikolai tras medir que la dedup del CRM era desigual: el contrario solo miraba el NIF, el colaborador solo el email, en judicial **no había ninguna**, y el alta de expediente solo consultaba el `_caso.md` **local**. Ahora las partes se resuelven por **NIF o email** en las dos jurisdicciones (son el mismo elemento), y **si los dos criterios apuntan a fichas distintas la corrida para**: no fusiona dos personas ni parte una en dos. El alta pregunta al CRM con tres criterios que **no valen lo mismo** —el **W-code bloquea**, dirección y contrario **avisan**—, y **aborta si algo no se pudo comprobar** (`--force` es la salida explícita), decisión de Nikolai. **R1 de Codex: `NO-SHIP`, 10 hallazgos, 10 confirmados, 5 ALTO**; la frontera que explica cinco: el código **colapsaba «no lo sé» en «no hay» y desde ahí escribía**. Acta [`2026-09-04-crm-dedup-r1-adversarial-review.md`](docs/superpowers/specs/2026-09-04-crm-dedup-r1-adversarial-review.md). Vivo y declarado: el criterio por contrario no está cableado en el alta —allí aún no hay contrario resuelto—, y el escape de `%`/`_` en `like` sigue sin medirse. **PR #272** (2026-09-04)

- ✅ **[CRM-FICHA-VALIDAR]** El `_ficha_crm.yaml` se valida contra la documental, y tres campos dejan de perderse — nada genera ese fichero: lo teclea a mano quien prepara el caso, y un NIF mal copiado hace fallar la dedup del #272 por la razón más tonta. `scripts/crm_ficha_validar` dice, dato por dato, **en qué documento aparece**, con **tres veredictos y no dos**: de los 58 documentos del caso real, los cinco ilegibles son **los DNI**, o sea justo donde vive el NIF, así que «no encontrado» ahí sería mentir. Y `cp`, `provincia` y `telefono` del contrario estaban escritos en el YAML **desde el 15/08** sin llegar nunca al CRM; `provincia` además es un **Select** cuyos 52 valores llevan tildes y caja propia, así que la convención del despacho —MAYÚSCULAS— **perdía el dato sin avisar**. **R1 de Codex: `NO-SHIP`, 12 hallazgos, 12 confirmados, 6 ALTO**, y el peor invalidaba el resultado: los espejos empiezan por un frontmatter con `source_path`, así que **el nombre del fichero contaba como prueba**. **Dos de los doce eran afirmaciones mías en voz alta, nacidas entre dos commits del mismo día.** Acta [`2026-09-04-crm-ficha-validar-r1-adversarial-review.md`](docs/superpowers/specs/2026-09-04-crm-ficha-validar-r1-adversarial-review.md). **PR #275** (2026-09-04)

- ✅ **[GITIGNORE-INERTE]** Una regla del `.gitignore` escrita sin morder — `.claude/settings.local.json` estaba **trackeado pese a figurar en el `.gitignore`**, y `.gitignore` no aplica a lo que ya está en el índice: la línea era una **regla inerte**, y el fichero bloqueaba la creación de ramas desde la UI cada vez que Claude Code le escribía un permiso. Sale del índice (queda en disco) con plantilla `.example`, y un **guard cierra la clase**: ningún fichero trackeado puede estar ignorado por un `.gitignore` **que el repositorio reparta**. **R1 de Codex: `LISTA-CON-CAMBIOS`, 4 hallazgos, 4 confirmados**; el caro solo se ve ejecutando —el fixture del laboratorio se comía el `returncode` de `git init`, así que el guard escrito para detectar contaminación del índice **contaminaba uno de verdad, y callado**—. Acta [`2026-09-04-gitignore-reglas-inertes-r1-adversarial-review.md`](docs/superpowers/specs/2026-09-04-gitignore-reglas-inertes-r1-adversarial-review.md). **PR #273** (2026-09-04)

- ✅ **[SIGUIENTE-APERTURA-INTEGRAL]** Apertura integral V1 cableada y corriendo sobre un caso real — `--modo v1` dejó de ser una puerta que validaba sin encadenar: ahora corre identidad → esqueleto → Drive E&V → CRM → atomización → sala de máquina bajo **un único mutex**, termina en uno de los tres estados del §13 y deja constancia en el log forense y en un `_apertura_v1.json` por ronda. **El mutex de #247, que llevaba desde agosto con cuatro rondas y sin proteger nada, empezó a proteger algo.** **Cuatro rondas adversariales, ~97 hallazgos**, todas adjudicadas contra la fuente: R-A sobre el plan (`NO-EJECUTABLE`, 12), **R-B con revisor sustituto y su independencia declarada MÁS DÉBIL** —Codex agotó cupo a mitad de ronda— (`NO-SHIP`, 73, 10 críticos), R-C con Codex sobre el diff remediado (`NO-SHIP`, 7) y R-D sobre `#142` (`NO-SHIP`, 5). **El hallazgo que más enseña es un defecto que introduje AL remediar:** saqué la publicación del registro durable fuera del bloque de mutex —escribiendo así **sin exclusión ninguna**— cuatro líneas debajo de un comentario mío que decía «escribir sin mutex es la violación que el mutex existe para impedir»; la frontera resultó ser `revalidar → publicar → liberar → salir`, indivisible. **Task 11 ejecutada sobre W-02Q38C**, que trajo **2 documentos** que con el código anterior no habrían llegado nunca (el `.pulled` como falso punto fijo) y evitó una apertura abortada por un expediente CRM con el gestor vacío. **3.901 tests, 0 fallos; 35/35 mutantes por su frontera.** Deuda declarada y viva: catorce bloques en los §6/§7 del plan, más `MEJORAS #145` (carrera de producción en la primitiva de exclusión), `HC-05` y `HC-07`; **3B y 3C aplazados** por decisión de Nikolai. Plan: [`2026-09-03-apertura-v1-plan5-cableado.md`](docs/superpowers/plans/2026-09-03-apertura-v1-plan5-cableado.md) + tres actas `…-r{A,B,C}-adversarial-review.md`. **PRs #263, #264 (`MEJORAS #142`), #265 (Task 11 + `#144`), #266** (2026-09-03)

- ✅ **[MEJORAS-124-PARTICION]** `MEJORAS #124` recortada y partida en dos — la costura recibe el `CaseWorkspace` que el llamador ya resolvió, y después se separan **ubicación** e **identidad**, que era la alternativa anotada y no tomada. **Cuatro rondas sobre la pieza, ninguna limpia**: R21 y R24 tumbaron los **dos diseños** coincidiendo en que el problema era el **alcance** y no el detalle —de ahí el recorte, decidido por Nikolai—, y R25/R26 sobre el diff midieron que mi vía nueva **aceptaba una identidad que la histórica rechazaba** y que al remediarlo cerré **el ejemplo y no la frontera**. R27 sobre la partición cobró un crítico más. Actas: [r24](docs/superpowers/specs/2026-09-02-mejoras-124-r24-adversarial-review.md), [r25](docs/superpowers/specs/2026-09-02-mejoras-124-r25-adversarial-review.md), [r26](docs/superpowers/specs/2026-09-02-mejoras-124-r26-adversarial-review.md), [r27](docs/superpowers/specs/2026-09-02-mejoras-124-r27-adversarial-review.md); plan [`2026-09-02-mejoras-124-copia-de-trabajo.md`](docs/superpowers/plans/2026-09-02-mejoras-124-copia-de-trabajo.md). Condición de cierre ejecutable: `python -m tests._mutantes_particion_124`. **PRs #261, #262** (2026-09-02)
...
**Elección confirmada por Nikolai el 2026-09-26**: respondió «ok» a la recomendación de confirmar
`gpt-6-sol`·`high` como habitual y dejar las filas de Astra como estaban. **Con dos salvedades que
el resumen no le dio**: `CLAUDE.md` preveía que la calibración reutilizara objetos con defectos ya
confirmados, y las cinco filas fueron encargos vivos; y las rondas de las filas 2 a 5 recibieron el
mandato con todo carácter no ASCII cambiado por «?» (nota bajo el ledger). Se le dijeron después de
su «ok» y, **sabiéndolas, la mantuvo** y encargó la medición que el método preveía (fila #50), sin
que eso ratifique el método incumplido. Lo que sigue es el registro de cómo se llegó. Cerrar la calibración cumplió además el
disparador de `MEJORAS #280` y `#281`, promovidas a las filas #48 y #49.

**Qué quedaba por hacer, y se hizo:** correr los **cinco primeros encargos ordinarios** con
`gpt-6-sol`·`high`·velocidad estándar y anotarlos abajo; con los cinco, **Nikolai decidía** con los
datos delante: confirmar, ajustar o volver a Astra. **Hasta entonces la política se dijo
provisional** — el remedio a un sesgo ya medido: en el 55º cierre redefiní mi propia regla de
...
**Ledger de calibración — 5 de 5: completo; Nikolai confirmó la elección el 2026-09-26, con las
salvedades de arriba.** Una fila por encargo **ordinario**, que es la fila de **una**
...
| 2 | 2026-09-26 | diff `ca250a4..8cebec4`, `MEJORAS #303` (el aviso «PLAN.md ↔ git» de `session_close`; 5 ficheros, +222 / −8, de ellos +117 / −4 de código y tests) — [acta](docs/superpowers/plans/2026-09-26-mejoras-303-aviso-plan-git-r1-adversarial-review.md) | `gpt-6-sol` · `high` **releídos** del rollout · `default` **afirmada** desde el lanzador | 83.683 tokens (entrada nueva + salida, la medida de la fila 1; 1.171.939 con la caché) · Δcupo semanal **75 % → 76 %** (12:37 → 12:45 UTC) | en paralelo, la R1 de otra sesión (`git-voz-alta`, `gpt-6-astra`·`medium`) y su sonda: el Δ es de la cuenta, no de esta ronda | **4** (1 `media`, 3 `baja`), 0 refutados; dos eran la misma frontera —qué es una fila— | **una, de alcance y no de lectura**: una cita rota en el propio plan (una ruta de ejemplo bajo `docs/superpowers/plans/`), que el guard G2 puso en rojo en la suite completa; el revisor había declarado la suite SIN VERIFICAR y el mandato no se la pedía |
| 3 | 2026-09-26 | diff `cc98244..778920b`, fila #45 (`MEJORAS #306`, `#307`, `#287`: C1, C2 y C3 de `verificar_apertura`; 7 ficheros, +988 / −88, de ellos +767 / −84 de código y tests) — [acta](docs/superpowers/plans/2026-09-26-verificar-apertura-c1-c2-c3-r1-adversarial-review.md) | `gpt-6-sol` · `high` **releídos** del rollout · `default` **afirmada** desde el lanzador | 160.359 tokens (entrada nueva + salida, la medida de las filas 1 y 2; 6.213.607 con la caché) · Δcupo semanal **78 % → 78 %** (14:33:53 → 14:49:45 UTC) | los primeros minutos, la R2 de otra sesión (`facturables-r2`, `gpt-6-sol`·`medium`, 14:32 → 14:37 UTC): el Δ es de la cuenta, no de esta ronda | **6** (1 `alta`, 3 `media`, 2 `baja`), 0 refutados; tres eran la misma frontera —la unidad del catálogo es la entrada, un par ruta/sha256—, y la `alta` era un falso verde sobre lo que C3 existe para vigilar | **una, menor y de mensaje**: el `ok` de C3 contaba como catalogadas las fuentes excluidas; la vi yo esperando la ronda, y el revisor no la señaló |
| 4 | 2026-09-26 | diff `1aa1871..196b0aa`, fila #46 (`MEJORAS #316`: la población de la sala de lectura en la skill y en el motor CLI, y la C3 que honra lo declarado; 20 ficheros, +1.052 / −74, de ellos +859 / −62 de código y tests) — [acta](docs/superpowers/plans/2026-09-26-sala-lectura-poblacion-316-r1-adversarial-review.md) | `gpt-6-sol` · `high` **releídos** del rollout · `default` **afirmada** desde el lanzador | 199.623 tokens (entrada nueva + salida, la medida de las filas anteriores; 6.358.983 con la caché) · Δcupo semanal **78 % → 79 %** (16:08:55 → 16:21:15 UTC) | **ninguna**: ningún otro rollout de Codex en la ventana (la sonda acabó antes; el siguiente, de otro proyecto, empezó a las 16:33 UTC). Aun así el contador es de la cuenta | **11** (3 `alta`, 7 `media`, 1 `baja`), 0 refutados, y dos sub-puntos de H-05 adjudicados en contra; H-03 era la frontera que la C3 había cerrado en la fila 3, reescrita en la verja nueva | **dos, del remedio y no del diff revisado**, vistas al medirlo contra los casos reales: las copias con el relleno de `#225` que la línea `duplicado` tapaba, y once «duplicados» de W-0462E1 que nada confirma |
| 5 | 2026-09-26 | diff `aa46fcc..e55468c`, filas #42 y #47 (`MEJORAS #296` y `#301`: el token y la carpeta de Drive con su motivo, el precheck de la skill y las carpetas con el W-code delante; 14 ficheros, +960 / −151, de ellos +826 / −145 de código y tests) — [acta](docs/superpowers/plans/2026-09-26-token-drive-y-carpetas-w-delante-296-301-r1-adversarial-review.md) | `gpt-6-sol` · `high` **releídos** del rollout · `default` **afirmada** desde el lanzador | 143.674 tokens (entrada nueva + salida, la medida de las filas anteriores; 3.950.778 con la caché) · Δcupo semanal **4 % → 5 %** (18:33:21 → 18:45:58 UTC; la ventana se había reiniciado desde la fila 4) | **ninguna**: ningún otro rollout de Codex en la ventana (la ronda de otro proyecto acabó a las 18:25:48 UTC; la sonda, a las 18:32:11). Aun así el contador es de la cuenta | **7** (1 `alta`, 3 `media`, 3 `baja`), 0 refutados; el `alta` era una derivación que adivinaba —la dirección de una carpeta hacia el `case_id`—, y tres eran la frontera de la propia pieza (una causa elegida al pintar) | ninguna conocida a esta fecha |

**Nota añadida el 2026-09-26, al adjudicar la R1 de la confirmación (#411):** las rondas de las
filas 2 a 5 recibieron el mandato con **todo carácter no ASCII cambiado por «?»** —tildes, eñes,
rayas, comillas—: así lo guarda el mensaje de usuario de sus rollouts. El lanzador pasaba
`MANDATO.md` por una tubería de Windows PowerShell 5.1, cuyo `$OutputEncoding` es ASCII. La fila 1
llegó intacta, y los ficheros del objeto los leían todas del disco, sin tocar. No invalida los
hallazgos confirmados; sí es una condición de la medición que las filas no decían. El remedio del
lanzador y su sonda, en `MEJORAS #279`.
