...
| 28 | Sala de lectura plana + guarda de colisión de nombre (`MEJORAS #67.b`, `#67.c`, `#36`) | ✅ **implementada el 2026-09-10** — destino plano y `__<sha8>` para **todos** los miembros de un grupo colisionado (nadie conserva el nombre pelado), reserva **global** de rutas, poda del cascarón por fuente confinada y sin seguir enlaces, y `n_en_sala` en el resumen (el `COPY` contaba copias intentadas). Un caso poblado con el layout viejo se aplana solo. **R1 de Codex: NO-SHIP, 12 hallazgos, 12 confirmados** — 8 remediados, 4 preexistentes abiertos como `MEJORAS #229`, `#230`, `#232` y `#233`; adjudicación en el §5 del plan | Plan `docs/superpowers/plans/2026-09-10-sala-lectura-plana.md`; acta `…-r1-adversarial-review.md`. Disparador: dos casos reales el mismo día (W-02YZO4 y W-048UOL). **Skill `organizar-sala-lectura` a v1.17 (literal `_2`/`_3` → `__<sha8>`): pendiente re-importar en Cowork** |
