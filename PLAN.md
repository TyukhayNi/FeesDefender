...
- [x] Guard §6 **cableado** en `core/intake_manual.py` (`save_file` → origen `manual`, `save_file_crm_branch` → `crm_manual`): si el caso está prestado/conflicto, el fichero va a `_pendiente_checkin/<origen>/...` con evento; si disponible, escritura normal (retrocompatible). +3 tests.
