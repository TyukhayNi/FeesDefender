...
| 45 | Los tres falsos rojos de `verificar_apertura` en las aperturas del 15-25/09: C1 con los nativos de Google, C2 con el relleno de lo que ya acaba en ceros, C3 por identidad (`MEJORAS #306`, `#307`, `#287`) | ✅ **CERRADA el 2026-09-26 (PR [#407](https://github.com/TyukhayNi/FeesDefender/pull/407))** — [plan](docs/superpowers/plans/2026-09-26-verificar-apertura-c1-c2-c3.md). Promovida ese día por decisión de Nikolai, que pidió seguir limpiando el backlog de las últimas aperturas y eligió este bloque. C1 cruza cada nativo por el nombre con que rclone lo exporta; C2 prueba cada frontera de la cola de ceros y el sha256 fija cuál es; C3 cruza la cobertura con el catálogo por ruta de origen y sha256, en las dos direcciones. Sobre los 23 expedientes con los dos: 5 en `ok`, 14 que nombran lo no catalogado (`MEJORAS #316`) y 4 con dos catálogos que no cuadran, que ya fallaban. R1 de Codex pendiente | toca `core/verificar_apertura.py` y su adaptador: una ronda sobre el diff (`gpt-6-sol`·`high`, tercera fila de la calibración de la #38) | bajo |
...
**Ledger de calibración — 2 de 5.** Una fila por encargo **ordinario**, que es la fila de **una**
