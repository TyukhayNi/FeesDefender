...
| 30 | El botón de Streamlit deja de llamar al constructor deprecado de la sala de lectura (`[APER-70]`) | sin construir | decisión de Nikolai del 2026-09-13: **gobierna la skill** `organizar-sala-lectura`, que ya era el estado de facto. Lo que tiene daño no es la contradicción del runbook §7 sino `streamlit_app.py:1397`: el botón «📚 Sala de lectura» llama a `core/sala_lectura.py`, **declarado `[DEPRECADO 2026-06-18]` en su propia primera línea**, así que Paola y Ana —que no tocan código— pueden disparar el camino muerto sin saberlo, y lo que obtienen no es la sala plana. Desbloquea además **P4** | medio |
