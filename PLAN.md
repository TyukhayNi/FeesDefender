...
| 43 | El aviso «PLAN.md ↔ git» de `session_close`: una ruta no es una rama, `claude/` sí, y cada fila de la cola es su propio ítem (`MEJORAS #303`) | **en construcción — promovida el 2026-09-26**, por decisión de Nikolai (eligió el #303 ese día y pidió hacerlo en la sesión de `crm_ficha`, en un PR aparte): [plan](docs/superpowers/plans/2026-09-26-mejoras-303-aviso-plan-git.md). Medido sobre el `PLAN.md` real: de 49 avisos, los 49 rutas `docs/…`, a uno —una nota que conservaba el nombre de una rama podada— y a cero al retirarlo | toca `scripts/session_close.py`: una ronda sobre el diff (`gpt-6-sol`·`high`, segunda fila de la calibración de la #38) | bajo |
