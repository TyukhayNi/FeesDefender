...
| 40 | `crm_ficha`: el YAML se lee entero y se verifica el conjunto **y sus datos** (`MEJORAS #283` + `#288`) | **diseño rev. 2 con su R1 adjudicada, SIN construir** — [spec](docs/superpowers/specs/2026-09-25-crm-ficha-claves-y-conjunto-design.md) · [acta R1](docs/superpowers/specs/2026-09-25-crm-ficha-claves-y-conjunto-r1-adversarial-review.md). R1 de Codex con `gpt-6-astra`·`medium` sobre la rev. 1: `REQUIERE-REVISION`, **4 hallazgos, 4 confirmados, 0 refutados**. El que cambió el diseño: una ficha existente con el apellido vacío se resolvía por NIF, se vinculaba, **no se completaba** (el apellido no es completable) y la igualdad de ids daba «VERIFICADA» — exactamente lo que habría pasado al relanzar W-030A13 con el YAML corregido. La rev. 2 añade lectura sin pérdida (claves repetidas, alias), tipos por campo, identidad obligatoria (NIF, email o `id_crm`) y la verificación de los **datos** declarados. **La rev. 2 no tiene ronda propia**, declarado | decisión de Nikolai del 2026-09-25: el `_ficha_crm.yaml` es la lista **completa** de partes (un vínculo no declarado es fallo) y `crm_ficha` no desvincula ni pisa. **Pieza de dos rondas Astra: la de diseño, hecha; la del diff, con la implementación.** **Plan de implementación escrito** ([plan](docs/superpowers/plans/2026-09-25-crm-ficha-claves-y-conjunto.md), 8 tareas TDD, sus suposiciones técnicas comprobadas con un prototipo). Siguiente: ejecutarlo | medio-alto |
