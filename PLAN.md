...
- [ ] **F0 — saneamiento + layout + botón reorganizar:** renombrar `Sala lectura` → `01_Sala de lectura`, crear `02_Sala de máquina/`; **botón `reorganizar_caso`** (`plan`/`apply`, `--todos`, journal reversible estilo `migrate_05crm_buckets`) que migra la flota (empezando por W-02VND1) al layout nuevo; sello **`layout_version` en `_caso.md`**; **cablear `--force`** en `run_pipeline`/`pipeline.run`; alinear umbrales, corregir docstring/etiqueta de `extractor.py`, unificar set de extensiones de imagen + HEIC. Ver §J del doc.
...
- [ ] **F4 — conector MCP + empaquetado + botón reformar plugin** (preflight de capacidades, aislamiento por subproceso, versión/modelos pinneados, sin fuga de datos, preservar `core/anon`). **Botón `rebuild_plugin`** (orquesta `sync_skill_helpers`+`sync_taxonomia_skills`+`validate_skills`+`check_skills`+`package_*`) mecánico + **señalización semántica** de skills con prosa afectada (handoff estilo `motor_mejora`) + **hook de drift** no-silencioso (`session-start-hook`). Ver §K del doc.
