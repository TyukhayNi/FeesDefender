...
| 42 | Un solo lector del token de `gdrive_ev`, con timeout holgado, que distinga «tardó» de «no existe» y lo diga (`MEJORAS #296`) | **pendiente — promovida el 2026-09-26**, por decisión de Nikolai: su disparador («el próximo alta que aborte con ese mensaje») se cumplió el mismo día en que se fichó, en la apertura de W-02Y2J6 —segunda alta abortada con «token/red» teniendo el token vigente; las dos veces la salida fue pasar `--team-id` explícito—. | toca `core/intake_drive.py` y el precheck de la skill `organizar-sala-lectura`: una ronda sobre el diff | bajo |
