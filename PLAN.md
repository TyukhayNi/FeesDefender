...
1. **Restaurar los deltas perdidos** por el `git checkout` del incidente (VS Code → Timeline): `PLAN.md`, `docs/DEAD_ENDS.md`, `docs/MEJORAS_FUTURAS.md`, `docs/PLAN_PRERELLENO_LLM_VIABILIDAD.md`, plantillas viabilidad → luego re-sanear (script idempotente). **Prioridad baja** y quizá ya cubierto por la reescritura de gobernanza de `origin/main`: comparar contra la versión actual antes de restaurar.
...
3. **Parar/coordinar las sesiones cloud** y decidir el destino de las **6 ramas `origin/claude/*`** (el rewrite + force-push las desincroniza). **VERIFICADO 2026-07-06:** 5 están MERGEADAS en `main` → borrables sin pérdida; **1 sin mergear con 3 commits** (`origin/claude/feesdefender-scaffolding-review-y7l2o2`) → mergear/PR o descartar ANTES del rewrite.
