...
| 29 | Rescatar la verificación por hash del pull de Drive E&V (`MEJORAS #225`, vía (a)) | **ESCRITA Y SIN COMMITEAR desde el 2026-09-10** — 838 líneas en el worktree `caso-drive-juicio-8843b7` (rama `claude/verificar-pull-hash`, base 21 commits atrás): `core/intake_drive_hash.py` (451) y sus tests (387), más el cableado en `intake_drive`, `intake_control` y `abrir_caso`. Contrasta el `sha256` de cada fichero del destino contra el que declara la API de Drive, con **cuatro veredictos** —el cuarto, `no_verificable`, para los nativos de Google, que no tienen bytes canónicos—, informe `.verificacion_hash.json` y puerto `verificar: bool`. **Rebasar sobre `main`, ronda adversarial y PR.** Ojo: mete red en el camino del pull, que es justo lo que el §3.4 del handoff de la fila #27 proponía NO hacer — la divergencia hay que resolverla por escrito, con su puerto cerrado, no heredarla | decisión de Nikolai del 2026-09-13 sobre `MEJORAS #225`: se declara la limitación del histórico y se rescata la vía (a), que es la que vuelve el defecto **visible**. El histórico se repone solo por disparador, porque reponer antes de cerrar la causa (vía (b)) es trabajo que el siguiente pull deshace | alto |
