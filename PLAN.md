...
- [ ] **Gate de auth (testeable YA con el usuario de Nikolai):** login CRM → observar `Bearer JWT` + refresh en DevTools + endpoint de login/refresh → llamada REST con ese JWT → confirmar atribución del evento a Nikolai; precisar si el REST necesita PHPSESSID o basta JWT+refresh. NO exfiltrar el JWT. El test de rol (403 en contabilidad) necesita un usuario abogado, aparte.
