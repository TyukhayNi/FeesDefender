...
| 19 | Adoptar el CANON como copia local está permitido, y desactiva el desvío del guard (`MEJORAS #136`) | pendiente — **defecto VIVO en `main`** | **disparador: R21, 2026-09-02**, reproducido con sonda propia. `repository_cli adoptar <ruta del canon>` es **ACEPTADO**: ni `WorkspaceRegistry.alta` ni `verificar_adopcion` comprueban `bajo_catalogo`, y el canon **sí** tiene `MANIFEST_CHECKOUT.json` mientras está prestado. Desde ahí, `es_copia_prestada` → `True` y `dir_intake` devuelve el canon **sin desviar** con el caso `prestado`; y `resolver_por_identidad` da `LOCAL_CHECKOUT` con `working_root` = canon. **Gate de la rev. 2 de `MEJORAS #124`**: `es_canon` no discrimina nada mientras esta puerta esté abierta. **Radio de daño: decide quién escribe → dos rondas** | bajo-medio |
