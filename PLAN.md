...
| 39 | Mejoras de las aperturas de la semana (2026-09-15 → 25): WhatsApp (`MEJORAS #236`, `#285`), `verificar_apertura` C2/C3/C9 (`#268`, `#269`, `#218`) y `_viabilidad.json` sobre `G:` (`#276`) | ✅ **CERRADA el 2026-09-25 (PR [#397](https://github.com/TyukhayNi/FeesDefender/pull/397))** — [plan y adjudicaciones](docs/superpowers/plans/2026-09-25-mejoras-aperturas-semana.md). **Dos piezas, dos rondas.** Bloque 1 con `gpt-6-sol`·`high`: `NO-SHIP`, **6 hallazgos, 6 confirmados, 0 refutados**, tres fronteras (qué identifica al chat de un export; limpiar un identificador no puede ligarlo a otros bytes; lo que el verificador declara tiene que ser lo que mira), remediados en `94522ce` y `61523e8`, y es la **primera fila de la calibración de la #38**. `#276` con `gpt-6-astra`·`high`: `SHIP`, 0 hallazgos, sus dos mutantes supervivientes muertos en `2b2653c`. El remedio de `#276` cambió al medir: `os.rename` en `G:` **no pisa** (`WinError 183`), así que en Windows se conservan las dos promesas y no hizo falta la ventana a medias de `O_EXCL`. **Backlog:** `#287` y `#288` rescatadas del PR #387 (cerrado: sus cinco números los ocupaban otras entradas y la nota de W-030A13 figuraba `fichado`), `#237` con su diagnóstico disputado, PARTICULAR-659 como `#289`-`#291`, y `[APER-74]` en el runbook (los audios no se transcriben solos). **Las dos remediaciones NO han pasado ronda, declarado** | encargo de Nikolai del 2026-09-25 sobre las aperturas de dos semanas. Decidido por él ese día: `#276` antes que `crm_ficha`; cerrar el #387, fichar PARTICULAR-659 y anotar la nota de W-02UIQU; trabajar en paralelo a Codicert F3 sin tocar lo suyo. **Siguiente, con su precio:** `crm_ficha` (`#283` + `#288`, pieza de dos rondas Astra: decide qué partes quedan vinculadas y da acceso), `#237` (medir `threads().modify` en un hilo real, con permiso), `#190`, el cableado de `#205` y `#284`/`#265`/`#267` como una tanda de skills (re-importación en Cowork) | medio |

| 40 | `crm_ficha`: el YAML se lee entero y se verifica el conjunto **y sus datos** (`MEJORAS #283` + `#288`) | **diseño rev. 2 y plan rev. 1, con la R1 y la R2 adjudicadas; la R2, SIN aplicar; SIN construir** — [spec](docs/superpowers/specs/2026-09-25-crm-ficha-claves-y-conjunto-design.md) · [acta R1](docs/superpowers/specs/2026-09-25-crm-ficha-claves-y-conjunto-r1-adversarial-review.md) · [plan](docs/superpowers/plans/2026-09-25-crm-ficha-claves-y-conjunto.md) · [acta R2](docs/superpowers/plans/2026-09-25-crm-ficha-claves-y-conjunto-r2-adversarial-review.md). R1 de Codex con `gpt-6-astra`·`medium` sobre la rev. 1: `REQUIERE-REVISION`, **4 hallazgos, 4 confirmados, 0 refutados**. El que cambió el diseño: una ficha existente con el apellido vacío se resolvía por NIF, se vinculaba, **no se completaba** (el apellido no es completable) y la igualdad de ids daba «VERIFICADA» — exactamente lo que habría pasado al relanzar W-030A13 con el YAML corregido. La rev. 2 añade lectura sin pérdida (claves repetidas, alias), tipos por campo, identidad obligatoria (NIF, email o `id_crm`) y la verificación de los **datos** declarados. **R2 de Codex, `gpt-6-astra`·`medium`, sobre el plan y sobre si la rev. 2 remedia la R1: `REQUIERE-REVISION`, 9 hallazgos (2 `alta`), 9 confirmados, 0 refutados** — remedios de la rev. 2 a H-01/H-02 de la R1, reales; a H-03/H-04, incompletos. Tres fronteras: ninguna escritura sobre una ficha que el YAML contradice (`id_crm` se validaba solo por el nombre y los datos se comparaban después de escribir); lo declarado llega intacto a la auditoría (el DTO borra `'+34'`, la provincia nunca casaba); y cada test prueba lo que nombra (un doble que degradaba a «SIN VERIFICAR», dos negativos que el literal nuevo vaciaba). Decidido en la §9 del plan: **fase previa de solo lectura** que falla sin escribir ante un dato distinto en una ficha existente, y `id_crm` dentro de los resolutores del core | decisión de Nikolai del 2026-09-25: el `_ficha_crm.yaml` es la lista **completa** de partes (un vínculo no declarado es fallo) y `crm_ficha` no desvincula ni pisa. **Tres rondas Astra, la tercera autorizada expresamente por Nikolai el 2026-09-25 sobre el techo de dos:** R1 (diseño) y R2 (plan) hechas; la **R3, sobre el diff**. Siguiente: **aplicar la §9 del plan** —spec rev. 3 y plan rev. 2, antes de la Task 1— y ejecutarlo | medio-alto |
