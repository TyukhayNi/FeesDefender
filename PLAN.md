...
| 50 | Medir con `gpt-6-sol`·`high` dos o tres revisiones antiguas con defectos ya confirmados —la reutilización que el método de la #38 preveía y no se hizo— | **pendiente — encargada por Nikolai el 2026-09-26**, al saber las dos condiciones omitidas de la calibración. Objetos que revisó Astra en su día, en el commit que se revisó, sobre copias y **sin enseñar el remedio**: dan el denominador de defectos conocidos y los **pares Sol/Astra sobre el mismo objeto** que el ledger no tenía. Lanzador con `$OutputEncoding` en UTF-8. **Criterio propuesto, a fijar antes de correr:** si Sol no encuentra un defecto `alta` ya confirmado —el mismo defecto, no el mismo fichero, adjudicado contra la fuente—, la categoría de ese objeto sube a Astra en la tabla; si los encuentra todos, la tabla se queda | es una medición, no una pieza: no lleva ronda propia, y cada corrida se archiva con su mandato | medio |
