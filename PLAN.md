...
- [x] **Fase 1 — núcleo de workspace.** ✅ **PRs #230** (`291436d`, R7 + modelo + registro),
      ...
      **Los tres criterios de salida, contra código y no contra la memoria:** (1) la matriz del
      §14.1 corre entera sobre `sala_maquina` —nueve filas, ninguna declarada no aplicable— en
      `tests/_matriz_contractual.py`; (2) ninguna ruta crea un directorio bajo `CASOS_ROOT` para
      una identidad desconocida, y el plano 2 del arnés lo comprueba en cada fila; (3) con
      `--case-dir`, el evento de auditoría cae en el mismo árbol que los bytes.
