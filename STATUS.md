...
| Ítem | Estado |
|------|--------|
| Tests | ✅ **6.043 recogidos · 0 fallos · 0 errores · 94 en `skipped`** (incluye los `xfail`) — **medido el 2026-09-21** (130º cierre), con las **dos semillas** (777 y 31337), conteo idéntico en las dos. **+199 y cuadra al test**, medido contra el commit anterior al merge (`3305c4f`: 5.844 recogidos, contados con `--collect-only` en un worktree de ese commit, no tomados de un cierre anterior): son exactamente los 199 casos que recogen los doce ficheros de test del envío certificado por Codicert. De los 172 `def test_`, 27 salen de los parametrizados de `test_expedicion_ficha_postal.py`. La cifra se mide en cada cierre: un número viejo aquí no es un dato, es ruido. |
