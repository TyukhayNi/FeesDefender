...
| Ítem | Estado |
|------|--------|
| Tests | ✅ **4.844 passed · 88 skipped · 10 xfailed** (4.932 recogidos) · 0 fallos, 0 errores — **medido el 2026-09-09** (92º cierre) con **dos semillas**, 777 y 31337, y contado por JUnit XML (que reporta los `xfail` DENTRO de `skipped`: 78 saltados de verdad + 10 `xfail`). Lo de las dos semillas no es celo: la suite corre con `pytest-randomly` y en el 65º la 31337 salió verde mientras la **777 sacaba ocho fallos** dentro del PR que iba a mergearse. **Una sola corrida verde no dice nada sobre orden.** El salto desde los 2.879 del 2026-08-24 son los Tasks 6-11 de la Fase 1 dual (PRs #231-#236), no una anomalía. De los **10 `xfail`**, **6** son los defectos del frontal de la biblioteca reproducidos a propósito en la Fase 0 de la arquitectura dual —se arreglan en su Fase 2 (`PLAN.md` fila #3); eran 11 hasta que el Task 6 resolvió los cuatro de `test_constructores_localizador`, y 7 hasta que `MEJORAS #93-B` cerró A-2c— y los **4** restantes son los de `test_guard_copia_prestada`, que `MEJORAS #124` reprodujo el 2026-09-02. Histórico por sesión: bitácora de cierres. |
...
> **Por qué este bloque ya no lleva cifra** (2026-08-03): era un **segundo hogar** del
