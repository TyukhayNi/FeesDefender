...
| Ítem | Estado |
|------|--------|
| Tests | ✅ **6.904 recogidos · 0 fallos · 0 errores · 96 en `skipped`** (incluye los `xfail`) — **medido el 2026-09-26** en la rama del PR #409 (el token y la carpeta de Drive con su motivo, y las carpetas con el W-code delante; filas #42 y #47), sobre `main` (`aa46fcc`) y con las **dos semillas** (777 y 31337) por la verja de `session_close`: los 6.865 de `main` más los 25 de la construcción y 14 del remedio de la R1. El delta de cada cierre, explicado test a test, va en su bloque de la bitácora; la cifra se mide en cada cierre: un número viejo aquí no es un dato, es ruido. |
