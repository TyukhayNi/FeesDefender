...
**Última actualización:** 2026-06-17 (sesión 48, sala de lectura Fases 0–3, rama `feat/sala-lectura-f0a3`) — **Worktree dedicado `FeesDefender-sala`**, sin tocar `main` ni el worktree paralelo (WhatsApp Fase A). **Fase 0** (doble `extract_all`): verificada cerrada (s32). **Fase 1** — `core/catalogo_documental.py`: catálogo `indice_documental.yaml` con esquema completo (12 campos: `id_doc`, `ruta_relativa`, `nombre_original`, `tipo_documental`, `fecha_doc`, `parte`, `fuente`, `estado`, `hash`, `fecha_indexado`, `parent_id`, `orden_en_bundle`); `build_catalog` desde inventario (idempotente, mapeo de fuentes); `load_catalog` para lectura. Artefacto **nuevo e independiente** — reconciliación con `_intake_hashes.json` **diferida** (decisión de Nikolai). **Fase 2** — scaffolding: `ensure_case` crea `01_Procesado/{Sala lectura, MD, _revisar}/`. **Fase 3** — grifo de MD en claro: `markdown_generator.build` escribe en `01_Procesado/MD/{slug}.md` (era plano en `01_Procesado/`); consumidores `scorer.py` y `viability.py` actualizados al nuevo path. **Pinza de no-colisión respetada:** no se tocaron `intake_manifest.py`, `intake_log.py`, `streamlit_app.py` ni `core/whatsapp_*.py`. Commit `f253a84`. Plan de ejecución en `docs/superpowers/plans/2026-06-17-sala-lectura-f0a3.md`. **Suite: 843 passed, 58 skipped** (7 tests nuevos; `test_sudespacho_relations.py` excluido por import circular ajeno preexistente). **[SIGUIENTE]** para esta rama: Fases 4–5 (copiador organizado + bundles con `conjunto_detector`, render de índices `INDICE.md`/`CRONOLOGIA.md`). Para `main`: sin cambios (`[SIGUIENTE-INTAKE-PROCURADORES-EMAIL]` F3 sigue pendiente).

**Anterior (2026-06-16, sesión 47, homogeneización de skills — Ola 1 + mínimo reutilizable):** Ejecutado el «Alcance REVISADO 2026-06-16» de `[SIGUIENTE-HOMOGENIZACION-SKILLS]` (PLAN.md): solo corrección + mínimo reutilizable; superestructura de gobernanza **diferida** a `docs/MEJORAS_FUTURAS.md #32`. **Ola 1:** (1) `oposicion-alegacion-nulidad` — retirado el logger bespoke `log_uso.py`; migrada al helper canónico `registrar_uso.py` (store central `data/_skill_logs/`, métricas dentro de `metricas`); añadida a `_TARGETS` de `sync_skill_helpers.py`; **drift de vías 3→4** corregido (A nulidad absoluta · B vicio del consentimiento · C incorporación · D contenido/abusividad) en logger, `EVOLUCION.md` y `logs/README.md`; v1.5.2. (2) `preparacion-audiencia-previa` — retirado el shim `log_uso.py` (sin callers en código); prosa→`registrar_uso.py`; v1.0.1 + CHANGELOG nuevo. (3) `preparacion-juicio-oral` — `log_uso.js` convertido en **shim** que delega en `registrar_uso.py` (elimina la doble telemetría JS/Python; API `log`/`logTo` intacta; 4 generadores + `schedule_post_juicio.js` sin cambios; sigue exportando `LOGS_DIR`); **probado end-to-end** (node→python, esquema canónico); v1.0.1. (4) `verificacion-anclada-fuente` — `LICENSE` con el **texto íntegro de AGPL-3.0** + cabecera de atribución/modificación (NOTICE conservado; no relicenciar, no co-empaquetar, no exponer por red §13). **Mínimo reutilizable:** `_shared/_plantilla-skill/` (molde con ejes `rol`/`naturaleza` + módulos) y `scripts/validate_skills.py` (validador en **modo AVISO**, no bloquea; sin hook ni CI). El validador destapó el **bug latente #33** (frontmatter YAML inválido en `preparacion-juicio-oral`: `description` sin comillas con `:` interno) → anotado en `MEJORAS_FUTURAS.md`, **no arreglado** (retrofit de identidad diferido). **Decisión del letrado** (`AskUserQuestion`): el `log_uso.js` de juicio se retira vía shim→`registrar_uso.py`. `.skill` re-empaquetados en `dist/skills/` (4). **Commits acotados** `0282503` (homogeneización, 98 ficheros; incluye la **primera versión en git** de `oposicion-alegacion-nulidad`, que estaba untracked) + `d8eabf7` (este cierre). **Rebasados sobre `origin/main`** (`684d83c`) para reconciliar la divergencia: el commit duplicado de cendoj v1.1 (`4f08f9b` local, contenido idéntico a `684d83c` ya en origin) se descartó solo → historia lineal. **Pusheado a `origin/main`** (`684d83c..d8eabf7`). **Sin tag** (decisión 6 PLAN: Nikolai es el único aprobador). **Pendiente (Nikolai): re-importar los 4 `.skill` en el servidor**. Cambios ajenos de sesiones concurrentes (PLAN.md del handoff, viabilidad, drafts, helpers AP con churn CRLF) **dejados intactos**. **Suite: 935 passed, 58 skipped; `sync_skill_helpers --check` OK.** **[SIGUIENTE]** sin cambios (F3 — escritura en el CRM).
