...
| Ítem | Estado |
|------|--------|
| Tests | ✅ 524/524 (Anonimizador absorbido: +51 — 2026-05-07; sufijo captador Drive: +2 — 2026-05-11 s4; Numero_Expediente extrajudicial: +10 — 2026-05-11 s6; tests v2 dedicados paso 8: +113 — 2026-05-11 s7; rename informe_viabilidad: +9 — 2026-05-11 s7; verify_expediente_referencia: +15 — 2026-05-11 s8; categoría OTROS + clientes propios: +22 — 2026-05-11 s9; deanonimizar `_mapa_caso.json` 4 niveles: +13 — 2026-05-12 s11; subdivisión ciudades Fase 0: +36 — 2026-05-12 s16; retry rate-limit Drive API: +5 — 2026-05-12 s18) |
...
**[SIGUIENTE-PULL-RCLONE-EXIT1]** (sesión 18, 2026-05-12) Diagnosticar
`rclone exit 1` en `core/intake_drive.pull_drive_ev` reproducido tras
auto-fill correcto con URL `https://drive.google.com/drive/u/2/folders/1OReG4jZzwh6-l5j5AKYL_8Jpc5a6fzCX`
(Montsant 34 - Montcada i Reixac - W-0466A1, equipo **BaRS8**, Shared Drive
`0AJGVcVJqtIgpUk9PVA`). Auto-fill (Drive API v3) sí funciona; rclone copy no.
Pasos:
(a) modificar `streamlit_app.py` para mostrar el stderr completo del
`DriveIntakeError` en lugar de solo `rclone exit N` — actualmente
`pull_drive_ev` guarda `result.stderr[-2000:]` en `errors` del
`DriveIntakeResult` pero la UI muestra solo el mensaje truncado;
considerar un `st.expander("Detalles técnicos")` para no abrumar al usuario.
(b) reproducir manualmente desde PowerShell con `-vv`:
`rclone copy gdrive_ev: <target_dir> --drive-team-drive 0AJGVcVJqtIgpUk9PVA --drive-root-folder-id 1OReG4jZzwh6-l5j5AKYL_8Jpc5a6fzCX -vv`.
(c) verificar permisos de la cuenta `nikolai.tyukhay@engelvoelkers.com`
(remote `gdrive_ev`) sobre el Shared Drive de BaRS8.
(d) descartar rate-limit residual de la Drive API tras consumir cuota
en el auto-fill (hipótesis menos probable: rclone usa endpoints distintos
y reintenta solo).
