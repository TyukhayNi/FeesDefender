...
**Última actualización:** 2026-05-27 (sesión 26) — **Organizador local del Drive E&V con Ollama: Sprints 1 y 2 implementados, testeados y commiteados (`d29dbb1`).** (Sprint 1) `core/llm_local.py` nuevo: cliente Ollama con excepciones tipadas (`LLMLocalUnavailableError`/`LLMLocalTimeoutError`), retry con backoff, `health_check`/`warmup`/`complete`/`complete_json`; vars `LLM_LOCAL_*` en `.env.example`; **sin fallback a Anthropic** por diseño (la PII no sale del entorno local). Convive con `core/llm.py` (que ya era cliente Ollama), no lo reemplaza. (Sprint 2) `core/local_organizer.py` + `scripts/organizar_local.py`: clasifica los documentos en bruto de `00_Input/01_Drive EV/` sobre el material ya anonimizado y produce una vista navegable en `_organizado/` (taxonomía E&V de 8 categorías, READMEs por categoría, `_INDICE.md`, `_audit.jsonl` por SHA-256, loop de aprendizaje en `data/_aprendizaje/`); modos `--plan/--execute/--dry-run/--refresh/--rebuild/--renumerar`; invariante crítico: `_organizado/` lo ignora `core/anon/api.py` por el prefijo `_`. Constantes `TAXONOMIA_EV`/`ORDEN_POR_CATEGORIA`/umbrales en `core/config.py`; prompt versionado en `data/_prompts/clasificador_ev.md`; `scikit-learn` añadido a `requirements.txt` e instalado (few-shot TF-IDF, degrada en vacío). **Tests +13** (`tests/test_llm_local.py`: 2 verdes + 5 skip si Ollama down; `tests/test_local_organizer.py`: 11 verdes) — suite completa verde. Modelo: `qwen2.5:14b-instruct-q4_K_M`. **[SIGUIENTE-ORGANIZADOR-VALIDACION]**: validación real sobre el caso piloto BaRS1 (`BaRS1 - Tibidabo 8 - (W-02VND1) - Vuelta`) PENDIENTE — su `06_Anonimizado/` está vacío, así que hay que anonimizar primero (`python -m scripts.anonimizar_caso "BaRS1 - Tibidabo 8 - (W-02VND1) - Vuelta"`) y arrancar Ollama (`ollama serve` + `ollama pull qwen2.5:14b-instruct-q4_K_M`) antes de `--plan`/`--execute`. Fuera de alcance (futuros): integración como step de `core/pipeline.py`, botón Streamlit, router LLM local/externo.
