...
| Plan subdivisión CASOS_ROOT por ciudades | ✅ 2026-05-21 s25 — Fases 0-4 cerradas. Fase 0 (s16): `core/ciudades.py` extraído (catálogo único + `ciudad_de_equipo` + `es_carpeta_de_sistema`). Fase 1 (s25): `core/casos/case_locator.py` + refactor de call-sites en `core/case_manager`, `core/config`, `scripts/{audit_referencias_casos,scheduled_sync,sync_sudespacho}`. Fase 2 (s25): campo `ciudad` en `CaseMeta`/`_caso.md` y en `ensure_case`. Fase 3 (s25): validación blanda prefijo↔ciudad en UI Streamlit con override registrado en audit log. Fase 4 (s25): `scripts/migrate_to_city_structure.py` (Typer plan+apply, confirmación literal `MIGRAR`) + migración inicial ejecutada (9 expedientes en 5 ciudades; audit log JSONL + snapshot rollback en `_audit/`). Fases 5 (docs internas) y 6 (verificación con casos nuevos) follow-up no bloqueante. |
