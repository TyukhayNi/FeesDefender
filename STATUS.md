...
| Ítem | Estado |
|------|--------|
| Tests | ✅ **6.813 recogidos · 0 fallos · 0 errores · 96 en `skipped`** (incluye los `xfail`; por JUnit) — **medido el 2026-09-26** en la rama del PR #407 (`verificar_apertura` C1/C2/C3, fila #45), con `main` (`ad1cf3b`) fusionado y las **dos semillas** (777 y 31337) por la verja de `session_close`: los 6.771 de `main` —contados con `--collect-only` en una copia de `cc98244`; el #406 solo tocó documentos— más los 42 de la pieza (29 de la construcción y 13 del remedio de la R1). El delta de cada cierre, explicado test a test, va en su bloque de la bitácora; la cifra se mide en cada cierre: un número viejo aquí no es un dato, es ruido. |
