...
**Última actualización:** 2026-06-17 (sesión 48, intake WhatsApp Fase A) — **`[INTAKE-WHATSAPP-FASE-A]` COMPLETA en worktree `FeesDefender-wapp`, rama `feat/whatsapp-faseA`.** Parser puro `core/whatsapp_export.py` (iOS corchetes + Android guion; años 2/4 cifras; 12/24h; multilínea; mensajes de sistema; adjuntos incl. `<Media omitted>`/`Multimedia omitido`/`<archivo adjunto>` bare; `filter_by_date_range`). Glue `core/whatsapp_intake.py` (`analyze` previsualización + `deposit_export` depósito verbatim + zip original `_export_original.zip` + dedup por hash de zip vía `IntakeManifest` + evento `upload_whatsapp` + `_chat_recortado.txt` opcional). Expander «📲 Importar chat de WhatsApp» en `streamlit_app.py` tab Casos (multi-zip, rol por chat, preview con conteo mensajes/adjuntos/audios/faltantes, date-range pickers, aviso de adjuntos faltantes, feedback de dedup). Spec review: corregidos 3 gaps (patrones `<Media omitted>` y `<archivo adjunto>` bare en parser; date-range pickers en UI). **10 commits** `3734dcb`→`97a9239`. **Tests +25** (16 parser + 9 glue). **PLAN.md** actualizado con entrada ✅. **Sin merge a main** (rama en worktree separado; merge solo con la sesión concurrente de `feat/sala-lectura-f0a3` parada). Gaps menores aceptados: subida de `_chat.txt` + media sueltos (sin zip) no expuesta en UI; confirmación individual por chat en vez de batch. Fase B (email) y transcripción de audio diferidas. **Suite: 955 passed, 58 skipped** (5 fallos preexistentes en `test_sudespacho_relations.py` — ajenos, desde s37). **[SIGUIENTE]** sin cambios (F3 — escritura en el CRM en `main`; merge de esta rama pendiente de Nikolai).

**Anterior (2026-06-16, sesión 47, homogeneización de skills — Ola 1 + mínimo reutilizable):** Ejecutado el «Alcance REVISADO 2026-06-16» de `[SIGUIENTE-HOMOGENIZACION-SKILLS]` (PLAN.md): solo corrección + mínimo reutilizable; superestructura de gobernanza **diferida** a `docs/MEJORAS_FUTURAS.md #32`. **Ola 1:** (1) `oposicion-alegacion-nulidad` — retirado el logger bespoke `log_uso.py`; migrada al helper canónico `registrar_uso.py`; v1.5.2. (2) `preparacion-audiencia-previa` — prosa→`registrar_uso.py`; v1.0.1. (3) `preparacion-juicio-oral` — `log_uso.js` shim→`registrar_uso.py`; v1.0.1. (4) `verificacion-anclada-fuente` — AGPL-3.0 LICENSE. Mínimo reutilizable: `_plantilla-skill/` + `validate_skills.py`. Commits `0282503`+`d8eabf7`. **Suite: 935 passed, 58 skipped.** **[SIGUIENTE]** F3 escritura en el CRM.
