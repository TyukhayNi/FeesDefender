...
| Ítem | Estado |
|------|--------|
| Tests | ✅ **6.771 recogidos · 0 fallos · 0 errores · 96 en `skipped`** (incluye los `xfail`; por JUnit) — **medido el 2026-09-26** sobre el árbol de `main` en `cc98244` (PR #405, git que falla en voz alta), con las **dos semillas** (777 y 31337) por JUnit y por la verja de `session_close`: los 6.713 de `main` antes del #405 más sus 58 (53 del fichero nuevo y 5 del leak-guard). El delta de cada cierre, explicado test a test, va en su bloque de la bitácora; la cifra se mide en cada cierre: un número viejo aquí no es un dato, es ruido. |
