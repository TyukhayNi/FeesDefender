...
| Ítem | Estado |
|------|--------|
| Tests | ✅ **6.695 recogidos · 0 fallos · 0 errores · 96 en `skipped`** (incluye los `xfail`; por JUnit) — **medido el 2026-09-26** en la rama del PR #400 (`crm_ficha`) ya fusionada con `main` (`add7b2f`): verde con las **dos semillas** (777 y 31337) por la verja de `session_close`. **+212 y cuadra al test** contra `origin/main` (`220a3f9`: 6.483, contados en una copia de ese commit, no tomados de un cierre anterior): 186 de la construcción de `crm_ficha` y 26 del remedio de su R3; ningún test retirado. Los 256 que `main` sumó desde `4209582` (6.227) son del #398 (Codicert). La cifra se mide en cada cierre: un número viejo aquí no es un dato, es ruido. |
