...
**Última actualización:** 2026-06-25, **motor de atomización de correo — Fase 2 (Layer B: reconstrucción de autoría INLINE) COMPLETA**, vía brainstorming→spec→plan→executing-plans con **diseño sintetizado por workflow adversarial** (4 diseños independientes × 3 jueces + síntesis; ganador *header-anchor* + grafts; el workflow cazó 3 fallos que rompían la arquitectura —doble autoridad sobre el bloque del autor, guarda de intercalada ciega al HTML dominante, upgrade de fidelidad contra el path de IDs congelado— y los resolvió ANTES de codificar). Nuevos `core/email_atomize/inline.py` + `_segmenter.py` (**autoridad única** del corte autor/cita, compartida con `bodies` vía `conservar_resto`): segmenta cada portador (HTML por `html.parser` —caso dominante 120/138— y texto plano multimarcador, con guarda de intercalada en ambos formatos), atribuye remitente/fecha **solo desde cabecera inline parseable** (ES+CA), clasifica confianza **alta-reconstruida/media/baja** con guardas anti-misatribución (fecha de la cita posterior al portador→nunca alta; cualquier predicado fallido demota un nivel, **nunca redondea hacia arriba**), **fingerprint día-granular** con piso ≥24 chars (identidad estable de mensajes sin Message-ID; absorbe jitter tz), upgrade de fidelidad por alias `Message-ID`/`cuerpo_sha` **sin tocar Capa A**, y cola de revisión `_revision/{cola,casi_duplicados,del_burgo}.md`. **Directriz primaria: CERO MISATRIBUCIÓN** (un remitente solo se afirma con cabecera verificada; todo lo demás → revisión). Integración **aditiva**: `model` (+6 campos + `SegmentoEnterrado`), `ids` (índice fp + alias, `_registro.json` v1→v2 con loader tolerante), `pipeline` (pase B **tras** congelar los IDs de Capa A), `render`/`corpus`. **Corrida real W-02VND1 (verificación adversarial sobre datos reales):** los **277 `.md` de Capa A quedan BYTE-IDÉNTICOS** (prime directive, 0 cambios verificados por hash) + **14 mensajes Capa B alta-reconstruida** (todos participantes legítimos —UCSA/PersonaTres, asesores E&V, el letrado— con cita ESTRUCTURAL y fecha válida; **0 misatribuciones** auditadas a mano), **233 citas a revisión**, **1 upgrade**, **0 errores**, **idempotente** (re-corrida: mismos fp, sin renumerar). **PersonaUno:** 12 correos directos ya en Capa A (Fase 1) + **4 citas inline** localizadas (`per01a@example.invalid`×3, `per01c@example.invalid`×1; `media`→`del_burgo.md` con su portador) — su autoría enterrada queda **localizable**. **Tests +45** (`tests/test_email_atomize_{segmenter,model_b,ids_fp,inline,render_b,pipeline_b,regresion_b}.py`). **Suite: exit 0, 58 skipped** (excluyendo los 2 ficheros `mcp` pre-existentes ajenos). **Spec** `docs/superpowers/specs/2026-06-25-email-atomize-layerb-design.md` + **plan** `docs/superpowers/plans/2026-06-25-email-atomize-fase2.md`; código en `origin/main` (post-commit hook). **Residual → Fase 3:** recall PersonaUno conservador (citas en plain-Outlook/apple sin fecha parseable → `media`, no promovidas; ~4 de ~19 ocurrencias inline surgen atribuidas) → mejorar parser de fechas ES/CA y extracción de niveles profundos; mover `IDENTIDADES_VIGILADAS` a `identidades.yaml` por caso (añadir `per01b@example.invalid` candidato→tope media e `ignacio@despacho-ab.example` parte DISTINTA); vistas temáticas; `_entregas/`; OCR. **Concurrencia:** commits acotados a ficheros nuevos/propios (sin `add -A`); `PLAN.md`/`CLAUDE.md` con cambios ajenos → entrada de `PLAN.md` al working tree **sin commitear**, `CLAUDE.md` intacto. **[SIGUIENTE]** del proyecto sin cambios (F3 intake procuradores — escritura en el CRM).
