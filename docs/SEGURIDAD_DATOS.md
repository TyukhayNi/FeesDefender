...
| 7 referenciar por código | escaneo de nombres/emails de tercero en el contenido (blocklist gitignored) | `leak-guard` (`escanear`) en pre-commit + CI (con secret `PII_BLOCKLIST`) | **vigente** (CI exige el secret: el job `leak-scan` falla si no está definido). **En worktrees, desde 2026-09-05 (`MEJORAS #161`, PR #289):** la lista se lee también del **checkout principal** del mismo repo (el primero de `git worktree list`, verificado por resultado), porque los gitignored no existen en un worktree y el hook pasó meses en verde sin mirar; si no la encuentra en ninguna raíz, lo **declara en STDERR** en vez de callar, y el hook es `verbose` para que pre-commit muestre ese aviso. No falla cerrado todavía |
