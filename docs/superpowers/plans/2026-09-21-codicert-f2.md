...
def _claves_de_contacto(parte: dict) -> set[str]:
    ...
    porque el destinatario SMS real de producción lo lleva delante (`34XXXXXXXXX`, spec §1.1).
    """
    claves: set[str] = set()
    for campo in ("email",):
...
### Task 10: Cosechar lo provisional — **añadida durante la ejecución, por una medición**

No estaba en el plan y la razón de que esté es que **el humo de la Task 9 la pidió**,
lo que conviene dejar escrito en vez de disimularlo como si el plan lo hubiera
previsto.

Corriendo `_codicert_humo_f2` contra `W-04AKM2 - OVC` apareció `006catf83zx`, una
entrega electrónica con estados `[17, 14, 21]`: **entregada y nunca leída**. Con el
criterio de la Task 2 —cosechable solo al culminar (20 la EEC, 19 el burofax)— ese
certificado no se archivaría **hasta que el envío caducara**, y cuánto tarda eso no
está medido. No era un caso de laboratorio: era uno de los tres envíos de una
expedición viva, un tercio de la prueba de ese requerimiento.

El criterio por defecto **no cambia** —sigue siendo el seguro, y el motivo por el que
excluía los provisionales era bueno: sin estado en el nombre, el provisional ocupa el
sitio del definitivo—. Lo que se añade es la salida:

- `cosechar(..., incluir_pendientes=True)` y `--incluir-pendientes` en el CLI.
- El nombre lleva el estado: `<ASUNTO> - <REF>-<codigo> (estado N).pdf`.
- La clave del registro pasa a ser `<id_envio>@<codigo>` para el provisional y sigue
  siendo `<id_envio>` para el definitivo, así que **no se pisan**: se puede cosechar
  el provisional hoy y el definitivo cuando llegue, y hay un test que lo prueba
  en esa secuencia exacta.
- **La puerta del art. 17.2 no se relaja**: el emisor se verifica igual, con su test.

Cinco tests nuevos en `tests/test_expedicion_cosechar.py`, todos vistos en rojo antes.

---

---

## 11. Adjudicación de la revisión adversarial (Codex, 2026-09-21) — NO-SHIP, remediado

- **Objeto revisado:** el diff de `claude/codicert-f2-envio-2f9b7f` contra la base de F1 (`6d60187`), commit `8dd3952` — 18 ficheros, +5.659 / −12
- **Ronda:** 1, sobre el diff — **una, por lectura de la tabla de `CLAUDE.md`**, no por decisión de ampliar ni de reducir
- **Revisor:** Codex CLI `0.155.0-alpha.9.2`, `model_reasoning_effort=high`
- **Informe recibido:** `2026-09-21-codicert-f2-r1-adversarial-review.md`, `sha256` del bloque literal `543385cf0340bd070317f1464b1d18ddfa52dbd1f1867ff146516c20c6638898`
- **Hallazgos:** 10 — 3 `alta`, 7 `media`, 0 `baja`. **Los diez confirmados, ninguno refutado**
- **Remediado en:** los diez

**Por qué esta ronda vale.** No leyó: **corrió**. Las dos semillas de la casa, una
subauditoría separada de los guards, y **una sonda ejecutable por hallazgo** — incluidas
tres que montan un árbol de pruebas propio para atacar mis guards desde fuera. Un
hallazgo con su sonda no se discute: se reproduce, y eso hice con los diez.

**Dos de los diez los encontré yo mientras el revisor trabajaba** (H-01 y H-07), y
estaban remediados en commits posteriores al objeto revisado antes de que llegara el
informe. Eso no resta valor a la ronda: **los otros ocho no los vi**, y dos de ellos son
`alta`.

### Los tres altos

| # | Hallazgo | Coste | Adjudicación |
|---|---|---|---|
| **H-01** | El transporte de `entorno_real` no implementa `estados` ni `certificado`: `refrescar` y `cosechar` mueren con `AttributeError` en el camino real | acotado | **Confirmado**, y encontrado por las dos partes por separado. Los 92 tests pasaban porque todos inyectan un doble que sí los tiene. Ya remediado en `d929553`, **con un guard que deriva por AST lo que el core consume** en vez de escribir la lista, para que no vuelva a desincronizarse |
| **H-02** | Recuperar una reserva abierta cierra con `sha256=""` **sin descargar ni comparar**: un binario que la primera cosecha detectó como incorrecto queda dado por bueno | acotado | **Confirmado contra la fuente.** Es peor de lo que parece: `subir_documento` ya había abortado por hash distinto, y la segunda corrida **anula esa detección**. Contradice el `Global Constraint` «verificar por resultado, nunca por status» en el punto exacto en que más importa |
| **H-04** | Dos partes con el mismo email/móvil/nombre: `indice.setdefault` se la adjudica a la primera, y **invertir el orden del CRM cambia quién consta como receptor** | acotado | **Confirmado**, y no es hipotético: este CRM tiene fichas que comparten email — está medido y documentado. Mi propio docstring decía que atribuir al primero «inventaría un reloj», y el código hacía justo eso |

### Los siete medios

| # | Hallazgo | Coste | Adjudicación |
|---|---|---|---|
| H-03 | La cuenta emisora se lee de la **primera** mención de «con nombre de usuario», no del bloque certificante | acotado | **Confirmado.** Acoté la razón social al bloque del emisor y **no hice lo mismo con el usuario** — remedié el ejemplo y no la frontera, que es el defecto que `CLAUDE.md` me señala por escrito |
| H-05 | El registro de cosecha cuelga de `Path.cwd()`: lanzar desde otro worktree pierde la memoria y **vuelve a subir** | acotado | **Confirmado.** Y tiene razón en la crítica al test `test_MUTANTE_…`: de «no se puede sostener la ausencia» yo deducía permiso para subir, que es lo contrario del criterio del spec §5.2 |
| H-06 | `cosechar` no exige el mutex ni sostiene candado intraproceso; la exclusión vive solo en el CLI | acotado | **Confirmado.** Asimetría real con `ejecutar` de F1, que **exige** la sesión y nunca la adquiere. Dos llamadas al core desde el mismo proceso suben dos veces |
| H-07 | La rama «ya estaba» devuelve una ruta local sin comprobar existencia ni hash | acotado | **Confirmado**, encontrado también por mí y remediado en `d929553` con `local_presente`. **Su remedio es más completo que el mío**: pide además conservar la ruta real y el hash en el cierre, porque un cambio de asunto reconstruye una ruta distinta de la escrita |
| H-08 | El control positivo de la barrera CRM depende de que `SUDESPACHO_API_KEY` exista en la máquina | trivial | **Confirmado.** Pasa en mi PC porque la variable está puesta; en una máquina limpia el test falla **sin llegar a probar la barrera**. Un control que depende del entorno no acredita nada |
| H-09 | La barrera CRM es una fixture de función: **no cubre la fase de colección** | acotado | **Confirmado.** Y lo acreditó con un `httpx` sintético que demuestra que la llamada se ejecuta. Es la limitación heredada del patrón de F1, reproducida en un módulo que **escribe** |
| H-10 | El guard de disco comprueba una keyword por AST, no el destino real de la escritura | acotado | **Confirmado, y es el que más escuece.** Mi docstring afirmaba cubrir «el único camino»; el guard solo ve presencia sintáctica. Peor: el «default a producción» que decía impedir **ya no existe**, porque el core para si falta el puerto. La afirmación era falsa en las dos direcciones |

### Lo que el revisor NO pudo verificar, y lo declaró

Su §SIN VERIFICAR es tan útil como los hallazgos: no ejerció la API real de Codicert ni
la del CRM (no tiene credenciales, y no debe tenerlas), así que **el contrato del §17 y
la anatomía del certificado siguen acreditados solo por mis mediciones**. Tampoco
dictaminó sobre la corrección jurídica de los preceptos citados. Ninguna de las dos
cosas se le pedía, y decirlo es lo correcto.

### Remedio de los diez (2026-09-21)

Todos con su control **visto en rojo antes** del arreglo. Detalle en el §12.

---

## 12. Remedio de los diez, con su control en rojo antes

Cada uno se escribió viendo primero el control fallar **por la razón del hallazgo**,
no por un typo. Los commits van en el orden en que se hicieron.

| # | Qué se cambió | Commit |
|---|---|---|
| **H-01** | `_Transporte` gana `estados` y `certificado`. Y un guard nuevo, `tests/test_guard_transporte_real_completo.py`, que **deriva por AST** lo que el core consume de cada puerto inyectable y lo contrasta con lo que la implementación real ofrece — la lista no se escribe, para que no se desincronice | `d929553` |
| **H-02** | La recuperación de una reserva **baja el documento del CRM y compara su `sha256`** contra el certificado antes de cerrar. Antes cerraba con hash vacío, anulando la detección de la corrida anterior | `3fe613c` |
| **H-03** | La cuenta emisora se lee con **dos cercos**: bajo el rótulo `CERTIFICADO` **y** dentro de la frase certificante. Antes ganaba la primera aparición del literal en todo el texto | `3fe613c` |
| **H-04** | Índice contacto → **conjunto** de partes; un contacto que no es unívoco no atribuye a nadie y el envío va al cajón `SIN_CASAR` | `3fe613c` |
| **H-05** | Censo inmediato por **nombre canónico** vía `related_register` antes de subir: la pérdida del registro local ya no autoriza a duplicar. `buscar_por_nombre` comparte recorrido con `buscar_por_origen_id` | `edbe440` |
| **H-06** | `cosechar` sostiene candado intraproceso y **exige** el mutex, como `ejecutar` de F1. El core comprueba y nunca adquiere, que es la regla dura del proyecto | `edbe440` |
| **H-07** | `CertificadoCosechado.local_presente`, y el frontal avisa `⚠️ NO ESTÁ (solo en el CRM)`. El cierre guarda además el hash verificado | `d929553` + `3fe613c` |
| **H-08** | El control positivo de la barrera fija una **clave sintética**; y un segundo test fija el orden (sin clave se para antes de la red) como comportamiento deliberado | `edbe440` |
| **H-09** | La barrera documental se instala **al importar `conftest.py`**, antes de coleccionar. La fixture se queda como segunda capa. El control es una llamada en ámbito de módulo que tiene que morir en la barrera | `02a300a` |
| **H-10** | Control nuevo que **ejecuta** la cosecha y comprueba dónde cayeron los bytes. Y se retira del docstring una afirmación falsa: el guard no impedía «el default a producción», porque ese default ya no se alcanza | `02a300a` |

### Dos cosas que el remedio hizo más grandes de lo que el hallazgo pedía

No por completitud, sino porque en los dos casos el hallazgo era **un ejemplo de una
frontera** y remediar el ejemplo habría dejado la frontera abierta — que es el error
que `CLAUDE.md` me tiene anotado por escrito y que H-03 demuestra que cometí:

- **El guard del transporte cubre los DOS puertos**, no solo `codicert`. El defecto de
  H-01 es una propiedad del patrón —un puerto inyectable cuyo doble está completo y
  cuya implementación real no—, y `gestor` tiene la misma forma. De hecho `gestor` ganó
  tres métodos remediando esta misma ronda: sin el guard extendido, el siguiente se
  olvidaría igual.
- **H-03 se remedió por el cerco, no por el caso.** Acoté la razón social y dejé el
  usuario suelto; el revisor entró justo por ahí. El arreglo aplica el mismo
  tratamiento a los dos y deja escrito por qué.

### Lo que sigue SIN VERIFICAR, y no lo tapa ningún remedio

- **No hay R2.** La tabla de `CLAUDE.md` da **una** ronda a esta pieza y una se hizo.
  Los diez remedios los escribí yo, con su control en rojo antes y la suite verde con
  las dos semillas — eso prueba que cada remedio hace lo que dice, **no** que ninguno
  haya abierto algo que nadie fue a buscar. Sobre los remedios, la cobertura
  independiente es **AUSENTE**. Pedir una segunda ronda porque «la primera encontró
  algo» es el argumento que nunca se agota, y es el que `CLAUDE.md` prohíbe
  expresamente.
- **La subida real al CRM sigue sin ejercerse.** Todos los tests van contra dobles. El
  contrato del §17 está medido sobre nueve documentos reales, pero **este módulo no ha
  subido ninguno**: la primera subida real es una verificación pendiente y debe hacerse
  sobre un expediente de prueba.
- **Las otras seis plazas.** Solo Madrid tiene credencial.
