...
class TestAnclajeVerificable:
    ...
    def test_un_fallo_de_extraccion_no_se_confunde_con_un_documento_vacio(self, cd):
        """MEJORAS #201: `empty` mezcla «no tiene texto» y «no pude leerlo». La
...
1. **Cabecera con la regla en una frase:** «los documentos se resuelven por CENSO y por ROL, nunca por ruta canónica», y la medición que la sostiene (los 23 + 9 documentos de W-02USSI en `99_Sin categoria` y `99_Otros` por el `id_carpeta 304` sin mapear, `MEJORAS #203`).
...
4. **Sección «Cautelas medidas»**: `estado: empty` mezcla «no tiene texto» y «no pude leerlo» (`MEJORAS #201`), así que se lee también `nota`; y basta una copia con `estado: ok` para sostener el anclaje.
