...
- Produces: `RegistroIntencion` (clase con `ruta`, `anotar(id_personalizado, canal, etiqueta) -> str`, `cerrar(clave, id_envio)`, `en_vuelo() -> list[dict]`, `hechos() -> set[str]`), `ya_expedido(listado, id_personalizado) -> set[str]`.
...
## 13. Adjudicación de la revisión adversarial (Codex, 2026-09-21) — NO-SHIP, remediado

- **Objeto revisado:** el diff de `claude/codicert-burofax-ovcs-89e7aa` contra `main`, commit `52c2c0e` — 8.102 líneas, 41 commits
- **Ronda:** 2, sobre el diff
- **Revisor:** Codex CLI `0.154.0-alpha.6.2`, con cupo propio recuperado el 2026-09-19
- **Informe recibido:** `2026-09-17-codicert-f1-r2-adversarial-review.md`, `sha256` `a936c008984a256e48bbe5da56d8e3f26552f9be4488cf9f255a81a991dfee4c`
- **Hallazgos:** 16 — 7 `alta`, 9 `media`, 0 `baja`
- **Remediado en:** **los dieciséis**, el 2026-09-21. Altos: `958102a` (H-06), `3744fc2` (H-01, H-02), `769ae02` (H-03, H-04), `731495c` (H-05, H-13). Medios: `e53d473` (H-11, H-12), `432791c` (H-07, H-08, H-15), `3e4846f` (H-09, H-10, H-14, H-16)

**Por qué esta ronda vale más que la R1.** Codex no leyó: **corrió**. 128 pruebas del ámbito en
verde, **sondas ejecutables que reproducen cada defecto**, y un mutante que sobrevive a las 128.
Un hallazgo con su sonda no se discute: se reproduce. La R1 la había ejecutado un sustituto con la
independencia débil; esta la ejecuta el revisor con bolsa propia.

### Los siete altos

| # | Hallazgo | Coste | Adjudicación |
|---|---|---|---|
| H-06 | **Los apellidos del CRM no se leen**: `1apellido` y `2apellido` existen en `clientes_contrarios` y el código usa solo `nombre` | acotado | **Confirmado.** `grep` sobre los dos módulos: cero ocurrencias. ANA LÓPEZ GIL sale como «ANA» en los tres canales |
| H-01 | El digest **no sella** ordinal, asunto, cuerpo, coste ni cuenta emisora; `frozen=True` no inmoviliza las listas | estructural | **Confirmado.** La vía es pública: `--ordinal` |
| H-02 | El PDF **se reabre** tras comprobar su huella, y entre medias se pagina el listado | acotado | **Confirmado** |
| H-03 | Un listado remoto vacío **reactiva envíos ya cerrados** en disco: 6 POST por 3 | acotado | **Confirmado** |
| H-04 | El registro no distingue **entorno ni cuenta**: los cierres de sandbox dan por completa una expedición de producción | estructural | **Confirmado** |
| H-05 | **Dos ejecuciones simultáneas** pasan ambas guardas y duplican | estructural | **Confirmado** con barrera de dos hilos |
| H-13 | **Olvidar `cliente=`** elude el guard entero: no importa `httpx`, no nombra URL, no llama a `_cliente_real` | acotado | **Confirmado.** El guard es un barrido de texto, no una barrera de ejecución |

### Los nueve medios

H-07 un rechazo deja una intención sin transición de recuperación · H-08 reanudar exige saldo
para lo ya pagado · H-09 sin preflight de formato, límites ni bytes · H-10 el móvil postal lleva
prefijo y cualquier país se convierte en España · H-11 el transporte aún propaga excepciones
ajenas a su contrato · H-12 un mensaje remoto que refleje la clave se imprime sin filtrar ·
H-14 la suite no detecta que `texto_de` deje de llamar a su guarda (mutante superviviente) ·
H-15 el registro acepta corrupción semántica que hace desaparecer pendientes · H-16 un expediente
sin contrarios termina con éxito y «ENVIADO» vacío.

### La alarma documental, resuelta — y el revisor hizo bien en no afirmarla

El informe señala que el diff elimina `docs/PROCESO_BAD_DEBT_EV.md` y fragmentos de bitácora, y
**declara que sin historial no puede determinar** si son borrados de la rama o diferencias por
comparar contra un `main` posterior. Es lo segundo: `main` avanzó con `c8b99d1` (PR #389) después
de que esta rama saliera de `4c0634e`. **Medido con `git merge-tree --write-tree`, no predicho:**
el merge sale limpio y el fichero sobrevive. No hay regresión.

### Lo que el revisor intentó refutar y no pudo

La confirmación cruzada, el cambio de plaza o entorno, el PDF cambiado **antes** de ejecutar, el
saldo vivo insuficiente, el pendiente sin resolver y el identificador remoto desconocido **sí
bloquean**. El censo del guard **sí** incluye el doble, y su control positivo falla donde dice.
Eso acota el daño: lo que falla no es la puerta entera, son huecos concretos en ella.

### Remedio de los siete altos (2026-09-21)

Cada uno con su modo de fallo reproducido **en rojo** antes del arreglo, que es lo que distingue
un remedio de una afirmación:

| # | Qué se hizo | Control que lo acredita |
|---|---|---|
| H-06 | `nombre_completo_de` compone el nombre desde `nombre`, `1apellido` y `2apellido`, y se usa en los tres sitios; las fixtures pasan a la forma real del CRM | Al separar los campos, **dos tests preexistentes cayeron solos**: `assert 'ANA Y LUIS' == 'ANA LOPEZ Y LUIS PEREZ'`. El fixture tapaba el defecto |
| H-01 | El sello cubre ahora identificador **con ordinal**, plaza, entorno, **usuario emisor**, asunto, cuerpo, coste y huellas; se ejecuta la instantánea validada | Cuatro ataques en rojo: colisión de ordinal, destinatario mutado tras aprobar, textos y coste sustituidos con `dataclasses.replace`, y cuenta emisora cambiada |
| H-02 | Los documentos se leen **una sola vez**: las huellas y los adjuntos salen de los mismos bytes | Sustituir el PDF durante la ventana de `listar()` — comprobado sobre el base64 realmente enviado |
| H-03 | Los cierres locales cuentan **también** ante censo remoto vacío: o se conservan como hechos, o se para y se declara SIN VERIFICAR | Reejecutar con listado vacío ya no manda otros tres |
| H-04 | El registro distingue **cuenta y entorno** y liga la identidad al contenido aprobado; el formato antiguo **para y lo declara** en vez de inferir | Cierres de sandbox completando producción; y un plan con el CP corregido dado por «completo» |
| H-05 | Exclusión entre procesos por cuenta, entorno y expedición, **adquirida en `scripts/`** sobre la primitiva que ya existe — `core/` la exige y nunca la toma (guard E5) — más un candado en proceso | Dos ejecuciones solapadas con barrera de hilos, y el aborto limpio cuando el mutex está ocupado |
| H-13 | **Barrera de ejecución** en `tests/conftest.py`, activa antes de que ningún test pueda llamar. Los barridos de texto se quedan como control auxiliar | Llamar a la API pública **sin inyectar cliente** alcanzaba de verdad `POST https://ws.codicert.tk/v2/usuarios/acceso`, comprobado con centinela y sin abrir conexión |

**Suite completa tras los cuatro commits: 5.907 pruebas, 0 fallos.**

### Los nueve medios, cerrados el mismo día

| # | Qué se hizo | Control que lo acredita |
|---|---|---|
| H-11 | Punto único que traduce los fallos de transporte; se valida la forma del sobre antes de indexarlo. Un fallo tras un POST **conserva** que el envío pudo salir, sin reintentos automáticos | Seis caminos en rojo: timeout en tres endpoints, `200 OK` sin `datos`, JSON `null` y JSON `[]` |
| H-12 | El texto remoto se sanea antes de entrar en el error de autenticación, también al encadenar excepciones | Una respuesta que refleja la clave; comprobado que no queda rastro ni en la causa ni en el traceback |
| H-07 | Se distingue **rechazo definitivo** —transición persistida, sin inventar identificador— de **desenlace desconocido**, que sigue bloqueado hasta que un humano reconcilie | Un 422 en el tercero de seis; y el control inverso: el timeout **sigue** bloqueado |
| H-08 | Los pendientes se determinan **antes** y el saldo se compara con su **coste residual**; el plan muestra total, hecho y pendiente | Saldo de 16,9716 € con solo el burofax pendiente: antes exigía 18,2348 € |
| H-15 | Se validan campos, estados y transiciones por clave; un estado desconocido, un cierre duplicado o uno huérfano **paran diciendo fichero y línea** | Una línea JSON válida con estado mal escrito hacía desaparecer el pendiente de los dos controles |
| H-09 | Preflight de los documentos contra los canales: que sean PDF de verdad, número, tamaño y páginas; el presupuesto incorpora el volumen | Once ficheros que no eran PDF llegaban a los tres envíos; y `0,7931 €` idénticos con 0 bytes y con 7 MiB |
| H-10 | La forma del móvil depende del canal —nueve dígitos en el campo postal, internacional en el electrónico— y un domicilio extranjero se rechaza en el plan | `34600000001` donde el contrato pide `600000001`; y un domicilio francés convertido en español |
| H-14 | Un test recorre la conexión de verdad, no el helper | **Desconectar la guarda pone en rojo el test nuevo, mientras el viejo sigue verde**: exactamente el defecto descrito |
| H-16 | No se puede planificar sin requeridos, y el mensaje dice que faltan las relaciones del CRM | Un expediente sin contrarios devolvía código 0 y un «ENVIADO» vacío |

### Lo que esto dice de mí, y conviene retener

Seis de los siete altos son fallos de **cobertura de mis propios tests**, no de lógica que se me
escapara: el más caro —los apellidos— estaba en el atlas del CRM que consulté el primer día, y mis
fixtures ponían el nombre completo en `nombre`, así que **ningún test podía verlo**. Es «el doble
que tapa la condición» otra vez, y esta vez sobre el dato que va impreso en un burofax.

### No hay R3 — y los dieciséis remedios quedan sin verificar

Nikolai denegó la tercera ronda el **2026-09-21**. Es suya por construcción: el techo duro la
reserva a su autorización expresa. La consecuencia es que **nadie con interés contrario ha leído
los dieciséis remedios**: su cobertura es **AUSENTE**, no refutada. Lo que los acredita —cada
control visto en rojo antes del arreglo, y la suite verde con las dos semillas— dice que cada
remedio hace lo suyo, no que ninguno haya abierto algo que nadie fue a buscar.

Declaración completa, con la distinción entre exención y dispensa: **§12.2 del spec**.
