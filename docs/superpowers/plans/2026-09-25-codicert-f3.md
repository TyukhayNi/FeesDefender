...
fuera **de lo visible**, huellas iguales a las del manifiesto, sello de firma presente en el
íntegro y ausente en el aportable (comprobado a ojo, renderizando solo la esquina del widget);
crédito 836,9215 € antes y después. ⚠️ **El humo leía las páginas, no el fichero**, y los dos
aportables llevaban dentro el formulario de las condiciones: lo encontró la R1 (H-01, §12).

## 12. Adjudicación de la revisión adversarial (Codex, 2026-09-25) — NO-SHIP, remediado

- **Objeto revisado:** el diff de `claude/feesdefender-codicert-connection-32083b` contra `origin/main` (`9880165`), commit `a87e2d9` — 13 ficheros, +4.154 / −14
- **Ronda:** 1, sobre el diff — una, por lectura de la tabla de `CLAUDE.md`
- **Revisor:** Codex CLI `0.155.0-alpha.16.3`, `gpt-6-astra`·`medium` releídos del log; `service_tier="default"` afirmado desde el lanzador conservado
- **Informe recibido:** `2026-09-25-codicert-f3-r1-adversarial-review.md`, `sha256` del bloque literal `c20d75e915f1783840831d66d6c143b26e2f0d621aa7a33c3231e37de24d1253`
- **Hallazgos:** 7 — 2 `alta`, 5 `media`, 0 `baja`. **Los siete confirmados contra la fuente, ninguno refutado**, más dos observaciones laterales también confirmadas
- **Remediado en:** los siete y las dos observaciones, `1cd7f81`..`d3850be` (nueve commits)

**El hallazgo que justifica la ronda era PEOR de lo que su sonda enseñaba (H-01).** El revisor
lo demostró con un enlace sintético `/Link` → `/Dest` a la página retirada. Al adjudicarlo sobre
los **certificados reales** apareció la vía normal, no la exótica: cada página de la reproducción
dibuja un Form XObject (`q /TPL2 Do Q`) y los formularios de **todas** viven en un `/Resources`
que comparten las ocho. Copiar las páginas conservadas copiaba ese diccionario entero, y con él
`/TPL2`: **los dos aportables del humo llevaban dentro la página de condiciones**, invisible y
recuperable de su estructura —dibujada aparte, 1.327 caracteres con el rótulo—. Mi «verificación
por resultado» releía el TEXTO de las páginas del aportable, no el FICHERO: comprobaba lo que se
ve y no lo que hay. Esos dos PDF solo existieron en el directorio temporal de la sesión: nunca
fueron al expediente, al CRM ni a nadie.

### Los dos altos

| # | Hallazgo | Coste | Adjudicación y remedio |
|---|---|---|---|
| **H-01** | La página retirada viaja dentro del PDF por una referencia (el revisor: un `/Link`/`/Dest`) | estructural | **Confirmado, y agravado al medirlo en los reales** (arriba). Remedio de frontera, no del ejemplo: cada página conservada se lleva solo sus claves de dibujo y sus recursos **podados a lo que su contenido invoca** (`_podar`), sin anotaciones; y una **relectura del grafo entero** (`_verificar_grafo`) recalcula en el original qué streams usaban solo las páginas retiradas y exige que ninguno esté en el fichero, cuenta los objetos de página y exige cero anotaciones. Sobre los reales, sin red: formularios con el rótulo **1 → 0** en los dos canales; páginas conservadas **idénticas píxel a píxel** al íntegro salvo la caja del sello de firma. `1cd7f81` |
| **H-02** | Un segundo documento cuyo rótulo parte la extracción («CONFIDENCIAL -» / «CONDICIONES») sale entero, `PRODUCIDO` y sin aviso | acotado | **Confirmado** contra la fuente: `lleva_rotulo` casaba la línea entera. Ahora casa la **frase** con su guion, con cualquier espacio o salto alrededor; las palabras sueltas de M-7 siguen sin formarla (test de control con el requerimiento, que lleva «Oferta Vinculante Confidencial» y «condiciones adjuntas» en la misma página). Un test mío que exigía lo contrario —que «CONFIDENCIAL - CONDICIONES ECONÓMICAS DE PAGO» **no** contara— se invierte: la regla se endurece, retirando de más. `569c170` |

### Los cinco medios

| # | Hallazgo | Coste | Adjudicación y remedio |
|---|---|---|---|
| H-03 | El parser de FICHEROS acepta un bloque fuera del acta o una lista truncada | acotado | **Confirmado.** Solo cuenta el bloque de una página del **acta**, único, leído desde su cabecera «Nombre Huella digital» y **hasta su línea de cierre**; sin cierre, para. Las tres listas reales siguen pasando. `21db02f` |
| H-04 | Un manifiesto existente no se valida y uno huérfano se pisa | acotado | **Confirmado.** El que ya está se compara con el recién calculado —todo salvo `generado`, que conserva su instante—, tenga o no el aportable al lado; si no corresponde, `PARADO` sin tocarlo. `c9d4f62` |
| H-05 | Falta el aviso de sobre conjunto si un nombre lleva « Y » | acotado | **Confirmado.** Se lee al revés: el destinatario es una parte, o se cubre entero con dos o más nombres completos unidos por « y », o **no se puede atribuir, y se avisa** —lo que cubre también los nombres del CRM que cambian, que el revisor señaló—. `8a86d3f` |
| H-06 | La despedida del primer documento anula el aviso de los siguientes | acotado | **Confirmado.** Un cuerpo por documento, cada uno cortado en su propia despedida. `a27e56a` |
| H-07 | La suite no protege las comprobaciones de contenido de la relectura | acotado | **Confirmado**, y su mutante sobrevivía. Controles por rama, con la **misma** cuenta de páginas —página equivocada, copia de las condiciones, rótulo—, con mensajes separados por causa; y las paradas 2 y 3 exigen ahora el mensaje de **su** comprobación, porque con la defensa en profundidad nueva otra capa también las paraba y tapaba la suya. `9ec1ecf`, `d3850be` |

### Las dos observaciones laterales, también confirmadas

- **M-4 estaba protegida por cómo construye F1, no comprobada por resultado** (§3 del informe):
  un burofax con otro segundo documento se recortaba igual. Ahora cada certificado tiene que
  reproducir **exactamente** los documentos enviados —misma cuenta y, página a página, la copia de
  cada una que tenga texto—, o para. Los dos reales pasan. `21230a5`
- **`emisor.verificado: true` afirmaba de más** con `verificar_plaza=False` (§5 del informe): el
  manifiesto dice ahora `razon_social_verificada` y `usuario_verificado` por separado. `c9d4f62`

### Lo que la remediación acredita, y lo que NO

- Cada remedio se escribió con su test **visto en rojo antes** por la razón del hallazgo.
- **Mutantes: 26 de 26 muertos, cada uno por su test** —los nueve de antes que siguen existiendo,
  el del revisor en H-07 (una relectura que solo cuenta páginas) y dieciséis de la remediación,
  entre ellos quitar la poda, la relectura del grafo, la comprobación de la reproducción o la
  validación del manifiesto—. Arnés fuera del repo, con los dos módulos restaurados byte a byte.
- Sobre los **certificados reales, sin red**: el formulario de las condiciones ya no viaja, lo
  conservado no pierde un píxel, el parser estricto acepta las tres listas y la comprobación de la
  reproducción pasa. Suite entera con las dos semillas: ver el cierre.
- **La remediación NO ha pasado ronda.** Sobre los nueve commits la cobertura independiente es
  **AUSENTE**: cada remedio hace lo que dice, y eso no prueba que ninguno haya abierto algo que
  nadie fue a buscar. Una segunda ronda excede la tabla —que da una a F3— y es decisión de
  Nikolai.

**Lo que el revisor declaró sin verificar, contrastado:** reprodujo H-01 con `pypdf` 6.10.1; yo lo
reproduje con 6.12.2 —la del venv— y sobre los certificados reales. Las 18 mutaciones de la
ejecución original no estaban en su objeto; el arnés nuevo las sustituye con las de la remediación.

**La lección, de frontera y no de ejemplo:** verificar por resultado es releer **todo** lo que el
resultado contiene, no la parte que uno sabe mirar. El aportable tenía más superficie que su
texto, y el control que yo había escrito para no fiarme de la aritmética se fiaba de que lo
visible fuera todo lo que había.

## 13. Adjudicación de la revisión adversarial (Codex, 2026-09-25) — NO-SHIP, remediado

- **Objeto revisado:** la remediación de la R1, `a87e2d9` → `3ae4cbe` — 9 ficheros, +1.191 / −123 —, con el informe de la R1 y su adjudicación dentro
- **Ronda:** 2, sobre la remediación — una por encima de la tabla de `CLAUDE.md`, autorizada expresamente por Nikolai el 2026-09-25
- **Revisor:** Codex CLI `0.155.0-alpha.16.3`, `gpt-6-astra`·`medium` releídos del log; `service_tier="default"` afirmado desde el lanzador conservado
- **Informe recibido:** `2026-09-25-codicert-f3-r2-adversarial-review.md`, `sha256` del bloque literal `7fdb90acae94ccea32e33a5c718776b1e15aa8becafb170af1fc713ce638f827`
- **Hallazgos:** 7 — 1 `alta`, 5 `media`, 1 `baja` —, más el dictamen de los nueve remedios de la R1 (cinco reales, cuatro incompletos). **Los siete confirmados contra la fuente, ninguno refutado**
- **Remediado en:** los siete, `92f5536` (un commit: la imagen cambia el módulo puro y la orquestación a la vez, y un corte intermedio no estaría en verde); los tests de las ramas sin cubrir, `340e170`

**El alto no se remedia: se cambia la pieza (H-01).** La R1 encontró el formulario de las
condiciones viajando dentro del aportable, y la remedié podando recursos a lo que cada página
dibujaba con `Do`. La R2 enseñó siete vías más por las que el contenido de la página retirada
seguía dentro —patrones, sombreados, fuentes Type3, máscaras, máscaras de imagen, metadatos de un
formulario y un `/Contents` en array indirecto— y que la relectura aceptaba un stream reescrito
porque buscaba huellas conocidas. Es lo que el corolario de `CLAUDE.md` llama remediar el ejemplo:
dos rondas seguidas encontrando la misma propiedad mal cerrada. **La frontera es copiar
estructura**, y se cierra no copiándola: **el aportable es la IMAGEN de las páginas conservadas**
—una imagen JPEG por página, a 200 ppp, dibujada sin anotaciones—, con una **capa de texto
invisible leída por OCR** para que se pueda buscar y lo lea una LLM, que es lo que Nikolai pidió
al proponérsele la imagen. Lo que ninguna página conservada dibuja no está, venga por la vía que
venga; y el OCR solo ve esos píxeles.

### El alto

| # | Hallazgo | Coste | Adjudicación y remedio |
|---|---|---|---|
| **H-01** | La poda y la relectura omiten contenido de recursos y aceptan streams reescritos | estructural | **Confirmado**: las 20 sondas del revisor reproducen el defecto sobre `3ae4cbe` con el `pypdf` del venv (6.12.2), no solo con el 6.10.1 del revisor. En la fuente: `_nombres_invocados` solo miraba `Do`, `_podar` solo filtraba el `/XObject` de la página y no bajaba a los formularios, `_flujos_usados` no expandía un `/Contents` en array indirecto, y `_verificar_grafo` comparaba contra una **lista negra** de huellas. **Remedio estructural** (arriba): `_rasterizar` + un puerto `ocr` (`aportable_de`); y la relectura (`verificar_aportable`) pasa a **lista blanca** —el «perfil admitido» que pedía el revisor—: cada página lleva su imagen, **byte a byte la del raster**, y como mucho una capa de texto invisible (modo 3, operadores de texto, sin fuentes Type3); catálogo, árbol y metadatos mínimos; **ningún objeto del fichero suelto**; y lo que el OCR lee no puede llevar el rótulo ni parecerse más a una página retirada —o a su vecina— que a la suya. Las siete sondas quedan como tests de regresión; la de la reescritura, como un control de la relectura. |

### Los cinco medios y el bajo

| # | Hallazgo | Coste | Adjudicación y remedio |
|---|---|---|---|
| H-02 | La unicidad del bloque FICHEROS se contaba por páginas | acotado | **Confirmado** (dos bloques en una página: mandaba el primero). Se cuentan **bloques**. |
| H-03 | Citar el título en prosa retira el requerimiento como condiciones | acotado | **Confirmado** (la sonda retiraba 3, 4 y 5). El rótulo **abre** las condiciones solo como **título** —abriendo línea, aunque venga partido—; una **mención** en prosa no las abre, y si cae en una página que se conserva la ve la red y **para**, diciendo que es una mención. |
| H-04 | Una caída del CRM invalida un manifiesto y un aportable idénticos | acotado | **Confirmado** (segunda corrida con el CRM caído: `PARADO`). Lo que se sabe del destinatario va a su propia clave, **volátil**: no es identidad del aportable. Solo obliga a revisar **si el manifiesto calla y hoy la comprobación avisa**; si hoy no se pudo comprobar, o el manifiesto ya avisaba, vale el manifiesto y se cuenta la diferencia. |
| H-05 | M-4 no comparaba las páginas con menos de 40 caracteres | acotado | **Confirmado** («Recibo 40 EUR» sustituido por una quita pasaba). Por debajo del mínimo se compara **letra a letra**; una página sin texto exige una copia sin texto, y se **avisa** de que solo la acredita la cuenta. |
| H-06 | Una coincidencia íntegra callaba una composición igual de posible | acotado | **Confirmado**. Si el nombre casa con una parte **y** se lee como varias, aviso propio: `AVISO_DESTINATARIO_AMBIGUO`. |
| H-07 (`baja`) | La cobertura repetía subproblemas: 250.904 llamadas para 18 segmentos | acotado | **Confirmado**. Programación dinámica de atrás adelante, cada posición una vez; el test cuenta trabajo, como el revisor, y no milisegundos. |

### Lo que encontró CORRER el código y no la revisión

Sobre los **certificados reales**, sin red, el OCR de verdad destapó dos defectos que el
sintético no tenía cómo enseñar, los dos con su test lento visto en rojo antes del arreglo:

- **Por encima de 1 MB, OCRmyPDF linealiza la salida**, y la linealización mete un diccionario
  `/Linearized` y un stream de pistas que no cuelgan de nada. La relectura los paró —con razón—
  sobre el aportable real de 4,7 MB y no sobre el sintético de 1 MB. Se apaga (`fast_web_view`).
- **`ocrmypdf.ocr()` deja `sys.stderr` sustituido por un `StringIO`.** Desde ahí se perdían en
  silencio los avisos del frontal y la traza de cualquier fallo: el proceso salía con 1 sin decir
  por qué, y así se me presentó el primer intento. El adaptador lo devuelve como estaba.

### Lo que la remediación acredita, y lo que NO

- Cada remedio con su test **visto en rojo antes** por la razón del hallazgo; los de la capa de
  texto, con un **OCR de mentira que lee la imagen** —casa cada página del raster por sus píxeles
  con la del certificado que dibuja—, porque uno que devolviera el texto de lo que el código cree
  haber conservado aprobaría justo un error de índices.
- **Sobre los certificados reales, sin red** (`006catetonk` y `006catfpdv6`): **ni un stream del
  íntegro sobrevive en el aportable** (0 por huella de datos decodificados), el formulario con el
  rótulo pasa de 1 a 0, cada imagen es su página (distancia 0,33-1,96 contra ≥ 5,98 de la más
  cercana), el OCR lee lo conservado (0,54-0,95 con su original) y no lee el rótulo, y releer el
  aportable ya escrito con un recorte nuevo lo admite. 3,8 y 4,8 MB; 12-23 s de OCR.
- **Mutantes: 75 de 75 muertos, cada uno por SU test** —catorce de antes que siguen existiendo,
  veintisiete del módulo puro nuevo (el raster, la relectura rama a rama, lo que lee el OCR, H-02,
  H-03 y H-05), catorce de la orquestación (H-04, H-06, H-07, la idempotencia, el puerto), los
  dos del adaptador, que solo mata el test lento, y **dieciocho más** sobre las ramas que el aviso
  de cobertura de `session_close` dio por no ejecutadas (89 %): siete ramas de la lista blanca
  sin test propio, el bloque FICHEROS sin cabecera o vacío, el fallo de escritura, el íntegro
  ilegible, el código de error del OCR…; para cada una se escribió el test y luego su mutante—.
  Arnés fuera del repo, que corre solo los tests
  declarados de cada mutante y deja copia de los originales. **Dos tropiezos del arnés, dichos:**
  un mutante mío dejaba un `try:` vacío —no compilaba, y el arnés lo contaba como superviviente—, y
  al parar una primera corrida demasiado lenta quedó un mutante dentro del fichero, que se detectó
  comprobando que el texto original de los 57 aparece exactamente una vez, y se revirtió.
- **Suite entera con las dos semillas: 6.328 casos, 6.233 verdes y 95 omitidos, idénticos con la
  777 y la 31337, 0 fallos**; y `session_close --runslow` pasó las dos con los lentos dentro, el del
  OCR real incluido (vuelto a correr sobre el estado final). Frente a los 6.257 de `3ae4cbe`,
  **+71: entran 76 casos, todos de F3, y salen cinco, dichos** —los tres de la relectura del grafo
  (`_podar` y `_verificar_grafo` ya no existen; su propiedad la prueban los tests del perfil), el
  de la copia de las condiciones (renombrado: ahora la ve lo que lee el OCR) y el del manifiesto
  huérfano que se reponía (ahora para)—. Ninguno cambia de estado. El omitido de más es el test del
  OCR real, que solo corre con `--runslow` (`MEJORAS #292`).
- **La remediación de la R2 NO ha pasado ronda: su cobertura independiente es AUSENTE.** Y aquí
  pesa más que en la R1, porque no es un remedio sobre la pieza sino **otra pieza**: la imagen, el
  perfil y el puerto de OCR son código nuevo que nadie ha atacado. Una R3 supera el techo de dos
  rondas y solo la puede autorizar Nikolai.

**Lo que cambia para quien lo usa, dicho:** el aportable es una **imagen** —el texto fiel es el
del íntegro, y el de la capa lo ha leído un OCR que puede equivocarse (lo dicen el manifiesto y el
frontal)—; pesa unos 545 KB por página; necesita Tesseract en el PC y tarda entre 10 y 40 s por
certificado; un manifiesto sin su aportable **para** (el aportable no se puede reponer idéntico:
el OCR no da dos veces los mismos bytes); y volver a lanzar relee lo escrito contra la imagen de
hoy sin volver a pasar el OCR, así que un cambio de versión del render haría **parar** los
aportables ya escritos en vez de pisarlos.

**Lo que el revisor declaró sin verificar, contrastado:** las sondas, con `pypdf` 6.12.2 (arriba);
los 26 mutantes de la R1 no estaban en su objeto, y el arnés nuevo los sustituye; el `/ObjStm` con
xref comprimido lo ejerce ahora la salida real de OCRmyPDF, que los usa y pasa la relectura. **Sigue
sin verificar**, como él dijo: qué tipos de adjunto no PDF reproduce el prestador.

## 14. Adjudicación de la revisión adversarial (Codex, 2026-09-25) — NO-SHIP, remediado

- **Objeto revisado:** la remediación de la R2, `3ae4cbe` → `6d950b1` — 14 ficheros, +2.668 / −476 —, con los informes de la R1 y la R2 y sus adjudicaciones dentro
- **Ronda:** 3, sobre la remediación de la R2 — dos por encima de la tabla de `CLAUDE.md`, autorizada expresamente por Nikolai el 2026-09-25 («r3»)
- **Revisor:** Codex CLI `0.155.0-alpha.16.3`, `gpt-6-astra`·`medium` releídos del log; `service_tier="default"` afirmado desde el lanzador conservado. Un primer intento murió por el filtro de contenido y no cubre nada (acta)
- **Informe recibido:** `2026-09-25-codicert-f3-r3-adversarial-review.md`, `sha256` del bloque literal `bd46f2204108a2f678c3fc9af86a07f5daa10f904e8bc94ed84414368ae1767d`
- **Hallazgos:** 5 — 2 `alta`, 3 `media`, 0 `baja` —, más el dictamen de los siete remedios de la R2 (cinco reales, dos incompletos: H-01 y H-03). **Los cinco confirmados contra la fuente, ninguno refutado**: las 34 sondas del revisor reproducen sobre una copia limpia de `6d950b1`
- **Remediado en:** los cinco, `1754e68` (un commit: el puerto del OCR, la composición y la relectura cambian el módulo puro y la orquestación a la vez)

**Los cuatro primeros son UNA propiedad, y es la de la R2 otra vez (H-01 a H-04).** La R2 cambió
la pieza —la imagen— pero dejó que el PDF lo escribiera OCRmyPDF, y la relectura intentó
**enumerar** lo que un fichero ajeno puede llevar dentro. Un fichero tiene más sitios de los que
una lista enumera: revisiones anteriores, objetos de otra generación, datos auxiliares de una
fuente, comentarios, una segunda capa que nadie invoca; y una página puede guardar su imagen sin
dibujarla. Es la pregunta del corolario de `CLAUDE.md` —¿de qué frontera es esto un ejemplo?—, y
la frontera es **admitir un fichero que no ha escrito el motor**. Se cierra quitándole al tercero
la escritura: **el OCR devuelve líneas —texto y caja— y nada más, y el aportable lo compone el
motor, byte a byte, en una forma fija** (`_componer`): catálogo, árbol, Helvetica estándar y, por
página, su imagen a página completa y un renglón invisible por línea leída. **La relectura ya no
admite: recompone** el fichero con la imagen del recorte y los renglones que lee de él, y exige
**los mismos bytes**. Lo que no está en la forma no cabe en los mismos bytes, y no hay nada que
enumerar.
...
| # | Hallazgo | Coste | Adjudicación y remedio |
|---|---|---|---|
| **H-01** | El inventario no cubre el fichero físico ni todas las generaciones | estructural | **Confirmado** (sondas: historial incremental con la marca en la revisión anterior, un objeto de generación 1, `/ObjStm` y `/XRef` sueltos). En la fuente: `_sin_sueltos` recorría solo la generación 0 y callaba los errores, y admitía cualquier stream por su etiqueta. **Remedio: la composición canónica y la recomposición byte a byte** (arriba). Las sondas de bytes —que no reescriben nada: dos actualizaciones incrementales, un objeto de generación 1, cola tras el `%%EOF`, una actualización que sustituye el contenido por otro de la forma— quedan como regresiones. |
| **H-02** | La lista blanca admite subgrafos, cadenas y streams que nunca enumera | estructural | **Confirmado** (nueve vías —`/Info`, XMP, clave auxiliar de la fuente, `FontFile2`, dependencia del contenido, `/ProcSet`, `BDC` con `/ActualText`, comentario, ciclo— y la segunda capa sin invocar). En la fuente: `_admitir` daba por bueno todo lo que colgara de algo admitido. **Mismo remedio que H-01.** Las 24 salidas con que se probaban las ramas de la lista blanca de la R2, una de control —una reescritura con pypdf sin cambios, que ya no es el aportable— y las 21 de esta ronda: 46 casos de un test parametrizado, cada uno con lo que la relectura tiene que decir. |

### Los tres medios

| # | Hallazgo | Coste | Adjudicación y remedio |
|---|---|---|---|
| H-03 | Una imagen correcta en recursos no acredita que la página la dibuje | acotado | **Confirmado** (cuatro dibujos distintos aceptados, y el mutante del revisor —guardar la imagen sin dibujarla— **sobrevivía a los 242 tests rápidos**). **Remedio en dos capas:** el contenido tiene **una sola forma** —la imagen a página completa, `BT 3 Tr`, los renglones y `ET`—, así que ningún otro dibujo cabe; y la relectura **dibuja el fichero** con PDFium, a su tamaño exacto, y exige que cada página pinte su imagen tal cual (miniatura, diferencia máxima ≤ 8 niveles: medido 0, y sin dibujar ≥ 85, M-15). Un test dibuja el aportable con un render propio; el mutante del revisor está en el arnés, en la plantilla **y** en la forma a la vez, y lo mata el dibujo. |
| H-04 | La relectura admite ausencia de OCR y texto no acreditado por la imagen | acotado / estructural | **Confirmado** (sin capa, una capa solo con la marca, la marca añadida al texto bueno, una página lejana). **Remedio:** (a) cada página cuyo original tiene texto (≥ 40 caracteres) trae el suyo, con un parecido de **0,30** como poco (M-13: la propia, 0,56 como poco; cualquier otra, 0,31 como mucho); (b) **qué página es cada imagen lo dice la IMAGEN**, contra las miniaturas de **todas** las del íntegro: ninguna otra más cerca que la suya, ninguna retirada ni empatada, y un techo de cordura; (c) el puerto solo devuelve líneas, que es acotar la salida del OCR. Desaparece el falso rechazo por empate que el revisor señaló (§2): el texto ya no discrimina páginas, así que dos plantillas casi iguales que el OCR no distingue no paran. **Lo que NO se remedia, y se dice:** que cada palabra salga de los píxeles —la marca añadida al texto bueno sigue pasando—. Eso descansa en que el OCR solo recibe esos píxeles; la relectura acredita parecido y ausencia del rótulo, no procedencia, que es lo que el revisor pidió no anunciar. |
| H-05 | Una mención partida por la extracción vuelve a abrir las condiciones | acotado | **Confirmado** (la sonda retiraba 3, 4 y 5). Un título no lleva prosa delante **ni detrás**: la frase abre una línea y **en esa línea no sigue nada en minúscula** —«ECONÓMICAS DE PAGO» vale, «y se adjunta» no—; los dos rótulos reales van solos en su línea (M-14). Y una mención **antes** del título para ya en `paginas_de_condiciones`, con el nombre de la página: como los documentos son los de todos los envíos, en la orquestación para la expedición entera (`ExpedicionError`). Lo que sigue al título sale con las condiciones, mencione lo que mencione. |

### Las observaciones del §2, contrastadas

- **El escaneo** (con el adaptador real): una página conservada escaneada que reproduce las
  condiciones sin su rótulo pasaba. **Confirmado**, y el revisor lo acota bien: no contradice «lo
  que no se dibuja no está», pero sí que eso equivalga a «no lleva contenido de condiciones».
  **Aviso nuevo** sobre lo que el OCR lee: las frases de 8 palabras de las condiciones que la
  imagen muestra y el texto de la página no lleva —lo que el texto ya repite lo avisa el recorte—.
  Aviso y no parada, como el del §7.4; con el OCR de verdad en un test lento y con el doble en uno
  rápido, y viaja al manifiesto.
- **Lo que pypdf extrae y PDFium no dibuja** (capa opcional apagada, transparencia, fuera de la
  caja, modo 3): **confirmado**; no es fuga —desaparece de los píxeles— y puede dar paradas
  conservadoras en la clasificación. Sin cambio.
- **La compatibilidad con otras versiones de OCRmyPDF** deja de aplicar: OCRmyPDF sale del camino
  del aportable. Lo que sí depende de versión, dicho: **Tesseract** —otra versión puede leer otras
  líneas, y entonces un manifiesto huérfano no se repone: para— y **PDFium/Pillow** —otros bytes de
  imagen, y un aportable ya escrito para en vez de pisarse—.
- **La regresión de recuperación de la R2** (un manifiesto sin su aportable paraba siempre) **se
  revierte**: el mismo OCR sobre la misma imagen da el mismo fichero (M-16: dos corridas, los dos
  certificados reales), así que se repone si sale con la huella del manifiesto, y si no, para.

### Mediciones de la remediación (2026-09-25)

Las dos primeras las citaba el código desde la R2 sin que estuvieran aquí; se asientan.

- **M-11.** A 200 ppp las huellas del acta, en cuerpo pequeño, se leen a ojo, y el OCR lee lo mismo
  que a 150 (similitud con el original, 0,61-0,95 contra 0,66-0,95); ~545 KB por página contra ~362.
- **M-12.** Sobre los dos reales, lo que el OCR lee se parece a su original 0,54-0,95 y a cualquier
  otra página 0,29 como mucho; el rótulo, solo en las dos de condiciones.
- **M-13.** Tesseract `5.4.0.20240606`, directo, salida TSV: la misma imagen da **la misma salida**
  en las 18 páginas reales; parecido con la propia 0,56-0,95 y con otra ≤ 0,31; el rótulo, solo en
  la 9 de `006catfpdv6` y la 7 de `006catetonk`; miniaturas, la propia a 0,10-0,52 y la más cercana
  de las demás a ≥ 4,22; de 1,4 a 10,9 s por página.
- **M-14.** En los dos adjuntos reales de condiciones, el rótulo va **solo en su línea**: nada
  delante ni detrás. La regla nueva del título los reconoce.
- **M-15.** Lo que el PDF compuesto **dibuja** coincide con su imagen: diferencia máxima de
  miniatura **0** en los sintéticos y en los dos reales; una página que no la dibuja, **≥ 85**. Pero
  solo a su tamaño EXACTO, y ahí salieron dos cosas: **pypdfium2 redondea con `ceil(puntos ×
  escala)`**, y sin margen, a 200 ppp, la página salía un píxel más ancha y redibujada; y **PDFium
  da los puntos en float32** —421,2 pt salen 421,20001220703125—, así que con un margen de 10⁻⁹ una
  imagen de 1.170 px salía de 1.171. Lo destapó el test del techo, a otra resolución; con los A4
  reales no se veía por suerte de las medidas. Margen de 10⁻⁵: mayor que el error de float32, menor
  que un píxel.
- **M-16.** La tubería entera sobre los dos reales, sin red: **ni un stream del íntegro** en el
  aportable, el formulario con el rótulo **1 → 0**, lo que dibuja cada página a 0,33-1,96 de la suya
  contra ≥ 5,98 de la más cercana (a resolución completa), el OCR a 0,56-0,95 de cada original y sin
  el rótulo, **un segundo OCR da el mismo fichero**, y releerlo con un recorte nuevo da los mismos
  avisos. 3,8 y 4,8 MB; recorte de 12 y 35 s, y de 2,6 a 6,1 s por página entre OCR, composición y
  relectura. Repetido con el código final —el margen de M-15 cambió después—: los mismos
  resultados; sus tiempos no valen, porque la suite corría a la vez.

### Lo que la remediación acredita, y lo que NO

- Cada remedio con su test **visto en rojo antes** por la razón del hallazgo, y las sondas del
  revisor portadas a la suite como regresiones: 46 salidas fuera de la forma que la relectura tiene
  que parar, cada una con su mensaje. Y las 34 sondas literales, adaptadas solo en la API, contra el
  remedio: el balance, en la §2 del acta.
- **Mutantes: 76 de 76 muertos, cada uno por SU test** —treinta y dos de antes que siguen existiendo
  y cuarenta y cuatro de la remediación: la composición, la recomposición, el dibujo, la lectura, el título,
  la orquestación y el adaptador de Tesseract—, entre ellos el del revisor (la página que guarda su
  imagen y no la dibuja), que en la R2 sobrevivía a los 242 tests rápidos: ahora lo mata dibujar el
  fichero. Dos son dobles —la plantilla del contenido **y** su forma a la vez—, porque la propiedad
  está dicha en los dos sitios y un mutante de uno solo lo para el otro. Arnés fuera del repo, con
  copia de los originales y los dos módulos restaurados byte a byte, comprobados por `sha256`.
- **Suite entera con las dos semillas: 6.378 casos, 6.282 verdes y 96 omitidos, idénticos con la 777
  y la 31337, 0 fallos**; y `session_close --runslow` las pasó con los lentos dentro —los dos del
  OCR real incluidos—, con la cobertura de las líneas nuevas al **93 %**. Frente a los 6.328 de
  `6d950b1`, **+50: entran 81 casos y salen 31, todos de F3, y ninguno cambia de estado** —los 26
  que probaban ramas de la lista blanca pasan a ser casos del test parametrizado de la forma
  canónica (24 con su equivalente directo; los dos de las claves y los recursos de la capa de
  OCRmyPDF no lo tienen, porque esa capa ya no existe, y los cubren la segunda capa y los datos de
  la fuente); dos se renombran con su comportamiento (la relectura que acepta el limpio, y la cita
  del título, que ahora para ya en `paginas_de_condiciones`); los dos del adaptador de OCRmyPDF se
  sustituyen por los de Tesseract; y el del manifiesto huérfano que paraba se invierte—. El omitido
  de más es el segundo test lento, el del escaneo con el OCR real (`MEJORAS #292`, fila 41).
- **La remediación de la R3 NO ha pasado ronda: su cobertura independiente es AUSENTE.** Es, otra
  vez, una pieza nueva —la composición y la recomposición— y no un parche. Una cuarta ronda solo la
  puede autorizar Nikolai.

**Lo que cambia para quien lo usa, dicho:** el aportable lo compone FeesDefender y el OCR es
**Tesseract directo** (OCRmyPDF ya no interviene); el manifiesto pasa a la **versión 3**; un
manifiesto sin su aportable **se repone** si el aportable sale con su huella; una mención del
rótulo antes de su título **para la expedición entera**; hay un **aviso nuevo** cuando la imagen de
una página muestra frases de las condiciones que su texto no lleva; y tarda de 12 a 35 s en el
recorte más de 2,6 a 6,1 s por página. **La capa sitúa cada renglón en su sitio y a su altura, pero
no lo estira a su anchura**: para buscar y para que lo lea una LLM basta, y al seleccionar en un
visor el resaltado puede no coincidir con el renglón de la imagen.
...
- **No entrega un aportable sin su capa de texto**: si el OCR falla o no está, para (R2).
- **No acredita que cada palabra de la capa de texto salga de los píxeles**: acredita que se parece a la página y no lleva el rótulo (R3/H-04).
