...
- **Remediado en:** este mismo PR, sobre `93c1907`: `core/crm_ficha.py` y `scripts/crm_ficha.py` con sus tests (`cc5a241`), los dos arneses, el spec rev. 4 y `MEJORAS #313`/`#314`. **Ninguna ronda revisa el remedio**: esta era la última autorizada
...
- **H-02 (`media` · `acotado`).** Un NIF declarado que se queda en nada sin sus separadores
  ...
  **Remedio:** se rechaza al validar, en los dos roles y aunque haya email o `id_crm`, y la identidad
  ...
  `MEJORAS #314`, con su propia regresión.
