...
| H5b | Cerrado | 2026-05-12 | **Sin commit — vive en el caso, ignorado por git.** Sub-hilo abierto durante H6 por insuficiencia detectada en sanity de PII previo a exposición al frontier: 37 hits residuales (4 case_id en frontmatter, 3 "Pedro San Martín" FP intencional aceptable, 30 FN reales — 11 variantes "Castelar 37-39" con formatos OCR diversos, 6 variantes muy degradadas del cliente "Engel & Völkers", 6 emails despacho actor con `@` corrompido como E/O/G/C, 2 URLs corporativas no marcadas en H5, 1 "Adelaida Peñil" parcial, 2 propietarias nuevas no detectadas — "Adelaida Gómez Sainz" y "Mercedes Pita Wonenburger" — que comparten primer nombre con personajes etiquetados). Script `07_AI cowork/_h5b_sars1_cobertura_completa.py` (no versionado): backup `.bak.h5b` + ampliación mapa (45 etiquetas, +5 entidades incluyendo categoría URL nueva `[URL]`=delriomiera.es y `[URL_2]`=engelvoelkers.com) + 16 reglas FN_RULES_H5B operando solo sobre body (frontmatter conservado para H7) + regeneración de `08_Para frontier/` con frontmatter neutralizado (case_id literal queda fuera del LLM externo). 35 sustituciones FN automáticas + 1 parche puntual línea 708 DEM (regla H5b no contemplaba cierre Markdown `**` entre separador `£:` y "VÓLKERS"). Sanity final: 0 hits PII residuales (excluyendo "Pedro San Martín" FP intencional). 169 etiquetas totales en `08_Para frontier/` (+68 vs post-H5). Nueva entrada §23 en `docs/MEJORAS_FUTURAS.md`: el motor expone `case_id` literal en frontmatter; mitigación a futuro = anonimizar case_id en `api.py` o modo "para frontier" explícito. Nueva entrada en `docs/DEAD_ENDS.md`: PowerShell `Add-Content/Get-Content -Raw` sin `-Encoding` produce double-encoding. |
| H6 | En curso | 2026-05-12 (pasos 6.1 y 6.2 cerrados; 6.3 pendiente sesión nueva) | **Sin commit pendiente.** Paso 6.1 cerrado: 4 piezas split subidas al gdocu del expediente judicial 659 vía SPA `sudespacho.net`, todas en rama raíz `General/` (decisión usuario opción b: solo piezas split, sin OCR completos ni originales sin OCR — descarte de duplicados). Caveat OCR pp 1-20 del DOC_ANEXO se incluye tal cual en los .md anonimizados con advertencia explícita al frontier (opción a). Documentos pre-existentes en gdocu (2): `ESCR PROCU-PERSONAMIENTO.pdf` + `JUSTIF APUD-ACTA.PDF` (personación procesal previa). Paso 6.2 cerrado: prompt frontier redactado en `07_AI cowork/_prompt_frontier_H6.md` con contexto del caso (E&V Spain demandada en juicio ordinario LPH 249.1.8 sobre instalación no autorizada de aires acondicionados en patio comunitario, cuantía 18.000 €) + 4 .md adjuntos + caveat OCR + estructura procesal Sala 1ª TS + reglas anti-alucinación con placeholders explícitos. **Reorganización del expediente** durante H6: `08_Borradores/` renombrada a `09_Borradores/` (output frontier + deanonimizados H7); nueva `08_Para frontier/` como drop zone canónica de input al LLM externo (4 .md sin frontmatter del motor + `_PROMPT.md` + `README.md` con contrato de la carpeta). Por decisión del plan §9.3 ninguna de las dos se cabla en `core/config.py::INPUT_SUBDIRS` (flujo borrador-iterativo no estabilizado). Paso 6.3 pendiente: entrega manual al frontier + recepción borrador → guardar como `09_Borradores/contestacion_demanda_SaRS1_v1_anonimizado.md`. Operativa en sesión nueva (cuenta personal Claude.ai web o app/Cowork con perfil distinto del repo FeesDefender, acceso de carpeta solo a `08_Para frontier/`). |
| H7 | Pendiente | | Pre-condición: H6 paso 6.3 cerrado. La carpeta destino es ahora `09_Borradores/` (renombrada desde `08_Borradores/` durante H6/H5b por inclusión de la nueva `08_Para frontier/`). |
