...
> **Versión:** v3 (2026-06-25)
> **Estado:** Esquema del evento probatorio COMPLETO (Fases 0, 1 y 2 cerradas). **Fase 3 (correlación) COMPLETA (F3.D1–D5 cerradas).** Fases 4–7 pendientes.
...
**Opcionales (sobre todo analíticos):** `autor`+`fecha` · `justificacion` (OBLIGATORIA en contradice y en apoyos a presunción) · `estado_revision` (pendiente/confirmado/descartado) · `matiz_contradiccion` (de_contenido/de_credibilidad/de_autenticidad) · **`riesgo_tergiversacion`** (bool; reenvío descontextualizado o cita parcial — añadido en F3.D2) · **log de revisiones lean** {autor·fecha·motivo·estado_anterior→nuevo} (añadido en F3.D3).
...
- **Punto controvertido** (agrupa versiones rivales; la rivalidad va por enlace; nodo donde se computa el tope por rival seria — ver §8.5).
...
- **Plano de ACTOS:** Colapso (intra-fuente, mismo registro → un evento, suma de procedencias) · Correlación (inter-fuente, acto distinto, mismo hecho → enlace, nunca fusión) · Reconstrucción (acto sin registro directo, solo referido → evento reconstruido baja confianza) · Sin relación.
...
**Dos corroboraciones distintas:** de **CONTENIDO** (por orígenes independientes) y de **CIRCULACIÓN** (por canales). No se suman igual (ver §8.4 y §8.6).
...
- **S3 — Actores resueltos coincidentes:** rareza = frecuencia + **lista a priori de actores omnipresentes** (peso bajo fijo) + **co-ocurrencia rara**. La diagnosticidad condicional a hipótesis → §8.4.
- **S4 — Proximidad temporal en ventana:** nunca empareja sola; sobre todo descarta.
...
**★ Invariante de no-fuga:** la fuerza probatoria nunca depende del score de matching; el cómputo del semáforo (§8) usa solo enlaces `confirmado`.
...
**Rank de cola = ordinal interno INTERMEDIO** (el número **nunca se muestra**; la UI muestra "Tier X · posición N"). Amurallado: no decide ruta, no es magnitud, no entra en §8.
...
6. **Topes (degradan):** rival **seriamente sostenida** → 🟡 (ver §8.5); socava cualificado (§8.5); presunción 386 ≤ hecho base + nexo preciso y directo; credibilidad/fiabilidad bajas.
...
- **Tres dianas según `matiz_contradiccion`** (proyección múltiple: diana principal + efectos secundarios):
  - **De contenido** → **versiones rivales (N, paramétricas, no solo ¬X**: 21,3M vs 19M vs 20M) bajo un **punto controvertido**; cada versión computa su estatus por §8.4; se reduce al mecanismo de rival ACH. En hecho **compuesto**, el tope afecta solo al **componente discutido**.
...
- **Hecho (diseño):** Fase 0 (inventario), Fase 1 (esquema del evento — D1–D4, D6), Fase 2 (identidades — D5), **Fase 3 (correlación — F3.D1 a F3.D5).**
- **Pendiente (diseño):** Fase 4 (tiempo heterogéneo: EDTF, fecha-hecho vs fecha-declaración vs fecha-registro, orden de eventos difusos), Fase 5 (arquitectura de ingesta: un atomizador por fuente), Fase 6 (vistas y entregable humano + custodia), Fase 7 (alcance piloto: correo + WhatsApp).
- **Build:** en `core/` de FeesDefender, por Claude Code, incremental (correo + WhatsApp primero), tras cerrar el diseño y con el motor de correo terminado. Materiales build-ready ya disponibles: pseudocódigo de F3.D3/D4/D5 y tablas de decisión en los handoffs de stress-test.
