...
## ⚠️ Faltan dos handoffs (build-ready de la Fase 3)

El spec cita, para el cómputo del semáforo y la contradicción, dos handoffs que **NO están
en el repo** (no se aportaron):

- `handoff_F3D4_stresstest_claude.md` — pseudocódigo `calcular_estatus_soporte` + tabla
  "estructura del soporte → rango de estatus" (§7.4).
- `handoff_F3D5_stresstest_claude.md` — mini-pseudocódigo `procesar_contradiccion` (§7.5).

Hasta que se aporten, **el pseudocódigo build-ready del 🟢🟡🔴 y de la contradicción no está
en el repo**; el build de esas piezas tendrá que reconstruirlo desde las reglas del §7.4/§7.5
o esperar a los handoffs.
