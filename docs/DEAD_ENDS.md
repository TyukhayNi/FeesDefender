...
- **Reconfirmado 2026-07-27 (PR #138) — cómo se ENTRA en este estado a mitad de sesión: `git worktree remove` se completa A MEDIAS.** Lanzado desde la raíz (`git -C <raíz> worktree remove <ruta>`) sobre el worktree que es *cwd* de la sesión viva, el comando **desregistra el worktree y LUEGO falla** al borrar el directorio (`error: failed to delete '<ruta>': Permission denied`). Queda un estado intermedio que la redacción anterior de esta entrada no cubría: el directorio existe, ya **no** sale en `git worktree list` (ni como *prunable*), y **desde ese momento todo comando git de la sesión opera sobre la raíz compartida sin avisar** — el riesgo real es commitear en `main` de la raíz creyendo estar en tu rama (ya pasó una vez, ver bitácora del 38º cierre). **Señal inequívoca y barata:** `git status -sb` pasa a decir `## main...origin/main`. **Qué hacer al verla:** dar por perdido el worktree y usar **`git -C <ruta>` explícito** para todo lo que quede de sesión; la rama local, al quedar desregistrada, ya se puede podar con `git branch -D` (tras verificar `git diff --stat origin/main <rama>` vacío).
  - **Matiz que corrige el paréntesis de la Solución:** mientras la sesión dueña siga viva, el directorio **tampoco** lo borra otra sesión ni la terminal de Nikolai — el bloqueo lo tiene *ese proceso*, no depende de desde dónde se lance el borrado. Y **`Set-Location`/`cd` a la raíz no lo libera**: el harness **devuelve el *cwd* a la carpeta del worktree** tras cada comando (`Shell cwd was reset to <ruta>`). Único camino: cerrar la sesión y borrar después.
- **Reconfirmado 2026-07-30 (PR #173) — no es solo un accidente de media sesión: es el resultado ESPERADO de seguir `FLUJO_GIT.md §4` al pie de la letra.** La Fase 4 del cierre manda `git worktree remove <ruta>` «desde la raíz, nunca desde dentro», y eso **no basta**: si la sesión que cierra es la que vive en ese worktree, el paso local nunca puede completarse, por muy correcto que sea el sitio desde el que se lance. O sea que **todo cierre de una sesión-en-worktree termina en este estado a medias**, y el resto de la Fase 4 (rama remota, `git branch -D` verificando por contenido, raíz a `main`) sí se completa con normalidad. No es señal de que algo haya ido mal.
  - **Dos hechos que abaratan el diagnóstico:** (1) la carcasa que queda está **vacía** — git borra todo el contenido y solo falla con el directorio en sí, así que **no hay nada que rescatar** y basta un `rmdir` (no hace falta `rm -rf`); compruébalo con `ls -a <ruta>` (solo `.` y `..`). (2) El fallo **no se ve en el código de salida** si hay tubería: `git … worktree remove … | cat` deja `$?` en **0** mientras el `error: failed to delete` va por *stderr* — el mismo modo de engaño que la regla de `$LASTEXITCODE` de `CLAUDE.md`, aquí en Bash y con `| cat`. **Verifica por estado, no por código:** `test -d <ruta>` y `git worktree list`.
- **Reconfirmado 2026-09-10 — la variante ESPEJO, que la redacción anterior no cubría: no solo puedes commitear en la raíz creyendo estar en tu rama; puedes SACAR LA RAÍZ DE `main`.** Un `git switch -c <rama-nueva>` lanzado desde la carcasa **crea la rama y mueve la raíz compartida a ella**, en silencio y con código 0. La raíz dejó de estar en `main` —lo que `FLUJO_GIT.md §5` prohíbe— sin que ninguna orden lo pidiera y sin tocar un solo fichero, así que `git status` sale limpio y no hay nada que delate el cambio salvo `git status -sb` / `git branch --show-current`. **Cómo se descubrió:** un `FileNotFoundError` sobre `docs/MEJORAS_FUTURAS.md`, una ruta del repo que obviamente existe — ésa es la señal de que el *cwd* es la carcasa vacía. **Remedio, en este orden:** `git -C <raíz> switch main` (primero, porque la raíz la comparten otras sesiones), comprobar `git -C <raíz> status --short` para descartar cambios pendientes, y montar un worktree nuevo con `git -C <raíz> worktree add <ruta-nueva> <rama>` para seguir trabajando. En el caso medido no hubo daño: la raíz nunca tuvo modificaciones sin commitear y su árbol coincidía con `main`.
...
### El `.dxt` empaqueta una COPIA del código; con `python -m` el cwd gana sobre el `PYTHONPATH` del manifest
- **Intentado:** tras arreglar un bug en `expedientes_xl/oracle.py` (LazyOracle), mergearlo a `main` y `git pull` en la raíz, asumir que la **extensión `.dxt`** ya corría el código nuevo — su manifest arranca `python -m expedientes_xl.server` con `env.PYTHONPATH=…\FeesDefender\plugins` (el repo).
- **Resultado:** la extensión seguía corriendo el `oracle.py` **VIEJO**. El `.dxt` **empaqueta una copia** de `expedientes_xl/*.py` y Claude Desktop la descomprime en `%APPDATA%\Claude\Claude Extensions\local.dxt.…\`. Con `python -m`, `sys.path[0]` = **cwd (la carpeta de la extensión, con la copia empaquetada)**, que gana sobre el `PYTHONPATH` al repo → se importa el paquete empaquetado (viejo), no el del repo. Confirmado 2026-07-20 (`oracle.py` de la extensión sin `LazyOracle` pese al pull en la raíz).
- **Solución:** para llevar un cambio de código a la extensión hay que **rebuild del `.dxt`** (re-empaquetar `expedientes_xl/*.py` actual + bump `manifest.version`) y **reinstalar** (borrar la vieja por la papelera del panel + importar el nuevo). Procedimiento en `plugins/expedientes_xl/dxt-build/README.md`. El `PYTHONPATH` al repo del manifest es un fallback frágil, no la fuente real del código en runtime. (El bundle de **Claude Code** es el mismo caso: copia cacheada en `~/.claude/plugins/cache/…`; se refresca con bump de versión + `claude plugin update`.)

### El badge `failed` del panel Desarrollador es un health-check que expira en arranque en frío (no un fallo)
- **Resultado:** un server de extensión Python (p.ej. `expedientes-xl`) puede salir `failed` / disparar el popup "Could not connect" aunque **conecte y sirva**. El health-check inicial del panel expira antes de que el proceso termine el arranque en frío bajo contención (la app levanta todos los servers a la vez), y **no re-evalúa el badge** cuando conecta segundos después. Diagnóstico decisivo en `%APPDATA%\Claude\logs\main.log`: buscar `[LocalMcpServerManager] Connected to <server> (N tools)` — si aparece, el server está vivo. Afecta por igual a Gmail/Google (mismo patrón). Confirmado 2026-07-20 (MEJORAS #74).
- **Solución:** reducir el arranque ayuda (el `LazyOracle` bajó `expedientes-xl` de 9 s a 1 s → deja de disparar casi siempre), pero el badge en frío es del panel, no del código. Verificar con `main.log` / llamando una tool; ignorar el popup si el server conectó.

### Redirigir stderr (`2>>`) en la línea del intérprete mata el conector en Claude Code

- **Intentado:** arrancar los conectores MCP del despacho con la regla de oro que llevaban escrita desde el primer día en sus `run_server.bat`: «jamás redirigir `1>`, **solo** `2>>`», o sea `python server.py 2>>"%APPDATA%\Claude\<server>.log"` para tener un log de diagnóstico propio.
- **Resultado:** en **Claude Code** el conector no arranca: `CONNECTION_CLOSED: Connection closed`. En el log del propio server se ve que **no ha llegado a recibir `initialize`**: muere en `stdout.flush()` con `OSError: [Errno 22] Invalid argument` mientras contesta al primer mensaje del cliente. **Claude Desktop lo toleraba** —de ahí que la regla vieja sobreviviera un año—; el spawn de Claude Code no.
- **Confirmado:** 2026-08-31, con experimento de control: dos `.bat` idénticos salvo el `2>>`, registrados los dos con `claude mcp add` y medidos con `claude mcp list`. Sin la redirección, `✔ Connected`; con ella, `✘ CONNECTION_CLOSED`. No es el tamaño del log (el probe redirigía a un fichero nuevo) ni las barras del path ni `cmd /c` (un `.bat` sin redirección conecta bien).
- **Regla nueva:** en la línea que lanza el server, **ninguna** redirección, ni `1>` ni `2>>`. El stderr lo recoge el cliente. El diagnóstico propio del wrapper (esperas de montaje, resolución de intérprete) sí puede ir al log, pero **en líneas anteriores** a la del intérprete. Guards: `tests/test_mcp_wrappers.py::test_el_lanzamiento_no_redirige` (ninguna redirección ni tubería en la sentencia de lanzamiento) y `::test_el_lanzamiento_es_la_ultima_sentencia` (nada ejecutable detrás — sin este segundo, un `exit /b 0` detrás bastaba para que el primero mirase otra línea y pasara verde; lo encontró la R1).

### `mcp>=1.0` sin techo: `pip install --user mcp` trae 2.0 y apaga TODOS los conectores a la vez

- **Intentado:** nada, y eso es lo grave: los `requirements.txt` de los conectores declaraban `mcp>=1.0` (o `>=1.2.0`) sin techo, y un `pip install --user mcp` suelto el **2026-08-23** trajo **mcp 2.0.0** al site de usuario de Python 3.14.
- **Resultado:** mcp 2.0 **retiró `mcp.server.fastmcp`**, que es la API que usan los cuatro servers del despacho (`from mcp.server.fastmcp import FastMCP`). Tres conectores murieron **el mismo día y en silencio**, con `ModuleNotFoundError` enterrado en su log, y **por dos mecanismos distintos que conviene no confundir** (la primera redacción de esta entrada los metía a los tres en «resolvían por PATH», y eso era falso — lo señaló la R1): `expedientes-xl` y `email-export` sí resolvían por PATH (`%LOCALAPPDATA%\Python\bin`, y si no, `where python`); **`google-despacho` estaba cableado a una ruta absoluta** (`...\pythoncore-3.14-64\python.exe`) que resultó ser precisamente el intérprete envenenado. La clase que los une no es «el PATH»: es **elegir el intérprete por una ruta, sin comprobar que pueda arrancar el server**. El único que sobrevivió fue `gmail-multiaccount`, y solo porque su entrada de config apunta al `python.exe` del venv del repo en vez de pasar por el `.bat`.
- **Confirmado:** 2026-08-31 (diagnóstico). Reparación inmediata: `pip install --user "mcp>=1.28,<2"` en ese intérprete.
- **Regla nueva, y es la lección de verdad:** un wrapper **no debe resolver el intérprete por RUTA QUE EXISTE** (`%LOCALAPPDATA%\Python\bin\python.exe`, o el primer `where python` que no sea el stub de WindowsApps). Debe resolverlo por **CAPACIDAD**: probar `python -c "import mcp.server.fastmcp"` y **fallar ruidosamente** si no importa, en vez de lanzar un intérprete que arranca y muere. Esquivar el intérprete malo *por su nombre* —la asserción vieja `"WindowsApps" in txt`— solo cubre el ejemplo conocido; el de agosto no era el stub. **Y el corolario que costó una segunda vuelta:** cerrar la frontera no es **prohibir el PATH**. La primera versión de este arreglo lo prohibió, y con eso rompió el caso documentado del README del plugin —`expedientes-xl` se anuncia auto-contenido, «Python 3 + `pip install mcp`», sin clonar el repo—, que desde el bundle no tiene ningún venv relativo. El PATH vuelve a ser candidato; lo que no vuelve es aceptarlo sin probarlo.
- **Guards:** `tests/test_mcp_wrappers.py::test_la_capacidad_se_prueba_EJECUTANDO` (la sonda en una sentencia ejecutable, no en un `REM`), `::test_el_pin_de_mcp_excluye_de_verdad_la_2` (se **evalúa** el especificador con `packaging`: `mcp>=1,<20` contiene la subcadena `<2` y admite la 2.0), y sobre todo `::test_sin_interprete_capaz_el_wrapper_FALLA_RUIDOSAMENTE`, que es de **comportamiento**: ejecuta el wrapper con el entorno envenenado y exige que salga con código ≠ 0, sin escribir en stdout y nombrando la palanca accionable. Los textuales son burlables; ese no.
...
- **Solución (camino rápido):** ejecutar la MISMA lógica **en local sobre el montaje Drive for Desktop (`G:`)** — filesystem instantáneo + sync en bloque en segundo plano. Disparo local: CLI `scripts/sala_lectura.py` / skill en Claude Code local. La vía Cowork/conector queda solo como **fallback puro-nube** (sin montaje), asumiendo su lentitud. Pivote a "motor local plano primario" PENDIENTE de decisión (¿tiene el equipo el montaje `G:`?); ver `PLAN.md` `[SIGUIENTE-SALA-UNICA-PLANA]`.
  - **Corregido el 2026-09-13 (fila #30, `[APER-70]`): el botón de Streamlit ya NO es un disparo.** Esta línea lo listaba como tercera vía. Llamaba a `core/sala_lectura.py`, declarado `[DEPRECADO 2026-06-18]`, y quien lo pulsaba no tocaba código; además casi nunca podía terminar, porque el clasificador determinista manda a residuo todo lo que no case unas 30 palabras clave y `organizar` se detiene pidiendo una sesión de Claude. Hoy el expander **solo lee** el estado de la sala. Quedan las dos vías de arriba, y el constructor que gobierna es la skill.
...
### Módulo de correo (nest-mail / Roundcube) — el write del relate/adjuntar NO es REST nest-mail ni AppSync  [⚠️ **DEAD END REVERTIDO el 2026-09-07**: `MailRoundcube` SÍ funciona por REST — ver el último punto y `INTEGRACION_SUDESPACHO.md §10.10`]
...
- **✅ RESUELTO 2026-07-19 (HAR `judicial_648.har`, relate+adjuntar manual):** el write **no** es `MailRoundcube` de api-crm (candidato **descartado**) ni `PUT /api/mail/{id}` de nest-mail ni AppSync. Lo ejecuta un **plugin propio de Roundcube**: `POST roundcube.sudespacho.net/?_task=mail&_action=plugin.sudespacho_asignaa_{set_registros_seleccionados|set_adjuntos_relacionar_crm}` (form-urlencoded). **Auth = sesión web de Roundcube** (cookies `roundcube_sessid`/`sessauth` + `X-Roundcube-Request`), host legacy `.sudespacho.net`. **La llave es el Message-ID RFC** (`<...@...>`), aceptado directamente → **sí hay puente Gmail↔CRM** (corrige el punto anterior sobre nest-mail). Contrato completo (acciones + params) en `INTEGRACION_SUDESPACHO.md §10.10`. **Bloqueo restante de F3** = confirmar si la sesión legacy (`check-legacy`/PHPSESSID) sirve para el plugin o exige login Roundcube propio (spike A-vs-C).
- **⚠️ Vía a EVITAR (spike en vivo 2026-07-19):** NO reproducir la sesión de Roundcube **headless** (con `requests`) regenerando el `dataHash`. El acceso al webmail es **SSO por `init.php?randomvar=&dataHash=…`**, donde el `dataHash` es un blob **cifrado en el cliente** (JS del CRM, ~620 chars) que empaqueta las credenciales IMAP de la cuenta; Roundcube lo descifra (`dataHashDecrypted`). Replicarlo = reverse-engineering frágil del cifrado + **manejar credenciales de correo** → frágil e inseguro. **Vía correcta (F3): webview** — dejar que el propio CRM haga el SSO en un navegador embebido y llamar al plugin por `fetch` (confirmado en vivo: `get_relaciones` → 200 + JSON, con `X-Roundcube-Request=rcmail.env.request_token`). Auth y contrato → `INTEGRACION_SUDESPACHO.md §10.10`. **⚠️ Matizado el 2026-09-07:** sigue siendo cierto que no se debe reproducir el `dataHash`; lo que ya NO es cierto es que haga falta — ver el punto siguiente.
- 🔴 **DEAD END REVERTIDO (2026-09-07) — el descarte de `MailRoundcube` era mío, no del CRM.** El módulo **`MailRoundcube` de api-crm SÍ existe y SÍ escribe**, con la `x-api-key` de siempre y **sin sesión de Roundcube**: `GET /api/mail/findRelations/{base64(MsgID)}?account=` · `POST /api/mail/relate/selected` · `POST /api/mail/relate/attachments` · `POST /api/mail/findAssigned` · `POST /api/mail/attachments`. Medido de punta a punta contra el expediente de prueba 636: relate verificado por relectura, adjuntar verificado por censo del gestor documental. `cookies`/`dataHash` se exigen presentes y **funcionan vacíos**. Contrato completo en `INTEGRACION_SUDESPACHO.md §10.10`; diseño en la rev. 3 del spec F3.
  - **El error de método, que es lo que hay que recordar de esta entrada:** el HAR del 2026-07-19 probó **qué hace la interfaz** y de ahí se concluyó **qué permite la API**. No es lo mismo, y aquí costó siete semanas de diseño levantado sobre una restricción inexistente (webview, miniapp con navegador embebido, parseo de HTML, un «Track 1 de auth» que figuraba como bloqueo de F3).
  - **Y estaba declarado desde el principio:** los cinco endpoints salen en `/api/docs.json`, que es **público** y es la Fase A del atlas que este repo genera y versiona (`docs/CRM_SUDESPACHO_ATLAS.md`, sección MailRoundcube). Nadie lo abrió. Es la trampa exacta que avisan `INTEGRACION_SUDESPACHO.md §14.6` y `CLAUDE.md`: **leer el contrato declarado antes de sondear en vivo**, y no leer «no lo vi» como «no está».
  - **El aviso venía de fuera:** El Contable archivó en su spec F10 (2026-08-31) que `POST /api/mail/relate/attachments` «huele a la dirección contraria, o sea a correo entrante». Tenía razón y nadie siguió el hilo hasta hoy.
  - **Lo que sigue sin probarse:** que `cookies`/`dataHash` vacíos valgan mañana (es comportamiento observado, no contrato prometido); la idempotencia del adjuntar; y el caso `expedientes_judiciales` (la prueba fue sobre un extrajudicial).
