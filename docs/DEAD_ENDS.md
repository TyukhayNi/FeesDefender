...
- **Resultado:** mcp 2.0 **retiró `mcp.server.fastmcp`**, que es la API que usan los cuatro servers del despacho (`from mcp.server.fastmcp import FastMCP`). Los tres conectores que resolvían el intérprete por PATH (`google-despacho`, `expedientes-xl`, `email-export`) murieron **el mismo día y en silencio**, con `ModuleNotFoundError` enterrado en su log. El único que sobrevivió fue `gmail-multiaccount`, y solo porque su entrada de config apunta al `python.exe` del venv del repo en vez de pasar por el `.bat`.
- **Confirmado:** 2026-08-31 (diagnóstico). Reparación inmediata: `pip install --user "mcp>=1.28,<2"` en ese intérprete.
- **Regla nueva, y es la lección de verdad:** un wrapper **no debe resolver el intérprete por RUTA QUE EXISTE** (`%LOCALAPPDATA%\Python\bin\python.exe`, o el primer `where python` que no sea el stub de WindowsApps). Debe resolverlo por **CAPACIDAD**: probar `python -c "import mcp.server.fastmcp"` y **fallar ruidosamente** si no importa, en vez de lanzar un intérprete que arranca y muere. Esquivar el intérprete malo *por su nombre* —la asserción vieja `"WindowsApps" in txt`— solo cubre el ejemplo conocido; el de agosto no era el stub. Guards: `tests/test_mcp_wrappers.py` (`…_se_verifica_por_capacidad`, `…_sin_fallback_ciego_al_python_del_path`, `…_mcp_pinneado_por_debajo_de_2`).
