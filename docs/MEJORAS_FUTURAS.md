...
## 213. Drive for Desktop renombra bajo los pies: la custodia recorre y abre, y el pull duplica en cada ronda
...
## 214. `_MAGIC_BYTES` solo conoce firmas planas: un `.docx` sin extensión es `sin_soporte` y nadie lo lee
...
**De qué frontera es esto un ejemplo.** La misma que `MEJORAS #213` y `MEJORAS #190`: **E&V sube
...
**Disparador de promoción.** Alto, y sube junto al punto 1 de `MEJORAS #213`: los dos se disparan
...
## 215. `verificar_sala.py` cuenta su propio aviso como un problema, y el conteo manda a buscar un fantasma
...
## 216. `crm_colaboradores_firmas` lee el `href` del `mailto:`, no la dirección visible, y propone escribir el teléfono de A en la ficha de B
...
## 217. El alta por API manda la cuantía como ENTERO: los céntimos del principal reclamado se pierden en silencio
