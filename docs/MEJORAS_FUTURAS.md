...
## 304. `verificar_apertura` C1: los ficheros nativos de Google salen a la vez como faltantes y como sobrantes
...
## 305. `verificar_apertura` C2: todo fichero con formato ZIP sale «sin explicar» aunque sea el relleno de `#225`
...
## 306. `plan` de la sala de máquina: el gate que anuncia es un `.md` que `apply` no lee, y no es «instantáneo»
...
## 307. Un PDF con el texto en trazos vectoriales sale `empty` del OCR, y la vía de visión no se puede cablear desde el CLI
...
## 308. Las unidades de Madrid no llevan « - »: `codigo_de_unidad` no deriva `MaRS<N>` y `--codigo-caso` es siempre obligatorio
