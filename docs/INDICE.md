...
|---|---|---|
| `handoff-2026-08-14-migracion-procesal-continuacion-tnm33.md` | activo | Continuación de la migración de capacidad al perfil `Nikolai Tyukhay 1` (procesal@tyukhay.legal), tras cerrar desde ese perfil F.4.2, F.4.4, F.2, F.4.3, C.4.b y F.1 del checklist v5. Documenta los tres puntos que solo se cierran desde `tnm33` (F.4.1, E.6, E.1) y los hallazgos nuevos (`FEESDEFENDER_ACTOR` no llega a procesos no interactivos, `rclone` faltaba, `reportlab` falta en `requirements.txt`). |
