...
| `core/intake_manifest.py` — schema o reglas reconcile | `core/sync_sudespacho.pull_expediente_v2`, futuros pulls v2 (intake_drive si se migra), `core/email_atomize/{dedup,attachments,inline}.py` (usan `compute_sha256_bytes`) |
| `core/email_export.py` — `iter_nested_originals` / `message_id_of` / `parse_headers` / `split_eml` / `iter_body_text` / `_slug_descripcion` | `core/email_atomize/{extract,inline,bodies,attachments,render}.py` (motor de atomización a nivel de mensaje los reutiliza; no reimplementa el rebanado byte-fiel ni el slug) |
| `core/email_atomize/` — formato de salida `01_Procesado/Emails/` (frontmatter `.md`, `corpus.jsonl`, `_registro.json` IDs congelados, `_revision/`) | `scripts/atomize_emails.py` (CLI), futura skill/UI; consumidores aguas abajo que citen `MSG-NNNNN`/`ATT-NNNNN` (deben permanecer estables); spec/plan en `docs/superpowers/{specs,plans}/2026-06-2{4,5}-email-atomize-*.md` |
