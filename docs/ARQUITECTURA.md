...
| `core/abrir_caso.py` — cerebro PURO de `abrir-caso` (`resolver_identidad`/`plan_intake`/`reconcile`/`crm_payload`) | `scripts/abrir_caso.py` (orquestador CLI que lo consume), `tests/test_abrir_caso.py`. Depende de `core/config` (`posicion_de_tipo`) y `core/sudespacho_create` (DTO + `tag_defaults_for_tipo_caso`, import perezoso) |
