...
| `core/case_manager.py` — `CAMPOS_ACTUALIZABLES` + `_HOGARES_SUPERIORES` **[FUENTE ÚNICA de qué puede escribir `update_meta` y en qué hogares]** | `tests/test_caso_md_update_meta.py` (incluye el guard que recorre `_frontmatter_del_indice` y se pone rojo si un campo de la lista blanca gana un hogar superior que nadie ha enumerado), `scripts/abrir_caso.py` (`_alta_crm`). Añadir un campo **no** es ampliar la constante: hay que saber **cuáles son sus hogares** en el frontmatter — `referencia_crm` tiene dos y `cuantia` uno. Los campos sin actualizador están en `MEJORAS #246`; por qué la lista no crece «ya que estamos», en `docs/superpowers/specs/2026-09-11-cuantia-en-caso-md-comparar-no-localizar-design.md` §3.1 |
...
| `core/intake_log.py` — `INTAKE_EVENTS` | callers que emiten eventos: `core/sync_sudespacho.pull_expediente_v2`, `streamlit_app.py` (expander "📂 Subir al árbol CRM" emite `upload_manual` con `details.destination 05_CRM/...`), `core/email_export.py` (emite `upload_email`, `upload_drive_link` y `email_excluido_ruido` — este último es el ÚNICO rastro durable del correo de administración que el filtro de la acción 6a no deposita) |
...
| `core/whatsapp_export.py` — `elegir_chat` / `resolver_adjunto` / `_limpiar_ref` **[FUENTE ÚNICA de qué fichero es el chat de un export de WhatsApp y de a qué fichero apunta una referencia de adjunto]** (`MEJORAS #285`, `#236`) | Sus consumidores, que no deben volver a decidirlo por su cuenta: `core/whatsapp_intake.py` (`_find_chat_txt`, los faltantes y el `tipo_contenido` del manifiesto), `core/whatsapp_atomize/pipeline.py` (`chat_txt_de`, `descubrir_chats`), `core/whatsapp_atomize/adjuntos.py` y `core/whatsapp_atomize/propuesta_identidades.py`. La regla de **custodia** —el primer `.txt` aunque no se interprete— vale **solo para el intake**: el atomizador la apaga con `custodia=False`. **Dos copias que divergen a propósito**, y que un cambio de la regla NO arrastra: `core/adjuntos_contenido/zips.py::_como_whatsapp` (`#55`) y el `preclasificar` de la skill `organizar-sala-lectura`, que solo reconoce `_chat.txt` (cambiarlo es editar la skill y re-importarla en Cowork). Tests: `tests/test_whatsapp_export.py` (`TestElegirChat`, `TestMarcasInvisiblesEnAdjuntos`) |
| `core/ocurrencias_crm.py` — esquema de `00_Input/_ocurrencias_crm.json` (clave lógica `crm:<exp>:<doc_id>`, revisiones, estados `listada`/`materializada`/`superseded`) | **Escritor único:** `core/sync_sudespacho.pull_expediente_v2` (`registrar_listada` ANTES del filtro `only_doc_ids`; `registrar_materializada` en el bucle). **Lector real desde la pieza 4a (2026-09-09):** `core/procedimiento/universo.py` (spec `2026-07-27-vista-procesal-*`, §2.1) — precedencia **CRM > ocurrencias > `_intake_hashes.json`**. Consume `listadas(expediente_id)` y `materializadas(expediente_id)`, que **filtran por expediente**: un caso puede tener varios expedientes CRM enlazados (W-02VEKE tiene dos, de 5 y 76 documentos) y mezclarlos falsea el universo. NO amplía el manifiesto de intake: este resuelve `doc_id → ruta`, que el manifiesto (indexado por SHA) no puede cuando dos `doc_id` comparten contenido **y** ruta |
| `core/email_export.py` — `iter_nested_originals` / `message_id_of` / `parse_headers` / `split_eml` (envoltorio de `particionar_eml`, que es la primitiva desde la acción 6b y añade `es_firma`/`es_correo` por adjunto; `split_eml` conserva su contrato de 3 elementos a propósito, para que estos consumidores no cambien) / `iter_body_text` / `_slug_descripcion` | `core/email_atomize/{extract,inline,bodies,attachments,render,entregas}.py` (motor de atomización a nivel de mensaje los reutiliza; no reimplementa el rebanado byte-fiel ni el slug; `entregas` reutiliza `_slug_descripcion`) |
...
| `core/procedimiento/` — la vista procesal que **solo lee** (pieza 4a, 2026-09-09). `sede` decide qué copia está autorizada, `carpetas` fija las cinco de fase, `universo` cruza los tres conjuntos del CRM, `mapa` valida la gramática, `artefacto` elige el representante de cada documento, `vista`/`borrador` producen la salida y `__init__` es la fachada (`informe`, `borrador_mapa`) | `scripts/procedimiento.py` (CLI, **sin mutex a propósito: nada escribe**), `tests/test_procedimiento_*.py`. **Consume, no reimplementa:** `core/casos.workspace` (la autorización la da el `CaseWorkspaceResolver`, NUNCA el catálogo ni el mutex local), `core/ocurrencias_crm` (fila de arriba), `core/sala_maquina.cobertura_desde_dicts` y `.registro_legible`. **La propiedad que sostiene su presupuesto de 1 ronda —que la pieza no escribe— la prueba `tests/test_procedimiento_sin_escritura.py` por AST**, no un comentario: si algún día un módulo de aquí abre un fichero en modo escritura, ese guard es el que se pone rojo. `CARPETAS_FASE` es **espejo** de los subdestinos de `registrar_outputs.py` de las skills: ampliar uno sin el otro los desalinea (pieza aparte, `docs/superpowers/plans/2026-09-09-destinos-de-fase-registrar-outputs.md`) |
