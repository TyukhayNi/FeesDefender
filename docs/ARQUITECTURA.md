...
| `core/email_atomize/` — formato de salida `01_Procesado/Emails/` (frontmatter `.md`, `corpus.jsonl`, `_registro.json` IDs congelados, `_revision/`) | `scripts/atomize_emails.py` (CLI), futura skill/UI; consumidores aguas abajo que citen `MSG-NNNNN`/`ATT-NNNNN` (deben permanecer estables); spec/plan en `docs/superpowers/{specs,plans}/2026-06-2{4,5}-email-atomize-*.md`. `scripts/sala_maquina.py::apply` (cableado 2026-07-28: lo llama antes del OCR y declara `status` en `atomizado_email`; spec/plan `…-cableado-atomize-sala-maquina*`) |
